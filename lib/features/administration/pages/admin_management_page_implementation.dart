...
class _AdminManagementPageState extends State<AdminManagementPage> {
  ...
  Color _statusColor(String status,
      {bool escaped = false, bool hasDevice = false}) {
  ...
  Color _statusSoft(Color color) => Color.alphaBlend(
      color.withValues(alpha: 0.09), Theme.of(context).colorScheme.surface);
  ...
  List<Map<String, dynamic>> _asLocations(dynamic data) => _asList(data)
      .whereType<Map>()
  ...
  Future<void> _request(String method, String path,
    ...
    try {
      await _dio.request<dynamic>(path,
  ...
  Future<void> _updateWorkforce(String id, Map<String, dynamic> patch) async {
    ...
    if (mounted)
      setState(
          () => _workforceControls[id] = {...previous, ...patch, 'id': id});
    try {
      await _dio
          .patch('/api/workforce/live', data: {'employeeId': id, ...patch});
      await _load();
    } catch (error) {
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _showWorkforceControls(String id, String name) async {
    ...
    var isVip = current['isVip'] == true,
        autoCheckIn = current['autoCheckIn'] == true,
        autoCheckOut = current['autoCheckOut'] == true;
            ...
            builder: (context, setDialogState) => AlertDialog(
                  ...
                  content: Column(mainAxisSize: MainAxisSize.min, children: [
                    SwitchListTile(
                        ...
                        subtitle: const Text(
                            'تمييز الموظف كحالة أولوية في نظام الحضور.'),
                    ...
                    SwitchListTile(
                        ...
                        subtitle: const Text(
                            'السماح بمحرك الحضور التلقائي لهذا الموظف.'),
                        onChanged: (v) =>
                            setDialogState(() => autoCheckIn = v)),
                    SwitchListTile(
                        contentPadding: EdgeInsets.zero,
                        value: autoCheckOut,
                        title: const Text('الانصراف التلقائي'),
                        subtitle: const Text(
                            'السماح بمحرك الانصراف التلقائي لهذا الموظف.'),
                        onChanged: (v) =>
                            setDialogState(() => autoCheckOut = v)),
                  ]),
                  actions: [
                    TextButton(
  ...
  List<DropdownMenuItem<String>> _timeItems() => [
        for (var h = 0; h < 24; h++)
          for (var m in const [0, 30])
            DropdownMenuItem(
                value:
  ...
  List<DropdownMenuItem<String>> _locationItems() => [
        const DropdownMenuItem<String>(value: '', child: Text('المقر الرئيسي')),
        ..._locations
            .where((l) => '${l['name'] ?? ''}'.trim() != 'المقر الرئيسي')
            .map((l) => DropdownMenuItem<String>(
                value: '${l['id'] ?? ''}',
                child: Text('${l['name'] ?? 'موقع'}')))
      ];
  ...
  Future<void> _editEmployee(Map<String, dynamic> source) async {
    ...
    final grace =
        TextEditingController(text: '${employee['gracePeriodMinutes'] ?? 0}');
    final earlyCheckout = TextEditingController(
        text:
            '${employee['earlyCheckoutMinutes'] ?? employee['earlyCheckoutGrace'] ?? 0}');
    final rotationStart =
        TextEditingController(text: '${employee['rotationStartDate'] ?? ''}');
    final specialties = TextEditingController(
        text: employee['specialties'] is List
            ? (employee['specialties'] as List).map((e) => '$e').join(', ')
            : '${employee['specialties'] ?? 'general'}');
    ...
    if (scheduleType != 'ADMIN' && scheduleType != 'ROTATION')
      scheduleType = 'ADMIN';
    ...
    var workDays = <int>{
      if (rawDays is List)
        ...rawDays.map((e) => int.tryParse('$e')).whereType<int>()
    };
    ...
    try {
      final result = await showDialog<bool>(
          ...
          builder: (dialogContext) => StatefulBuilder(
              builder: (context, setDialogState) => AlertDialog(
                    title: Text('تعديل الموظف · ${employee['name'] ?? ''}',
                    ...
                    content: SizedBox(
                        ...
                        child: SingleChildScrollView(
                            child: Column(
                                ...
                                children: [
                              const Text('بيانات الموظف الأساسية',
                                  style:
                              ...
                              TextField(
                                  ...
                                  obscureText: true,
                                  decoration: const InputDecoration(
                                      labelText: 'PIN جديد (اختياري)',
                                      hintText:
                                          'اتركه فارغًا للإبقاء على PIN الحالي')),
                              const SizedBox(height: 8),
                              ...
                              Row(children: [
                                ...
                                Expanded(
                                    child: DropdownButtonFormField<String>(
                                        initialValue: _timeItems()
                                                .any((i) => i.value == endTime)
                                            ? endTime
                                            : '16:00',
                                        decoration: const InputDecoration(
                              ...
                              const SizedBox(height: 8),
                              DropdownButtonFormField<String>(
                                  initialValue: _locationItems()
                                          .any((i) => i.value == locationId)
                                      ? locationId
                                      : '',
                                  decoration: const InputDecoration(
                              ...
                              Row(children: [
                                ...
                                Expanded(
                                    child: TextField(
                                        ...
                                        keyboardType: TextInputType.number,
                                        decoration: const InputDecoration(
                                            labelText: 'سماح الانصراف المبكر')))
                              ]),
                                ...
                                Row(children: [
                                  Expanded(
                                      child: DropdownButtonFormField<int>(
                                          initialValue: rotationOn.clamp(1, 31),
                                          decoration: const InputDecoration(
                                          ...
                                          onChanged: (v) => setDialogState(() =>
                                              rotationOn = v ?? rotationOn))),
                                  const SizedBox(width: 8),
                                  Expanded(
                                      child: DropdownButtonFormField<int>(
                                          initialValue:
                                              rotationOff.clamp(0, 31),
                                          decoration: const InputDecoration(
                                              labelText: 'أيام الراحة'),
                                          items: [
                                            const DropdownMenuItem(
                                                value: 0,
                                                child: Text('بدون راحة')),
                                            for (var i = 1; i <= 31; i++)
                                              DropdownMenuItem(
                                                  value: i,
                                                  child: Text('$i يوم'))
                                          ...
                                          onChanged: (v) => setDialogState(() =>
                                              rotationOff = v ?? rotationOff)))
                                ]),
                                const SizedBox(height: 8),
                                TextField(
                                    controller: rotationStart,
                                    decoration: const InputDecoration(
                                        labelText: 'تاريخ أول مناوبة',
                                        hintText: 'YYYY-MM-DD'))
                              ],
                              const SizedBox(height: 12),
                              TextField(
                      ...
                      FilledButton.icon(
                          onPressed: () {
                            if (name.text.trim().isEmpty ||
                                job.text.trim().isEmpty ||
                                (scheduleType == 'ADMIN' && workDays.isEmpty))
                              return;
      ...
      final payload = <String, dynamic>{
        ...
        'rotationStartDate': rotationStart.text.trim().isEmpty
            ? null
            : rotationStart.text.trim(),
      ...
      await _dio.put('/api/employees/$id/checkout-policy', data: {
        'earlyCheckoutMinutes':
            (int.tryParse(earlyCheckout.text.trim()) ?? 0).clamp(0, 1440)
      });
      await _load();
      if (mounted)
  ...
  Future<void> _addEmployee() async {
    final name = TextEditingController(),
        ...
        earlyCheckout = TextEditingController(),
        rotationStart = TextEditingController(),
        specialties = TextEditingController(text: 'general');
    var status = 'active',
        scheduleType = 'ADMIN',
        startTime = '08:00',
        endTime = '16:00',
        locationId = '';
    var rotationOn = 7, rotationOff = 7;
    ...
    try {
                        ...
                        child: SingleChildScrollView(
                            child: Column(
                              ...
                              TextField(
                                  ...
                                  decoration: const InputDecoration(
                                      labelText: 'رمز PIN',
                                      hintText: '4 أحرف/أرقام على الأقل')),
                              const SizedBox(height: 8),
                              DropdownButtonFormField<String>(
                                  initialValue: status,
                                  decoration: const InputDecoration(
      ...
      if (result != true) return;
      final response = await _dio.post('/api/employees', data: {
        'name': name.text.trim(),
        'jobNumber': job.text.trim(),
        'pin': pin.text.trim(),
        'status': status,
      ...
      final employee = _asMap(response.data)['employee'];
      final employeeId = employee is Map ? '${employee['id'] ?? ''}' : '';
      final early = int.tryParse(earlyCheckout.text.trim()) ?? 0;
      if (employeeId.isNotEmpty)
        await _dio.put('/api/employees/$employeeId/checkout-policy',
            data: {'earlyCheckoutMinutes': early.clamp(0, 1440)});
      await _load();
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _deleteEmployee(String id, String name) async {
    final confirmed = await showDialog<bool>(
        ...
        builder: (c) => AlertDialog(
                ...
                content:
                    Text('سيتم حذف حساب «$name». لا يمكن التراجع عن العملية.'),
                actions: [
                  TextButton(
  ...
  Future<void> _directAttendance(String id, String name, String type) async {
    ...
    final confirmed = await showDialog<bool>(
        context: context,
        builder: (c) => AlertDialog(
                title: Text('$label مباشر'),
                content: Text('تأكيد $label للموظف «$name»؟'),
                actions: [
                  TextButton(
                      onPressed: () => Navigator.of(c).pop(false),
                      child: const Text('إلغاء')),
    ...
    try {
      await _dio.post(
          type == 'check-in'
              ...
              : '/api/workforce/live',
          data: {'employeeId': id, 'type': type});
      await _load();
      if (mounted)
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('تم تسجيل $label')));
    } catch (error) {
      if (mounted)
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text(_errorMessage(error))));
  ...
  Future<void> _changeEscape(String id, String name, String status) async {
    ...
    final confirmed = await showDialog<bool>(
        context: context,
        builder: (c) => AlertDialog(
                title: Text(status == 'escaped' ? 'تسجيل هروب' : 'تسجيل عودة'),
                content: TextField(
    ...
    try {
      await _dio.post('/api/escape-events', data: {
        'employeeId': id,
        'status': status,
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(
                status == 'escaped' ? 'تم تسجيل الهروب' : 'تم تسجيل العودة')));
    } catch (error) {
      if (mounted)
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text(_errorMessage(error))));
  ...
  Future<void> _employeeRequest(String id, String name, String type) async {
    ...
    try {
      final result = await showDialog<bool>(
          context: context,
          builder: (c) => AlertDialog(
                  title: Text('تسجيل $label · $name'),
                  content: Column(mainAxisSize: MainAxisSize.min, children: [
                    TextField(
                        controller: start,
                        decoration:
                            const InputDecoration(labelText: 'تاريخ البداية')),
                    TextField(
                        controller: end,
                        decoration:
                            const InputDecoration(labelText: 'تاريخ النهاية')),
                    TextField(
                        controller: reason,
                        decoration: const InputDecoration(
  ...
  Widget _card(Widget child) =>
      Card(margin: const EdgeInsets.only(bottom: 10), child: child);
  ...
  Widget _employeesView() {
    final filtered = _employees.where((raw) {
      ...
      final device = '${employee['deviceLabel'] ?? employee['deviceId'] ?? ''}'
          .toLowerCase();
      ...
      return (query.isEmpty ||
          ...
          (_employeeSchedule == 'all' || schedule == _employeeSchedule);
    }).toList();
    final active = _employees
            .where((r) => r is Map && '${r['status'] ?? 'active'}' == 'active')
            .length,
        inactive = _employees.length - active,
        devices = _employees
            .where(
                (r) => r is Map && '${r['deviceId'] ?? ''}'.trim().isNotEmpty)
            .length,
        vip = _employees
            .where((r) =>
                r is Map &&
                _employeeWithControls(Map<String, dynamic>.from(r))['isVip'] ==
                    true)
            .length,
        escaped = _employees
            .where((r) =>
                r is Map &&
                '${r['escapeStatus'] ?? r['fieldStatus'] ?? ''}'
                        .toLowerCase() ==
                    'escaped')
            .length;
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Row(children: [
        _metric(
            'إجمالي الموظفين', '${_employees.length}', Icons.groups_rounded),
        const SizedBox(width: 8),
        _metric('فعال', '$active', Icons.check_circle_outline_rounded),
        const SizedBox(width: 8),
        _metric('موقوف', '$inactive', Icons.pause_circle_outline_rounded)
      ]),
      const SizedBox(height: 8),
      Row(children: [
        _metric('أجهزة موثقة', '$devices', Icons.smartphone_rounded),
        const SizedBox(width: 8),
        _metric('VIP مفعل', '$vip', Icons.star_rounded),
        const SizedBox(width: 8),
        _metric('هروب', '$escaped', Icons.directions_walk_rounded)
      ]),
      const SizedBox(height: 12),
      _searchBox(
          hint: 'بحث بالاسم أو الرقم أو الجهاز',
          onChanged: (v) => setState(() => _employeeQuery = v)),
      const SizedBox(height: 8),
      Wrap(spacing: 8, runSpacing: 8, children: [
        _filterChip('كل الحالات', 'all', _employeeStatus,
            (v) => setState(() => _employeeStatus = v)),
        _filterChip('فعال', 'active', _employeeStatus,
            (v) => setState(() => _employeeStatus = v)),
        _filterChip('موقوف', 'suspended', _employeeStatus,
            (v) => setState(() => _employeeStatus = v))
      ]),
      const SizedBox(height: 8),
      Wrap(spacing: 8, runSpacing: 8, children: [
        _filterChip('كل أنواع الدوام', 'all', _employeeSchedule,
            (v) => setState(() => _employeeSchedule = v)),
        _filterChip('إداري', 'ADMIN', _employeeSchedule,
            (v) => setState(() => _employeeSchedule = v)),
        _filterChip('تناوبي', 'ROTATION', _employeeSchedule,
            (v) => setState(() => _employeeSchedule = v))
      ]),
      const SizedBox(height: 12),
      Row(children: [
      ...
      const SizedBox(height: 10),
      if (filtered.isEmpty)
        _empty('لا توجد نتائج مطابقة.')
      else
        ...filtered.map((raw) {
          final employee =
              _employeeWithControls(Map<String, dynamic>.from(raw as Map));
          final id = '${employee['id'] ?? ''}';
          final name = '${employee['name'] ?? 'بدون اسم'}'.trim();
          final status = '${employee['status'] ?? 'active'}';
          final isVip = employee['isVip'] == true,
              autoIn = employee['autoCheckIn'] == true,
              autoOut = employee['autoCheckOut'] == true;
          final escapeStatus =
              '${employee['escapeStatus'] ?? employee['fieldStatus'] ?? 'none'}'
                  .toLowerCase();
          final isEscaped =
              escapeStatus == 'escaped' || escapeStatus == 'escape';
          final hasDevice = '${employee['deviceId'] ?? ''}'.trim().isNotEmpty;
          final stateColor =
              _statusColor(status, escaped: isEscaped, hasDevice: hasDevice);
          final stateLabel = isEscaped
              ? 'هارب'
              : status.toLowerCase() == 'active'
                  ? (hasDevice ? 'فعال ومرتبط' : 'فعال وغير مرتبط')
          ...
          return _card(Container(
              decoration: BoxDecoration(
                  border:
                      Border(right: BorderSide(color: stateColor, width: 4)),
                      ...
                      children: [
                        ListTile(
                            leading: CircleAvatar(
                                backgroundColor: _statusSoft(stateColor),
                            ...
                            subtitle: Padding(
                                ...
                                child:
                                  ...
                                  Text(
                                      '${employee['jobNumber'] ?? '—'} · ${employee['scheduleType'] == 'ROTATION' ? 'تناوبي' : 'إداري'}'),
                                  Container(
                                      padding: const EdgeInsets.symmetric(
                                          horizontal: 7, vertical: 3),
                                      decoration: BoxDecoration(
                                          color: _statusSoft(stateColor),
                                          borderRadius:
                        ...
                        Wrap(spacing: 6, runSpacing: 6, children: [
                          _infoPill(
                              ...
                              employee['scheduleType'] == 'ROTATION'
                                  ? '${employee['rotationDaysOn'] ?? 7} عمل / ${employee['rotationDaysOff'] ?? 7} راحة'
                                  : '${employee['workStartTime'] ?? '--:--'} → ${employee['workEndTime'] ?? '--:--'}'),
                          _infoPill('التأخير',
                              '${employee['gracePeriodMinutes'] ?? 0} دقيقة'),
                          _infoPill('الموقع',
                              '${location?['name'] ?? 'المقر الرئيسي'}'),
                          if (employee['deviceId'] != null &&
                              '${employee['deviceId']}'.isNotEmpty)
                            _infoPill('الجهاز',
                                '${employee['deviceLabel'] ?? 'مرتبط'}'),
                          if (autoIn || autoOut)
                            _infoPill('الأتمتة',
                                '${autoIn ? 'حضور' : ''}${autoIn && autoOut ? ' + ' : ''}${autoOut ? 'انصراف' : ''}')
                        ]),
                        const SizedBox(height: 8),
                        SingleChildScrollView(
                            scrollDirection: Axis.horizontal,
                            child: Row(children: [
                              _smallAction('تعديل', Icons.edit_rounded,
                                  () => _editEmployee(employee)),
                              _smallAction(
                                  'إذن',
                                  Icons.event_available_rounded,
                                  () =>
                                      _employeeRequest(id, name, 'permission')),
                              _smallAction('إجازة', Icons.beach_access_rounded,
                                  () => _employeeRequest(id, name, 'leave')),
                              _smallAction(
                                  'تحضير',
                                  Icons.login_rounded,
                                  () =>
                                      _directAttendance(id, name, 'check-in')),
                              _smallAction(
                                  'انصراف',
                                  Icons.logout_rounded,
                                  () =>
                                      _directAttendance(id, name, 'check-out')),
                              _smallAction(
                                  isEscaped ? 'عودة' : 'هروب',
                                  isEscaped
                                      ? Icons.rotate_left_rounded
                                      : Icons.directions_walk_rounded,
                                  () => _changeEscape(id, name,
                                      isEscaped ? 'returned' : 'escaped')),
                              _smallAction(
                                  'قوى العمل',
                                  Icons.auto_awesome_rounded,
                                  () => _showWorkforceControls(id, name)),
                              _smallAction(
                                  'الجهاز',
                                  Icons.smartphone_rounded,
                                  () => _request(
                                      'DELETE', '/api/employees/$id/device')),
                              _smallAction('حذف', Icons.delete_outline_rounded,
  ...
  Widget _requestsView() {
    final filtered = _requests.where((raw) {
      ...
      final text =
          '${request['type'] ?? ''} ${request['employeeName'] ?? ''} ${request['employeeId'] ?? ''} ${request['reason'] ?? ''}'
              .toLowerCase();
      return (query.isEmpty || text.contains(query)) &&
          (_requestStatus == 'all' || status == _requestStatus);
    }).toList();
    final pending = _requests
        .where((r) => r is Map && '${r['status'] ?? 'pending'}' == 'pending')
        .length;
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Row(children: [
      ...
      const SizedBox(height: 8),
      _searchBox(
          hint: 'ابحث في نوع الطلب أو الموظف أو السبب',
          onChanged: (v) => setState(() => _requestQuery = v)),
      const SizedBox(height: 8),
      Wrap(spacing: 8, children: [
        _filterChip('الكل', 'all', _requestStatus,
            (v) => setState(() => _requestStatus = v)),
        _filterChip('معلق', 'pending', _requestStatus,
            (v) => setState(() => _requestStatus = v)),
        _filterChip('مقبول', 'approved', _requestStatus,
            (v) => setState(() => _requestStatus = v)),
        _filterChip('مرفوض', 'rejected', _requestStatus,
            (v) => setState(() => _requestStatus = v))
      ]),
      const SizedBox(height: 12),
      if (filtered.isEmpty)
        _empty('لا توجد طلبات مطابقة.')
      else
        ...filtered.map((raw) {
              ...
              child: ListTile(
                  ...
                  trailing: status == 'pending'
                      ? PopupMenuButton<String>(
                          ...
                          itemBuilder: (_) => const [
                                PopupMenuItem(
                                    value: 'approved', child: Text('موافقة')),
                                PopupMenuItem(
                                    value: 'rejected', child: Text('رفض'))
                              ])
                      : null)));
        })
    ]);
  ...
  Widget _reportsView() {
    final successful = _audit
        .where((raw) =>
            raw is Map &&
            raw['result'] == 'success' &&
            (raw['action'] == 'check-in' || raw['action'] == 'check-out'))
        .toList();
    final checkIns =
            successful.where((r) => (r as Map)['action'] == 'check-in').length,
        checkOuts =
            successful.where((r) => (r as Map)['action'] == 'check-out').length;
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Row(children: [
        _metric(
            'سجلات ناجحة', '${successful.length}', Icons.fact_check_rounded),
        const SizedBox(width: 8),
        ...
        return _card(Container(
            ...
            child: ListTile(
                title: Text(
                    ...
                    style: const TextStyle(fontWeight: FontWeight.w800)),
                subtitle: Text(
                    '${item['timestamp'] ?? ''} · ${item['distanceMeters'] ?? 0} متر'))));
      })
    ]);
  }
  ...
  Widget _auditView() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Row(children: [
        ...
        _metric(
            'ناجح',
            '${_audit.where((r) => r is Map && r['result'] == 'success').length}',
            Icons.verified_rounded)
      ]),
      const SizedBox(height: 12),
      ..._audit.take(120).map((raw) {
        final item = Map<String, dynamic>.from(raw as Map);
        final color = _statusColor('${item['result'] ?? ''}');
        return _card(Container(
            decoration: BoxDecoration(
                border: Border(right: BorderSide(color: color, width: 3)),
                borderRadius: BorderRadius.circular(12)),
  ...
  Widget _adminsView() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Row(children: [
        _metric('الحسابات', '${_admins.length}',
      ...
      const SizedBox(height: 8),
      _searchBox(
          hint: 'ابحث بالاسم أو اسم المستخدم أو الصلاحية',
          onChanged: (v) => setState(() => _adminQuery = v)),
      const SizedBox(height: 12),
      if (filtered.isEmpty)
        _empty('لا توجد حسابات مطابقة.')
      else
        ...filtered.map((raw) {
          final admin = Map<String, dynamic>.from(raw as Map);
          final id = '${admin['id'] ?? ''}';
          final color =
              _statusColor(admin['active'] == false ? 'inactive' : 'active');
          return _card(Container(
              decoration: BoxDecoration(
                  border: Border(right: BorderSide(color: color, width: 3)),
                  borderRadius: BorderRadius.circular(12)),
              child: ListTile(
                  ...
                  trailing: Switch(
                      ...
                      onChanged: (v) => _request('PATCH', '/api/admins/$id',
                          data: {'active': v})))));
        })
    ]);
  }
