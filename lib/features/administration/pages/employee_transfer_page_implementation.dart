...
class _EmployeeTransferPageState extends State<EmployeeTransferPage> {
  ...
  bool _asBool(Object? value) {
    ...
    return const {'1', 'true', 'yes', 'y', 'on', 'نعم', 'مفعل', 'فعال', 'مميز', 'vip'}.contains(normalized);
  }
  ...
  Set<String> get _existingJobs => _employees
      .map((e) => '${e['jobNumber'] ?? ''}'.trim().toLowerCase())
      .where((e) => e.isNotEmpty)
      .toSet();

  List<EmployeeTransferRow> get _exportRows => _employees
      .map((e) {
        ...
        return EmployeeTransferRow(
          ...
          isVip: _asBool(e['isVip']),
          autoCheckIn: _asBool(e['autoCheckIn']),
          autoCheckOut: _asBool(e['autoCheckOut']),
        );
      })
      .where((e) => e.name.trim().isNotEmpty && e.jobNumber.trim().isNotEmpty)
      .toList();
  ...
  Future<void> _export() async {
    try {
      ...
      await SharePlus.instance.share(
        ShareParams(text: csv, subject: 'HADIR - تصدير الموظفين'),
      );
    } catch (error) {
  ...
  void _previewImport() {
    try {
      ...
      setState(() {
        _preview = preview;
        _error = null;
        _message = 'تم تحليل الملف: ${preview.validRows} صف صالح.';
      });
    } catch (error) {
      setState(() {
        _preview = null;
  ...
  Future<void> _import() async {
    ...
    final existing = {..._existingJobs};
    var added = 0;
    var failed = 0;
    var skipped = 0;
    try {
      for (final row in preview.rows) {
        final jobNumber = row.jobNumber.trim().toLowerCase();
        if (existing.contains(jobNumber)) {
          skipped++;
          continue;
        }
        try {
          await _dio.post('/api/employees', data: row.toJson());
          existing.add(jobNumber);
          added++;
        } catch (_) {
          failed++;
        }
      ...
      if (skipped > 0) details.add('تم تجاوز $skipped مكرر');
      if (failed > 0) details.add('فشل $failed');
      setState(() {
        _busy = false;
        _preview = null;
        _csvController.clear();
        _message = 'اكتمل الاستيراد: أضيف $added موظف${details.isEmpty ? '' : '، ${details.join('، ')}.'}'.replaceFirst(RegExp(r'\.\.'), '.');
      });
    } catch (error) {
      if (!mounted) return;
      setState(() {
        ...
        body: _loading
            ...
            : ListView(
                ...
                children: [
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              Expanded(child: _metric('الموظفون الحاليون', '${_employees.length}', Icons.groups_rounded)),
                              const SizedBox(width: 10),
                              Expanded(child: _metric('حقول النقل', '${EmployeeTransfer.headers.length}', Icons.table_chart_rounded)),
                            ],
                          ),
                          const SizedBox(height: 16),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          TextField(
                            ...
                            decoration: const InputDecoration(border: OutlineInputBorder(), hintText: 'الاسم,الرقم الوظيفي,نوع الدوام,...'),
                          ),
                          ...
                          if (preview != null) ...[
                            ...
                            const SizedBox(height: 12),
                            FilledButton.icon(
                              onPressed: _busy || preview.rows.isEmpty ? null : _import,
                              icon: _busy
                                  ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2))
                                  : const Icon(Icons.cloud_upload_rounded),
                              label: Text(_busy ? 'جاري الاستيراد…' : 'تأكيد الاستيراد'),
                            ),
                          ],
                        ],
                      ),
                  ...
                  if (_message != null) ...[
                    const SizedBox(height: 12),
                    Card(child: ListTile(leading: const Icon(Icons.check_circle_rounded), title: Text('$_message'))),
                  ],
                  if (_error != null) ...[
                    const SizedBox(height: 12),
                    Card(child: ListTile(leading: const Icon(Icons.error_rounded), title: Text('$_error'))),
                  ],
                ],
              ),
      ),
