...
class _ManagerWorkforceControlsPageState extends State<ManagerWorkforceControlsPage> {
  ...
  Future<void> _load({bool spinner = true}) async {
    ...
    try {
      ...
      final response = await _dio.get('/api/workforce/live');
      if (!mounted) return;
      final data = response.data;
      final employees = _asList(data);
      final controls = _asControls(data);
  ...
  Widget _card(Map<String, dynamic> employee) {
    ...
    final directInBusy = _busyEmployee == '${employee['id'] ?? ''}' && _busyField == 'check-in';
    final directOutBusy = _busyEmployee == '${employee['id'] ?? ''}' && _busyField == 'check-out';
            ...
            if (_value(employee, 'isVip'))
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: Colors.amber.withValues(alpha: .40)),
                ),
