...
class AdminMobileShell extends StatelessWidget {
  ...
  int _selectedIndex(BuildContext context) {
    ...
    if (path == '/admin/reports/archive' || path == '/manager/report-archive') return 5;
    if (path == '/admin/settings' || path == '/manager/settings') return 6;
    return -1;
  }

  void _go(BuildContext context, int index) {
    switch (index) {
      case 0: context.go('/manager');
      case 1: context.go('/manager/requests');
      case 2: context.go('/manager/employees');
      case 3: context.go('/manager/audit');
      case 4: context.go('/manager/reports');
      case 5: context.go('/manager/report-archive');
      case 6: context.go('/manager/settings');
    }
  }
      ...
      child: Row(
        children: [
          Expanded(
            child: Row(
              children: [
                Image.asset('assets/branding/hadir_logo_transparent.png', width: compact ? 34 : 38, height: compact ? 34 : 38, fit: BoxFit.contain),
                const SizedBox(width: 7),
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Text('حاضر', style: TextStyle(color: scheme.onSurface, fontSize: compact ? 18 : 20, height: 1, fontWeight: FontWeight.w900)),
                    const SizedBox(height: 4),
                    Text('HADIR  •  v1.1', textDirection: TextDirection.ltr, style: TextStyle(color: scheme.onSurfaceVariant, fontSize: compact ? 8 : 9, fontWeight: FontWeight.w700, letterSpacing: .35)),
                  ],
                ),
              ],
            ),
  ...
  Widget _navigation(BuildContext context, int selected) {
    final scheme = Theme.of(context).colorScheme;
    final items = const [
      (Icons.dashboard_outlined, Icons.dashboard_rounded, 'لوحة القيادة', '/manager'),
      (Icons.assignment_outlined, Icons.assignment_rounded, 'إدارة الطلبات', '/manager/requests'),
      (Icons.groups_outlined, Icons.groups_rounded, 'الموظفون', '/manager/employees'),
      (Icons.fact_check_outlined, Icons.fact_check_rounded, 'سجل التدقيق', '/manager/audit'),
      (Icons.bar_chart_outlined, Icons.bar_chart_rounded, 'التقارير', '/manager/reports'),
      (Icons.archive_outlined, Icons.archive_rounded, 'أرشيف التقارير', '/manager/report-archive'),
      (Icons.settings_outlined, Icons.settings_rounded, 'الإعدادات', '/manager/settings'),
    ];
    return Container(
      height: 54,
      decoration: BoxDecoration(color: scheme.surface.withValues(alpha: .98), border: Border(bottom: BorderSide(color: scheme.outlineVariant.withValues(alpha: .65)))),
        ...
        itemBuilder: (context, index) {
          ...
          return InkWell(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(color: active ? scheme.primary.withValues(alpha: .13) : Colors.transparent, borderRadius: BorderRadius.circular(12), border: Border.all(color: active ? scheme.primary.withValues(alpha: .35) : Colors.transparent, width: 1.1)),
  ...
  Widget _headerButton(BuildContext context, {required IconData icon, required String label, required bool compact, required VoidCallback onTap}) {
    ...
    return OutlinedButton(
      onPressed: onTap,
      style: OutlinedButton.styleFrom(foregroundColor: scheme.onSurface, fixedSize: Size(compact ? 70 : 80, 48), padding: EdgeInsets.zero, minimumSize: Size.zero, tapTargetSize: MaterialTapTargetSize.shrinkWrap, side: BorderSide(color: scheme.outlineVariant.withValues(alpha: .85)), backgroundColor: scheme.surface.withValues(alpha: .55), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, size: 20), const SizedBox(height: 2), Text(label, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, height: 1))]),
    );
  }
      ...
      builder: (sheetContext) => SafeArea(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            _optionTile(sheetContext, Icons.settings_outlined, 'الإعدادات', 'إعدادات النظام والإدارة', '/manager/settings'),
          ]),
        ),
      ),
    );
  ...
  Widget _themeSelector(BuildContext context) => AnimatedBuilder(
    ...
    builder: (context, _) {
      ...
      return Padding(
        ...
        child: DropdownButtonFormField<ThemeMode>(
          ...
          items: const [DropdownMenuItem(value: ThemeMode.system, child: Text('تلقائي حسب الجهاز')), DropdownMenuItem(value: ThemeMode.light, child: Text('الوضع الفاتح')), DropdownMenuItem(value: ThemeMode.dark, child: Text('الوضع الداكن'))],
  ...
  Widget _optionTile(BuildContext context, IconData icon, String title, String subtitle, String route) {
    ...
    return ListTile(
      ...
      onTap: () { Navigator.of(context).pop(); if (route == '/notifications') { Navigator.of(context).push(MaterialPageRoute(builder: (_) => const _AdminNotificationsPage())); } else { context.push(route); } },
    );
  }
}
