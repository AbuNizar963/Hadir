...
class _EmployeePremiumPageState extends State<EmployeePremiumPage> {
  ...
  static const _tabs = <({String title, String subtitle, IconData icon})>[
    (
      title: 'البطاقة',
      subtitle: 'بطاقتك الرقمية وQR',
      icon: Icons.badge_outlined
    ),
    (
      title: 'نظرة عامة',
      subtitle: 'ملخص العمل والالتزام',
      icon: Icons.insights_outlined
    ),
    (
      title: 'التقويم',
      subtitle: 'أيام الحضور والانصراف',
      icon: Icons.calendar_month_outlined
    ),
    (
      title: 'النشاط',
      subtitle: 'الخط الزمني للعمليات',
      icon: Icons.timeline_rounded
    ),
    (
      title: 'الدوام',
      subtitle: 'المناوبة وأوقات العمل',
      icon: Icons.schedule_outlined
    ),
    (
      title: 'الطلبات',
      subtitle: 'الإجازات والاستئذانات',
      icon: Icons.list_alt_outlined
    ),
    (
      title: 'الأمان',
      subtitle: 'الحساب والجهاز الموثق',
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (token == null || token.isEmpty)
        throw StateError('انتهت جلسة الموظف.');
      final api = HadirApi(token: token);
      final results = await Future.wait<dynamic>(
          [api.employeeProfile(), api.attendance(limit: 2000), api.requests()]);
      ...
      final attendance = _maps(results.length > 1 ? results[1] : null)
          .where((row) => '${row['employeeId'] ?? ''}' == employeeId)
          .toList();
      final requests = _maps(results.length > 2 ? results[2] : null)
          .where((row) => '${row['employeeId'] ?? ''}' == employeeId)
          .toList();
      if (!mounted) return;
      setState(() {
  ...
  DateTime? _timestamp(Map<String, dynamic> row) =>
      DateTime.tryParse('${row['timestamp'] ?? ''}')?.toLocal();
  ...
  List<Map<String, dynamic>> _forDay(DateTime day) {
    return _attendance.where((row) {
      ...
      ..sort((a, b) => (_timestamp(a) ?? DateTime(0))
          .compareTo(_timestamp(b) ?? DateTime(0)));
  }
        ...
        child: Row(children: [
          ...
          Expanded(
              child: Column(
                  ...
                  children: [
                Text('مركز ميزات الموظف',
                    style: TextStyle(
                        color: scheme.onSurface,
                        fontSize: 20,
                        fontWeight: FontWeight.w900)),
                const SizedBox(height: 4),
                Text('${_employee['name'] ?? 'الموظف'}',
                    style:
  ...
  Widget _content(BuildContext context) {
    switch (_tab) {
      case 0:
        return _digitalCard(context);
      case 1:
        return _overview(context);
      case 2:
        return _calendar(context);
      case 3:
        return _timeline(context);
      case 4:
        return _schedule(context);
      case 5:
        return _requestsView(context);
      default:
        return _security(context);
    }
  }
  ...
  Widget _digitalCard(BuildContext context) {
    ...
    return _card(context,
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _infoRow(
              context, 'الرقم الوظيفي', '${_employee['jobNumber'] ?? '—'}'),
          _infoRow(context, 'القسم',
              '${_employee['department'] ?? _employee['role'] ?? 'موظف'}'),
          _infoRow(context, 'الحالة', '${_employee['status'] ?? '—'}'),
          _infoRow(
              context, 'نوع الدوام', '${_employee['scheduleType'] ?? 'ADMIN'}'),
          if (id.isNotEmpty) ...[
            const SizedBox(height: 12),
            Container(
                ...
                child: Column(children: [
                  QrImageView(
                      ...
                      size: 180,
                      eyeStyle: QrEyeStyle(
                          eyeShape: QrEyeShape.square, color: scheme.onSurface),
                      dataModuleStyle: QrDataModuleStyle(
                          dataModuleShape: QrDataModuleShape.square,
                          color: scheme.onSurface)),
                  const SizedBox(height: 6),
                  Text('رمز تحقق بطاقة الموظف',
                      style: TextStyle(
                          color: scheme.onSurface,
                          fontSize: 11,
  ...
  Widget _overview(BuildContext context) {
    final days = <String>{
      for (final row in _attendance)
        '${_timestamp(row)?.year}-${_timestamp(row)?.month}-${_timestamp(row)?.day}'
    }.where((v) => v != 'null-null-null').length;
    final ins = _attendance.where(_isIn).length;
    final outs = _attendance.where((row) => !_isIn(row)).length;
    final completed = _attendance.where(_isIn).where((row) {
      final t = _timestamp(row);
      if (t == null) return false;
      return _forDay(t).any(
          (other) => !_isIn(other) && (_timestamp(other)?.isAfter(t) ?? false));
    }).length;
    final completion =
        ins == 0 ? 0 : ((completed / ins) * 100).round().clamp(0, 100);
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Row(children: [
        Expanded(child: _metric(context, 'أيام النشاط', '$days')),
        const SizedBox(width: 8),
        Expanded(child: _metric(context, 'الحضور', '$ins')),
        const SizedBox(width: 8),
        Expanded(child: _metric(context, 'الانصراف', '$outs'))
      ]),
      const SizedBox(height: 10),
      _card(context,
          child:
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            _title(context, 'ملخص الالتزام', Icons.trending_up_rounded),
            const SizedBox(height: 12),
            Text('$completion%',
                style:
                    const TextStyle(fontSize: 32, fontWeight: FontWeight.w900)),
            const SizedBox(height: 4),
            Text('نسبة أيام الحضور التي اكتملت بانصراف مسجل.',
                style: TextStyle(
                    color: Theme.of(context).colorScheme.onSurfaceVariant,
                    fontSize: 10)),
            const SizedBox(height: 14),
            const Text('آخر النشاطات',
                style: TextStyle(fontWeight: FontWeight.w900, fontSize: 13)),
            const SizedBox(height: 8),
            ..._attendance.reversed
                .take(6)
                .map((row) => _activityTile(context, row)),
            if (_attendance.isEmpty)
              const _EmptyCard(message: 'لا توجد سجلات حضور حتى الآن.'),
          ])),
    ]);
  }
                              ...
                              children: [
                                ...
                                if (hasOut)
                                  Icon(Icons.logout_rounded,
                                      size: 9, color: scheme.secondary)
                              ])
                        ]));
              }),
          const SizedBox(height: 12),
          Text('التقويم مبني على سجلات الحضور الفعلية المرتبطة بحساب الموظف.',
              style: TextStyle(color: scheme.onSurfaceVariant, fontSize: 10)),
        ]));
  }

  Widget _timeline(BuildContext context) {
    final sorted = [..._attendance]..sort((a, b) =>
        (_timestamp(b) ?? DateTime(0)).compareTo(_timestamp(a) ?? DateTime(0)));
    return _card(context,
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (sorted.isEmpty)
            ...
            ...sorted.take(50).map((row) => _activityTile(context, row))
        ]));
  }
        ...
        child: Container(
            ...
            child: Row(children: [
              Icon(isIn ? Icons.login_rounded : Icons.logout_rounded,
                  color: isIn ? scheme.primary : scheme.secondary),
              const SizedBox(width: 10),
              Expanded(
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                    Text(isIn ? 'تسجيل حضور' : 'تسجيل انصراف',
                        style: const TextStyle(
                            fontWeight: FontWeight.w900, fontSize: 11)),
                    const SizedBox(height: 3),
                    Text(
                        '${time == null ? '—' : _date(time)} · ${_clock(time)}',
                        style: TextStyle(
              ...
              if ('${row['status'] ?? ''}'.isNotEmpty)
                Text('${row['status']}',
                    style:
                        TextStyle(color: scheme.onSurfaceVariant, fontSize: 9))
            ])));
  }

  Widget _schedule(BuildContext context) {
    final rotation =
        '${_employee['scheduleType'] ?? ''}'.toUpperCase() == 'ROTATION';
    return _card(context,
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            Expanded(
                child: _metric(
                    context, 'نوع الجدول', rotation ? 'تناوبي' : 'اعتيادي')),
            const SizedBox(width: 8),
            Expanded(
                child: _metric(context, 'أيام العمل',
                    '${_employee['rotationDaysOn'] ?? '—'}')),
            const SizedBox(width: 8),
            Expanded(
                child: _metric(context, 'أيام الراحة',
                    '${_employee['rotationDaysOff'] ?? '—'}'))
          ]),
          const SizedBox(height: 10),
          _infoRow(context, 'بداية الدوام',
              '${_employee['workStartTime'] ?? _employee['rotationStartTime'] ?? '—'}'),
          _infoRow(context, 'نهاية الدوام',
              '${_employee['workEndTime'] ?? _employee['rotationEndTime'] ?? '—'}'),
          _infoRow(context, 'أيام العمل الأسبوعية',
              '${_employee['workDays'] ?? '—'}')
        ]));
  }
  ...
  Widget _requestsView(BuildContext context) {
    return _card(context,
        child:
            Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Row(children: [
            Expanded(child: _title(context, 'طلباتي', Icons.list_alt_outlined)),
            OutlinedButton.icon(
          ...
          if (_requests.isEmpty)
            const _EmptyCard(message: 'لا توجد طلبات مرتبطة بحسابك.')
          else
            ..._requests.take(30).map((row) {
              final type = '${row['type'] ?? row['requestType'] ?? 'طلب'}';
              final status = '${row['status'] ?? 'قيد المراجعة'}';
              return Padding(
                  padding: const EdgeInsets.only(bottom: 8),
                  child: ListTile(
  ...
  Widget _security(BuildContext context) => _card(context,
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _infoRow(context, 'الجهاز', 'الجهاز الموثق في منظومة Hadir'),
        const SizedBox(height: 8),
        OutlinedButton.icon(
            onPressed: () => context.push('/employee/profile'),
            icon: const Icon(Icons.manage_accounts_outlined),
            label: const Text('إدارة الملف والحساب'))
      ]));
...
class _ErrorCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
      ...
      decoration: BoxDecoration(
          ...
          border: Border.all(
              color:
                  Theme.of(context).colorScheme.error.withValues(alpha: .35))),
      child: Column(children: [
        Text(message,
