...
///
/// The rules intentionally mirror the Web schedule contract: ADMIN schedules
/// use configured work days/times, while ROTATION schedules use the configured
/// start date and on/off cycle. This class is presentation-safe and does not
/// create or mutate attendance records.
...
class EmployeeScheduleService {
  ...
  EmployeeScheduleState _rotation(
    ...
    final startDay = '${employee['rotationStartDate'] ?? ''}'.substring(
      0,
      '${employee['rotationStartDate'] ?? ''}'.length.clamp(0, 10),
    );
    ...
    final firstStart = _localDateTime(
      _dateOnly(startDay),
    ...
    if (target.isBefore(firstStart)) {
      return EmployeeScheduleState(
        ...
        detail: 'تبدأ أول مناوبة في $startDay الساعة ${_formatTime(firstStart)}',
        start: firstStart,
        cycleDay: 0,
    ...
    if (cycleDay < daysOn) {
      return EmployeeScheduleState(
        ...
        detail: 'اليوم ${cycleDay + 1} من $daysOn في المناوبة',
        start: periodStart,
        end: periodStart.add(Duration(days: daysOn)),
        cycleDay: cycleDay + 1,
        cycleTotal: cycleTotal,
      );
    }
