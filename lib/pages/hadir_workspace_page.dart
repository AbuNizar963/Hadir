...
const _green = HadirBrand.darkPrimary;
const _greenDark = HadirBrand.darkPrimaryForeground;
const _soft = HadirBrand.darkPanel;
const _bg = HadirBrand.darkBackground;
const _ink = HadirBrand.darkText;
const _muted = HadirBrand.darkMuted;
const _line = HadirBrand.darkBorder;
const _accent = HadirBrand.darkAccent;
const _warning = HadirBrand.darkWarning;
const _danger = HadirBrand.darkDanger;
...
class _HadirWorkspacePageState extends State<HadirWorkspacePage> {
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Row(children: [
            Expanded(child: _dashboardAction(title: 'تسجيل حضور', subtitle: hasLeave || hasPermission ? status : checkedIn ? 'الدوام جارٍ' : 'مسح رمز QR', enabled: !checkedIn && !checkedOut && !hasLeave && !hasPermission, accent: false, onTap: () => context.push('/employee/scan/check-in'))),
            const SizedBox(width: 12),
            Expanded(child: _dashboardAction(title: 'تسجيل انصراف', subtitle: active ? 'إنهاء الدوام الآن' : checkOut != null ? 'تم تسجيل الانصراف' : 'بعد تسجيل الحضور', enabled: active, accent: true, onTap: () => context.push('/employee/scan/check-out'))),
          ]),
          const SizedBox(height: 16),
          _sectionCard(
            title: 'ملخص اليوم',
          ...
          const SizedBox(height: 16),
          _requestBanner(),
          if (_loading) ...[const SizedBox(height: 16), const _LoadingCard()] else if (_error != null) ...[const SizedBox(height: 16), _messageCard(_error!, Icons.cloud_off_rounded)],
        ],
      ),
    );
  }
  ...
  Widget _employeeHero(DateTime now) {
    return Container(
      ...
      decoration: BoxDecoration(color: HadirBrand.darkCard.withValues(alpha: .80), borderRadius: BorderRadius.circular(16), border: Border.all(color: _line.withValues(alpha: .70)), boxShadow: const [BoxShadow(color: Color(0x33000000), blurRadius: 18, offset: Offset(0, 6))]),
      child: Row(children: [
        _avatar(),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('مرحبًا بك', style: TextStyle(color: _muted, fontSize: 12)),
          Text(_name, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _ink, fontSize: 18, fontWeight: FontWeight.w900)),
          const Text('الموظف', style: TextStyle(color: _muted, fontSize: 12)),
        ])),
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Text(intl.DateFormat('HH:mm').format(now), style: const TextStyle(color: _ink, fontSize: 20, fontWeight: FontWeight.w900)),
          const SizedBox(height: 4),
          Text(intl.DateFormat('EEE، dd MMM', 'ar').format(now), style: const TextStyle(color: _muted, fontSize: 10)),
        ]),
      ]),
    );
  }
  ...
  Widget _statusCard({required DateTime now, required String status, required String detail, required bool active, required bool checkedOut}) {
    final danger = status == 'غائب' || status == 'هارب';
    final warning = status == 'متأخر';
    final primary = status == 'حاضر';
    final tone = danger ? _danger : warning ? _warning : primary ? _green : _accent;
    final toneBg = tone.withValues(alpha: .10);
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: HadirBrand.darkCard.withValues(alpha: .80), borderRadius: BorderRadius.circular(16), border: Border.all(color: tone.withValues(alpha: .40))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('حالة اليوم', style: TextStyle(color: _muted, fontSize: 12)),
            const SizedBox(height: 2),
            Text(status, style: const TextStyle(color: _ink, fontSize: 18, fontWeight: FontWeight.w900)),
          ])),
          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: toneBg, borderRadius: BorderRadius.circular(20)), child: Text(status, style: TextStyle(color: tone, fontSize: 11, fontWeight: FontWeight.w800))),
        ]),
        const SizedBox(height: 5),
        Text(detail, style: const TextStyle(color: _muted, fontSize: 12)),
        const SizedBox(height: 4),
        Text('حسب الجدول الإداري', style: const TextStyle(color: _muted, fontSize: 11)),
        if (active || checkedOut) ...[
          const SizedBox(height: 12),
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: _bg.withValues(alpha: .55), borderRadius: BorderRadius.circular(12), border: Border.all(color: _line.withValues(alpha: .40))), child: Row(children: [Icon(checkedOut ? Icons.task_alt_rounded : Icons.work_history_rounded, color: tone, size: 18), const SizedBox(width: 8), Expanded(child: Text(checkedOut ? 'تم تسجيل الانصراف لهذا اليوم.' : 'أنت مسجل حضور الآن.', style: const TextStyle(color: _muted, fontSize: 11)))])),
        ] else ...[
          const SizedBox(height: 12),
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: _bg.withValues(alpha: .55), borderRadius: BorderRadius.circular(12), border: Border.all(color: _line.withValues(alpha: .40))), child: Row(children: [const Icon(Icons.access_time_rounded, color: _muted, size: 18), const SizedBox(width: 8), const Expanded(child: Text('سيتم تحديث الحالة عند تسجيل الحضور أو تغير الجدول.', style: TextStyle(color: _muted, fontSize: 11))), Text(intl.DateFormat('EEE، dd MMM', 'ar').format(now), style: const TextStyle(color: _muted, fontSize: 10))])),
        ],
      ]),
    );
  }

  Widget _dashboardAction({required String title, required String subtitle, required bool enabled, required bool accent, required VoidCallback onTap}) {
    final tone = accent ? _accent : _green;
    return Material(color: HadirBrand.darkCard.withValues(alpha: .80), borderRadius: BorderRadius.circular(16), child: InkWell(onTap: enabled ? onTap : null, borderRadius: BorderRadius.circular(16), child: Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), border: Border.all(color: enabled ? tone.withValues(alpha: .35) : _line.withValues(alpha: .70))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(width: 40, height: 40, decoration: BoxDecoration(color: enabled ? tone.withValues(alpha: .12) : _soft.withValues(alpha: .55), borderRadius: BorderRadius.circular(12)), child: Center(child: Text(accent ? '↑' : '↓', style: TextStyle(color: enabled ? tone : _muted, fontSize: 21, fontWeight: FontWeight.w900))),
        const SizedBox(height: 12),
        Text(title, style: TextStyle(color: enabled ? _ink : _muted, fontWeight: FontWeight.w900, fontSize: 14)),
        const SizedBox(height: 4),
  ...
  Widget _sectionCard({required String title, required String subtitle, required Widget child}) {
    return Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: HadirBrand.darkCard.withValues(alpha: .80), borderRadius: BorderRadius.circular(16), border: Border.all(color: _line.withValues(alpha: .70))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(subtitle, style: const TextStyle(color: _muted, fontSize: 12)), const SizedBox(height: 2), Text(title, style: const TextStyle(color: _ink, fontSize: 16, fontWeight: FontWeight.w900))])), Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5), decoration: BoxDecoration(color: _soft.withValues(alpha: .70), borderRadius: BorderRadius.circular(20)), child: const Text('اليوم', style: TextStyle(color: _muted, fontSize: 10))) ]), const SizedBox(height: 12), child]));
  }

  Widget _summaryItem(String title, String value, IconData icon) {
    return Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: _soft.withValues(alpha: .40), borderRadius: BorderRadius.circular(16), border: Border.all(color: _line.withValues(alpha: .40))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon, color: _green, size: 17), const SizedBox(height: 7), Text(title, style: const TextStyle(color: _muted, fontSize: 10)), const SizedBox(height: 2), Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _ink, fontWeight: FontWeight.w900, fontSize: 11))]));
  }

  Widget _infoRow(IconData icon, String title, String value) {
    return Padding(padding: const EdgeInsets.symmetric(vertical: 5), child: Row(children: [Container(width: 34, height: 34, decoration: BoxDecoration(color: _soft.withValues(alpha: .60), borderRadius: BorderRadius.circular(10)), child: Icon(icon, color: _green, size: 16)), const SizedBox(width: 9), Expanded(child: Text(title, style: const TextStyle(color: _muted, fontSize: 11))), Flexible(child: Text(value, textAlign: TextAlign.end, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _ink, fontSize: 11, fontWeight: FontWeight.w700)))]));
  }

  Widget _requestBanner() {
    return Material(color: Colors.transparent, borderRadius: BorderRadius.circular(16), child: InkWell(onTap: () => context.push('/requests'), borderRadius: BorderRadius.circular(16), child: Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: HadirBrand.darkCard.withValues(alpha: .80), borderRadius: BorderRadius.circular(16), border: Border.all(color: _accent.withValues(alpha: .35), style: BorderStyle.solid)),
      child: Row(children: [Container(width: 40, height: 40, decoration: BoxDecoration(color: _accent.withValues(alpha: .12), borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.event_note_outlined, color: _accent)), const SizedBox(width: 12), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('طلب استئذان أو إجازة', style: TextStyle(color: _accent, fontWeight: FontWeight.w900, fontSize: 13)), SizedBox(height: 3), Text('إرسال طلب للإدارة', style: TextStyle(color: _muted, fontSize: 10))])), const Icon(Icons.chevron_left_rounded, color: _accent)]),
    )));
  }
  ...
  Widget _bottomNav() {
    ...
    return NavigationBar(selectedIndex: _tab, onDestinationSelected: (index) => setState(() => _tab = index), backgroundColor: HadirBrand.darkCard, indicatorColor: _soft, height: 70, destinations: [for (final item in items) NavigationDestination(icon: Icon(item.$1), selectedIcon: Icon(item.$1, color: _green), label: item.$2)]);
  }

  Widget _summaryCard() => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: HadirBrand.darkCard, borderRadius: BorderRadius.circular(16), border: Border.all(color: _line)), child: Row(children: [const Icon(Icons.calendar_month_rounded, color: _green), const SizedBox(width: 10), const Expanded(child: Text('هذا الشهر', style: TextStyle(color: _ink, fontWeight: FontWeight.w900))), Text('${_attendance.length} حركة', style: const TextStyle(color: _muted, fontSize: 11))]));
  ...
  Widget _requestTile(dynamic item) {
    ...
    return Container(margin: const EdgeInsets.only(bottom: 9), padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: HadirBrand.darkCard, borderRadius: BorderRadius.circular(16), border: Border.all(color: _line)), child: Row(children: [Container(width: 42, height: 42, decoration: BoxDecoration(color: _soft, borderRadius: BorderRadius.circular(13)), child: const Icon(Icons.event_note_outlined, color: _green)), const SizedBox(width: 11), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(type, style: const TextStyle(color: _ink, fontWeight: FontWeight.w900, fontSize: 12)), if (reason.isNotEmpty) ...[const SizedBox(height: 3), Text(reason, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _muted, fontSize: 10))]])), _statusChip(label, status)]));
  }

  // ignore: unused_element
  Widget _metric(IconData icon, String title, String value) => Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: HadirBrand.darkCard, borderRadius: BorderRadius.circular(16), border: Border.all(color: _line)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(width: 34, height: 34, decoration: BoxDecoration(color: _soft, borderRadius: BorderRadius.circular(10)), child: Icon(icon, color: _green, size: 18)), const SizedBox(height: 8), Text(title, style: const TextStyle(color: _muted, fontSize: 9)), const SizedBox(height: 2), Text(value, style: const TextStyle(color: _ink, fontWeight: FontWeight.w900, fontSize: 12))]));

  // ignore: unused_element
  Widget _quick(IconData icon, String title, String sub, VoidCallback onTap) {
    return Material(color: HadirBrand.darkCard, borderRadius: BorderRadius.circular(16), child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(16), child: Container(padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 6), decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), border: Border.all(color: _line)), child: Column(children: [Container(width: 39, height: 39, decoration: BoxDecoration(color: _soft, borderRadius: BorderRadius.circular(12)), child: Icon(icon, color: _green, size: 19)), const SizedBox(height: 7), Text(title, style: const TextStyle(color: _ink, fontWeight: FontWeight.w900, fontSize: 10.5)), const SizedBox(height: 2), Text(sub, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _muted, fontSize: 7.5))]))));
  }

  Widget _actionCard(IconData icon, String title, String sub, VoidCallback onTap) {
    return Material(color: HadirBrand.darkCard, borderRadius: BorderRadius.circular(16), child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(16), child: Padding(padding: const EdgeInsets.all(15), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(width: 40, height: 40, decoration: BoxDecoration(color: _soft, borderRadius: BorderRadius.circular(12)), child: Icon(icon, color: _green)), const SizedBox(height: 10), Text(title, style: const TextStyle(color: _ink, fontWeight: FontWeight.w900)), const SizedBox(height: 3), Text(sub, style: const TextStyle(color: _muted, fontSize: 10))]))));
  }

  Widget _menuTile(IconData icon, String title, String subtitle, VoidCallback onTap) => Card(margin: const EdgeInsets.only(bottom: 8), elevation: 0, color: HadirBrand.darkCard, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16), side: BorderSide(color: _line)), child: ListTile(onTap: onTap, leading: Container(width: 40, height: 40, decoration: BoxDecoration(color: _soft, borderRadius: BorderRadius.circular(12)), child: Icon(icon, color: _green)), title: Text(title, style: const TextStyle(color: _ink, fontWeight: FontWeight.w900, fontSize: 13)), subtitle: Text(subtitle, style: const TextStyle(color: _muted, fontSize: 10)), trailing: const Icon(Icons.chevron_left_rounded, color: _muted)));
  ...
  // ignore: unused_element
  Widget _iconButton(IconData icon, VoidCallback onTap) => Material(color: HadirBrand.darkCard, borderRadius: BorderRadius.circular(14), child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(14), child: Container(width: 43, height: 43, decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), border: Border.all(color: _line)), child: Icon(icon, color: _ink, size: 21))));
  Widget _avatar({bool light = false}) => Container(width: 56, height: 56, decoration: BoxDecoration(color: light ? Colors.white24 : _green.withValues(alpha: .12), borderRadius: BorderRadius.circular(16), border: Border.all(color: _green.withValues(alpha: .25))), child: Icon(Icons.person_rounded, color: light ? Colors.white : _green, size: 28));
  Widget _chip(String text, bool positive) => Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5), decoration: BoxDecoration(color: positive ? _green.withValues(alpha: .15) : _soft, borderRadius: BorderRadius.circular(20)), child: Text(text, style: TextStyle(color: positive ? _green : _muted, fontWeight: FontWeight.w800, fontSize: 9)));
  Widget _statusChip(String text, String status) { final good = status == 'approved' || status == 'confirmed'; return _chip(text, good); }
  Widget _messageCard(String text, IconData icon) => Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: HadirBrand.darkCard, borderRadius: BorderRadius.circular(16), border: Border.all(color: _line)), child: Row(children: [Icon(icon, color: _muted), const SizedBox(width: 10), Expanded(child: Text(text, style: const TextStyle(color: _muted, fontSize: 11)))]));
  Widget _emptyCard(String title, String subtitle, IconData icon) => Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: HadirBrand.darkCard, borderRadius: BorderRadius.circular(16), border: Border.all(color: _line)), child: Column(children: [Container(width: 48, height: 48, decoration: BoxDecoration(color: _soft, borderRadius: BorderRadius.circular(15)), child: Icon(icon, color: _green)), const SizedBox(height: 10), Text(title, style: const TextStyle(color: _ink, fontWeight: FontWeight.w900)), const SizedBox(height: 4), Text(subtitle, textAlign: TextAlign.center, style: const TextStyle(color: _muted, fontSize: 10))]));
...
class _LoadingCard extends StatelessWidget {
  const _LoadingCard();
  @override
  Widget build(BuildContext context) => Container(height: 86, decoration: BoxDecoration(color: HadirBrand.darkCard, borderRadius: BorderRadius.circular(16), border: Border.all(color: _line)), child: const Center(child: CircularProgressIndicator(strokeWidth: 2, color: _green)));
}