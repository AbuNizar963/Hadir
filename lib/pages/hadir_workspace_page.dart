...
class _HadirWorkspacePageState extends State<HadirWorkspacePage> {
  ...
  String _name = 'الموظف';
  String _jobNumber = '';
  String _locationName = 'الموقع المخصص';
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final profile = results[1] is Map ? Map<String, dynamic>.from(results[1] as Map) : <String, dynamic>{};
      ...
      final resolvedName = readText(employee, ['name', 'employeeName']).isNotEmpty
          ? readText(employee, ['name', 'employeeName'])
          : user is Map ? '${user['name'] ?? 'الموظف'}' : 'الموظف';
      final location = readText(employee, ['locationName', 'location_name', 'location']);
      final schedule = readText(employee, ['scheduleType', 'schedule_type', 'schedule']);
      final start = _normaliseTime(readText(employee, ['workStartTime', 'work_start_time', 'startTime']));
      final end = _normaliseTime(readText(employee, ['workEndTime', 'work_end_time', 'endTime']));
      if (!mounted) return;
      setState(() {
        _name = resolvedName;
        _jobNumber = readText(employee, ['jobNumber', 'job_number']);
        _locationName = location.isEmpty ? 'الموقع المخصص' : location;
        _scheduleType = schedule;
        _workStartTime = start;
        _workEndTime = end;
        _attendance = results[2] is List ? List<dynamic>.from(results[2] as List) : const [];
        _requests = results[3] is List ? List<dynamic>.from(results[3] as List) : const [];
        _notifications = results[4] is List ? List<dynamic>.from(results[4] as List) : const [];
        _loading = false;
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Widget _dashboard() {
    ...
    final restDay = _isRestDay(now);
    final late = !checkedIn && !checkedOut && !hasLeave && !hasPermission && !restDay && _isLate(now);
    final status = hasLeave ? 'إجازة' : hasPermission ? 'إذن' : checkedOut ? 'انتهى الدوام' : active ? 'حاضر' : late ? 'متأخر' : restDay ? 'مستريح' : 'جاهز لتسجيل الحضور';
    final statusDetail = hasLeave
        ? 'لديك إجازة معتمدة لهذا اليوم.'
        : hasPermission
            ? 'لديك إذن معتمد لهذا اليوم.'
            : checkedOut
                ? 'تم تسجيل الانصراف لهذا اليوم.'
                : active
                    ? 'أنت مسجل حضور الآن ويمكنك تسجيل الانصراف.'
                    : late
                        ? 'بدأ وقت الدوام ولم يتم تسجيل الحضور بعد.'
                        : restDay
                            ? 'اليوم خارج أيام العمل حسب جدولك.'
                            : _workStartTime.isEmpty ? 'لم يتم تسجيل حضورك بعد.' : 'يبدأ دوامك عند $_workStartTime.';
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: _dashboardAction(icon: Icons.login_rounded, title: 'تسجيل حضور', subtitle: hasLeave || hasPermission ? status : checkedIn ? 'تم تسجيل الحضور' : restDay ? 'اليوم يوم راحة' : 'التحقق من الموقع والجهاز', enabled: !checkedIn && !checkedOut && !hasLeave && !hasPermission && !restDay, onTap: () => context.push('/attendance?type=check-in'))),
          ...
          const SizedBox(height: 12),
          _sectionCard(title: 'معلومات الدوام', subtitle: 'الجدول والموقع والحالة', child: Column(children: [
            _infoRow(Icons.calendar_today_outlined, 'نوع الجدول', _scheduleType.isEmpty ? 'غير محدد' : _scheduleType),
            _infoRow(Icons.access_time_rounded, 'الفترة', _workStartTime.isEmpty && _workEndTime.isEmpty ? 'غير محددة' : '${_workStartTime.isEmpty ? '—' : _workStartTime} - ${_workEndTime.isEmpty ? '—' : _workEndTime}'),
            _infoRow(Icons.verified_outlined, 'الحالة', status),
            _infoRow(Icons.location_on_outlined, 'الموقع', _locationName),
            _infoRow(Icons.badge_outlined, 'الرقم الوظيفي', _jobNumber.isEmpty ? 'غير محدد' : _jobNumber),
          ])),
          const SizedBox(height: 12),
          _requestBanner(),
  ...
  Widget _employeeHero(DateTime now) {
    final identity = [_jobNumber, if (_locationName != 'الموقع المخصص') _locationName].where((x) => x.isNotEmpty).join(' · ');
    return Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(22), border: Border.all(color: _line), boxShadow: const [BoxShadow(color: Color(0x0A142D27), blurRadius: 16, offset: Offset(0, 6))]), child: Row(children: [
      _avatar(), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(identity.isEmpty ? 'HADIR · EMPLOYEE' : identity, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _green, fontSize: 9.5, fontWeight: FontWeight.w800)),
      ])), Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text(intl.DateFormat('HH:mm').format(now), style: const TextStyle(color: _ink, fontSize: 17, fontWeight: FontWeight.w900)), const SizedBox(height: 2), Text(intl.DateFormat('EEEE، d MMMM', 'ar').format(now), style: const TextStyle(color: _muted, fontSize: 9.5))]),
    ]));
  }
  ...
  Widget _statusCard({required DateTime now, required String status, required String detail, required bool active, required bool checkedOut}) {
    ...
    final stateIcon = checkedOut ? Icons.task_alt_rounded : active ? Icons.work_history_rounded : status == 'إجازة' ? Icons.beach_access_rounded : status == 'إذن' ? Icons.event_available_rounded : status == 'متأخر' ? Icons.schedule_rounded : status == 'مستريح' ? Icons.weekend_rounded : status == 'غائب' ? Icons.person_off_rounded : Icons.access_time_rounded;
    return Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(22), border: Border.all(color: _line), boxShadow: const [BoxShadow(color: Color(0x0D142D27), blurRadius: 18, offset: Offset(0, 7))]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('حالة اليوم', style: TextStyle(color: _muted, fontSize: 10, fontWeight: FontWeight.w700)), const SizedBox(height: 3), Text(status, style: TextStyle(color: stateColor, fontSize: 22, fontWeight: FontWeight.w900)), const SizedBox(height: 3), Text(detail, style: const TextStyle(color: _muted, fontSize: 10.5))])), Container(width: 48, height: 48, decoration: BoxDecoration(color: stateSoft, borderRadius: BorderRadius.circular(15), border: Border.all(color: stateColor.withValues(alpha: .22))), child: Icon(stateIcon, color: stateColor))]),
      const SizedBox(height: 14), Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10), decoration: BoxDecoration(color: _bg, borderRadius: BorderRadius.circular(14)), child: Row(children: [Icon(Icons.location_on_outlined, size: 16, color: stateColor), const SizedBox(width: 6), Expanded(child: Text(_locationName, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _ink, fontSize: 10.5, fontWeight: FontWeight.w800))), Text(intl.DateFormat('EEEE، d MMMM', 'ar').format(now), style: const TextStyle(color: _muted, fontSize: 9.5))])),
    ]));
  }
  ...
  Widget _infoRow(IconData icon, String title, String value) { final color = title == 'الحالة' ? _statusColor(value) : _green; return Padding(padding: const EdgeInsets.symmetric(vertical: 6), child: Row(children: [Container(width: 31, height: 31, decoration: BoxDecoration(color: color.withValues(alpha: .13), borderRadius: BorderRadius.circular(10)), child: Icon(icon, color: color, size: 16)), const SizedBox(width: 9), Expanded(child: Text(title, style: const TextStyle(color: _muted, fontSize: 10))), Flexible(child: Text(value, textAlign: TextAlign.end, overflow: TextOverflow.ellipsis, style: TextStyle(color: title == 'الحالة' ? color : _ink, fontSize: 10.5, fontWeight: FontWeight.w800)))])); }
  ...
  List<Map<dynamic, dynamic>> _todayApprovedRequests(DateTime now) { final day = intl.DateFormat('yyyy-MM-dd').format(now); return _requests.whereType<Map>().where((item) { final status = '${item['status'] ?? ''}'.toLowerCase(); if (status != 'approved' && status != 'confirmed') return false; final start = '${item['startDate'] ?? item['createdAt'] ?? ''}'.split('T').first; final end = '${item['endDate'] ?? item['startDate'] ?? item['createdAt'] ?? ''}'.split('T').first; final startDate = DateTime.tryParse(start); final endDate = DateTime.tryParse(end); final currentDate = DateTime.tryParse(day); return startDate != null && endDate != null && currentDate != null && !currentDate.isBefore(startDate) && !currentDate.isAfter(endDate); }).toList(); }
  String _eventTime(DateTime? value) => value == null ? '—' : intl.DateFormat('HH:mm').format(value);

  bool _isLate(DateTime now) { final start = _scheduledStart(now); return start != null && now.isAfter(start); }
  DateTime? _scheduledStart(DateTime now) { if (_workStartTime.isEmpty) return null; final parts = _workStartTime.split(':'); if (parts.length < 2) return null; final hour = int.tryParse(parts[0]); final minute = int.tryParse(parts[1]); if (hour == null || minute == null) return null; return DateTime(now.year, now.month, now.day, hour, minute); }
  bool _isRestDay(DateTime now) { final schedule = _scheduleType.toLowerCase(); if (schedule.contains('rotation') || schedule.contains('دوار') || schedule.contains('تناوب')) return false; final day = now.weekday; return day == DateTime.friday || day == DateTime.saturday; }
  String _normaliseTime(String value) { if (value.isEmpty) return ''; final match = RegExp(r'^(\d{1,2}):(\d{2})').firstMatch(value); return match == null ? value : '${match.group(1)!.padLeft(2, '0')}:${match.group(2)}'; }

  Widget _clockCard(DateTime now) { final today = _todayAttendance(now); final checkedIn = today.any((x) => x is Map && x['type'] == 'check-in'); final checkedOut = today.any((x) => x is Map && x['type'] == 'check-out'); final active = checkedIn && !checkedOut; final status = checkedOut ? 'تم إنهاء الدوام' : active ? 'أنت على رأس العمل' : 'جاهز لتسجيل الحضور'; final type = active ? 'check-out' : 'check-in'; return Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(gradient: const LinearGradient(begin: Alignment.topRight, end: Alignment.bottomLeft, colors: [_green, _greenDark]), borderRadius: BorderRadius.circular(26), boxShadow: const [BoxShadow(color: Color(0x240B6B5A), blurRadius: 26, offset: Offset(0, 12))]), child: Column(children: [Row(children: [Container(width: 8, height: 8, decoration: const BoxDecoration(color: Color(0xFF9AE3C8), shape: BoxShape.circle)), const SizedBox(width: 7), Expanded(child: Text(status, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 12))), Text(intl.DateFormat('d MMMM', 'ar').format(now), style: const TextStyle(color: Colors.white70, fontSize: 11))]), const SizedBox(height: 17), Text(intl.DateFormat('HH:mm').format(now), style: const TextStyle(color: Colors.white, fontSize: 44, height: 1, fontWeight: FontWeight.w900)), const SizedBox(height: 7), Text(intl.DateFormat('EEEE، d MMMM yyyy', 'ar').format(now), style: const TextStyle(color: Colors.white70, fontSize: 11)), const SizedBox(height: 17), SizedBox(width: double.infinity, height: 52, child: FilledButton.icon(onPressed: checkedOut ? null : () => context.push('/attendance?type=$type'), style: FilledButton.styleFrom(backgroundColor: Colors.white, foregroundColor: _green, disabledBackgroundColor: Colors.white24, disabledForegroundColor: Colors.white70, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))), icon: Icon(active ? Icons.logout_rounded : Icons.login_rounded, size: 20), label: Text(active ? 'تسجيل الانصراف' : 'تسجيل الحضور', style: const TextStyle(fontWeight: FontWeight.w900)))), const SizedBox(height: 9), const Text('سيتم التحقق من الموقع والجهاز وQR عند التسجيل', style: TextStyle(color: Colors.white70, fontSize: 9.5))])); }
  ...
  Widget _bottomNav() { const items = [(Icons.home_rounded, 'الرئيسية'), (Icons.calendar_month_rounded, 'الدوام'), (Icons.fingerprint_rounded, 'الحضور'), (Icons.event_note_outlined, 'الطلبات'), (Icons.more_horiz_rounded, 'المزيد')]; return NavigationBar(selectedIndex: _tab, onDestinationSelected: (index) => setState(() => _tab = index), backgroundColor: Colors.white, indicatorColor: _soft, height: 70, destinations: [for (final item in items) NavigationDestination(icon: Icon(item.$1), selectedIcon: Icon(item.$1, color: _green), label: item.$2)]); }
  ...
  Widget _attendanceList({int? limit}) { if (_loading) return const _LoadingCard(); if (_error != null) return _messageCard(_error!, Icons.cloud_off_rounded); if (_attendance.isEmpty) return _emptyCard('لا توجد حركات حضور', 'ابدأ أول تسجيل حضور من زر الحضور.', Icons.event_available_rounded); final data = limit == null ? _attendance : _attendance.take(limit); return Column(children: data.map(_attendanceTile).toList()); }
  Widget _attendanceTile(dynamic item) { final map = item is Map ? item : const <dynamic, dynamic>{}; final type = '${map['type'] ?? ''}'; final stamp = DateTime.tryParse('${map['timestamp'] ?? ''}'); final isIn = type == 'check-in'; final eventColor = isIn ? _statusColor('حاضر') : _statusColor('إذن'); return Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(13), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: _line)), child: Row(children: [Container(width: 40, height: 40, decoration: BoxDecoration(color: eventColor.withValues(alpha: .13), borderRadius: BorderRadius.circular(12)), child: Icon(isIn ? Icons.login_rounded : Icons.logout_rounded, color: eventColor, size: 19)), const SizedBox(width: 11), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(isIn ? 'تسجيل حضور' : 'تسجيل انصراف', style: const TextStyle(color: _ink, fontWeight: FontWeight.w900, fontSize: 12)), const SizedBox(height: 3), Text(stamp == null ? 'وقت غير متاح' : intl.DateFormat('EEEE، d MMMM • HH:mm', 'ar').format(stamp), style: const TextStyle(color: _muted, fontSize: 10))])), _chip(isIn ? 'حضور' : 'انصراف', isIn)])); }
  Widget _requestTile(dynamic item) { final map = item is Map ? item : const <dynamic, dynamic>{}; final status = '${map['status'] ?? 'pending'}'; final type = '${map['type'] ?? 'طلب'}'; final reason = '${map['reason'] ?? ''}'; final label = status == 'approved' ? 'مقبول' : status == 'rejected' ? 'مرفوض' : status == 'confirmed' ? 'مؤكد' : 'قيد المراجعة'; final statusColor = _requestStatusColor(status); return Container(margin: const EdgeInsets.only(bottom: 9), padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(19), border: Border.all(color: _line)), child: Row(children: [Container(width: 42, height: 42, decoration: BoxDecoration(color: statusColor.withValues(alpha: .13), borderRadius: BorderRadius.circular(13)), child: Icon(Icons.event_note_outlined, color: statusColor)), const SizedBox(width: 11), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(type, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 12)), if (reason.isNotEmpty) ...[const SizedBox(height: 3), Text(reason, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _muted, fontSize: 10))]])), _statusChip(label, status)])); }
  Color _requestStatusColor(String status) { switch (status) { case 'approved': case 'confirmed': return HadirBrand.lightPrimary; case 'rejected': return HadirBrand.lightDanger; case 'pending': return HadirBrand.lightWarning; default: return HadirBrand.lightMuted; } }
