...
class _ManagerEmployeesDirectoryPageState extends State<ManagerEmployeesDirectoryPage> {
  ...
  Widget _metric(String label, int value, Color color, IconData icon) {
    return Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: color.withValues(alpha: .07), borderRadius: BorderRadius.circular(14), border: Border.all(color: color.withValues(alpha: .22))), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(label, style: TextStyle(fontSize: 9.5, fontWeight: FontWeight.w800, color: color)), Icon(icon, size: 15, color: color)]), const SizedBox(height: 5), Text('$value', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900))]));
  }
  ...
  Widget _action(String label, IconData icon, VoidCallback onTap, Color color) {
    return OutlinedButton(onPressed: onTap, style: OutlinedButton.styleFrom(minimumSize: const Size(0, 48), padding: const EdgeInsets.all(2), foregroundColor: color, side: BorderSide(color: color.withValues(alpha: .25)), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, size: 16), const SizedBox(height: 2), Text(label, style: const TextStyle(fontSize: 8, fontWeight: FontWeight.w800))]));
  }
  ...
  Widget _employeeCard(Map<String, dynamic> e) {
    ...
    return Card(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16), side: BorderSide(color: Theme.of(context).dividerColor.withValues(alpha: .65))), child: Padding(padding: const EdgeInsets.all(10), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
    ...
    return Scaffold(
      body: RefreshIndicator(
        ...
        child: ListView(padding: const EdgeInsets.fromLTRB(14, 12, 14, 30), children: [
          Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), border: Border.all(color: Theme.of(context).colorScheme.primary.withValues(alpha: .22)), color: Theme.of(context).colorScheme.primary.withValues(alpha: .025)), child: LayoutBuilder(builder: (context, c) {
            final compact = c.maxWidth < 520;
            return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('EMPLOYEE DIRECTORY', style: TextStyle(fontSize: 9.5, fontWeight: FontWeight.w900, color: Theme.of(context).colorScheme.primary)), const SizedBox(height: 3), const Text('إدارة الموظفين', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)), const SizedBox(height: 1), Text('دليل الموظفين الموحد — البيانات من النظام مباشرة.', style: TextStyle(fontSize: 10, color: Theme.of(context).hintColor))]))]),
              const SizedBox(height: 10),
              if (compact) ...[
                SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: () => context.push('/manager/employees/transfer'), icon: const Icon(Icons.import_export_rounded, size: 18), label: const Text('استيراد / تصدير', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900)))),
                const SizedBox(height: 7),
                SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: _addEmployee, icon: const Icon(Icons.person_add_alt_1, size: 18), label: const Text('إضافة موظف', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900)))),
              ] else Row(children: [Expanded(child: OutlinedButton.icon(onPressed: () => context.push('/manager/employees/transfer'), icon: const Icon(Icons.import_export_rounded, size: 18), label: const Text('استيراد / تصدير', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900)))), const SizedBox(width: 8), Expanded(child: FilledButton.icon(onPressed: _addEmployee, icon: const Icon(Icons.person_add_alt_1, size: 18), label: const Text('إضافة موظف', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900))))]),
            ]);
          })),
          const SizedBox(height: 10),
          LayoutBuilder(builder: (context, c) { final columns = c.maxWidth > 900 ? 6 : c.maxWidth > 600 ? 3 : 2; return GridView.count(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisCount: columns, crossAxisSpacing: 7, mainAxisSpacing: 7, childAspectRatio: 1.7, children: [_metric('الإجمالي', _employees.length, Colors.lightBlue, Icons.groups_rounded), _metric('فعال', active, Colors.green, Icons.check_circle_outline_rounded), _metric('موقوف', suspended, Colors.red, Icons.pause_circle_outline_rounded), _metric('أجهزة موثقة', devices, Colors.deepPurple, Icons.smartphone_rounded), _metric('VIP مفعل', vip, Colors.amber.shade700, Icons.star_rounded), _metric('هارب الآن', escaped, Colors.red.shade700, Icons.directions_walk_rounded)]); }),
          const SizedBox(height: 10),
          Card(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('LIVE DIRECTORY', style: TextStyle(fontSize: 9.5, fontWeight: FontWeight.w900, color: Theme.of(context).colorScheme.primary)), Text('قائمة الموظفين (${list.length})', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900))])), IconButton(onPressed: _load, icon: const Icon(Icons.refresh_rounded, size: 20))]), const SizedBox(height: 7), TextField(onChanged: (v) => setState(() => _query = v), decoration: const InputDecoration(prefixIcon: Icon(Icons.search_rounded, size: 20), hintText: 'بحث بالاسم أو الرقم أو الجهاز', isDense: true)), const SizedBox(height: 7), Row(children: [Expanded(child: DropdownButtonFormField<String>(initialValue: _statusFilter, decoration: const InputDecoration(labelText: 'الحالة', isDense: true), items: const [DropdownMenuItem(value: 'all', child: Text('كل الحالات')), DropdownMenuItem(value: 'active', child: Text('فعال')), DropdownMenuItem(value: 'suspended', child: Text('موقوف'))], onChanged: (v) => setState(() => _statusFilter = v ?? 'all'))), const SizedBox(width: 7), Expanded(child: DropdownButtonFormField<String>(initialValue: _scheduleFilter, decoration: const InputDecoration(labelText: 'نوع الدوام', isDense: true), items: const [DropdownMenuItem(value: 'all', child: Text('كل أنواع الدوام')), DropdownMenuItem(value: 'ADMIN', child: Text('إداري')), DropdownMenuItem(value: 'ROTATION', child: Text('تناوبي'))], onChanged: (v) => setState(() => _scheduleFilter = v ?? 'all')))]),
          ]))),
          if (_loading) const Padding(padding: EdgeInsets.all(45), child: Center(child: CircularProgressIndicator())) else if (_error != null) Padding(padding: const EdgeInsets.all(24), child: Column(children: [const Icon(Icons.cloud_off_rounded, size: 42), const SizedBox(height: 8), Text(_error!, textAlign: TextAlign.center), const SizedBox(height: 8), FilledButton(onPressed: _load, child: const Text('إعادة المحاولة'))])) else if (list.isEmpty) const Padding(padding: EdgeInsets.all(40), child: Column(children: [Icon(Icons.search_off_rounded, size: 42), SizedBox(height: 8), Text('لا توجد نتائج', style: TextStyle(fontWeight: FontWeight.w900)), SizedBox(height: 4), Text('جرّب تغيير البحث أو الفلاتر.')])) else Padding(padding: const EdgeInsets.only(top: 10), child: LayoutBuilder(builder: (context, c) { final columns = c.maxWidth > 1200 ? 4 : c.maxWidth > 760 ? 3 : c.maxWidth > 480 ? 2 : 1; return GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: list.length, gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: columns, crossAxisSpacing: 8, mainAxisSpacing: 8, childAspectRatio: columns == 1 ? .72 : .68), itemBuilder: (_, i) => _employeeCard(list[i])); })),
        ],
      ),
    ));
  }
