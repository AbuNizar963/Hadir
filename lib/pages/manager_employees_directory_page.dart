    ...
    return Scaffold(
      body: RefreshIndicator(
        ...
        child: ListView(padding: const EdgeInsets.fromLTRB(14, 14, 14, 32), children: [
          Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(borderRadius: BorderRadius.circular(20), border: Border.all(color: Theme.of(context).colorScheme.primary.withValues(alpha: .25)), color: Theme.of(context).colorScheme.primary.withValues(alpha: .025)), child: Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('EMPLOYEE DIRECTORY', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, color: Theme.of(context).colorScheme.primary)), const SizedBox(height: 4), const Text('إدارة الموظفين', style: TextStyle(fontSize: 23, fontWeight: FontWeight.w900)), Text('دليل الموظفين الموحد — البيانات من النظام مباشرة.', style: TextStyle(fontSize: 11, color: Theme.of(context).hintColor))])), const SizedBox(width: 8), OutlinedButton.icon(onPressed: () => context.push('/manager/employees/transfer'), icon: const Icon(Icons.import_export_rounded, size: 18), label: const Text('استيراد / تصدير', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900))), const SizedBox(width: 8), FilledButton.icon(onPressed: _addEmployee, icon: const Icon(Icons.person_add_alt_1), label: const Text('إضافة موظف'))])),
