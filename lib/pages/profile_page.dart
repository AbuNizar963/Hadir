...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Widget _sectionHeader(String title, String subtitle) => Column(
        ...
        children: [
          ...
          Text(subtitle, style: const TextStyle(color: HadirBrand.muted, fontSize: 10.5)),
        ],
      );

  Widget _infoGroup(List<Widget> children) => Container(
  ...
  Widget _infoTile(IconData icon, String title, String value) => ListTile(
        ...
        leading: Container(width: 42, height: 42, decoration: BoxDecoration(color: HadirBrand.soft, borderRadius: BorderRadius.circular(13)), child: Icon(icon, color: HadirBrand.primary, size: 21)),
        title: Text(title, style: const TextStyle(color: HadirBrand.muted, fontSize: 10.5, fontWeight: FontWeight.w700)),
        subtitle: Padding(padding: const EdgeInsets.only(top: 2), child: Text(value, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w900))),
      );
  ...
  Widget _securityRow(IconData icon, String title, String subtitle, Widget? trailing) => Row(
        children: [
          Container(width: 40, height: 40, decoration: BoxDecoration(color: HadirBrand.soft, borderRadius: BorderRadius.circular(12)), child: Icon(icon, color: HadirBrand.primary, size: 20)),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w900)), const SizedBox(height: 3), Text(subtitle, style: const TextStyle(color: HadirBrand.muted, fontSize: 10.5))])),
  ...
  Widget _badge(String text, bool good) => Container(
        ...
        decoration: BoxDecoration(color: good ? HadirBrand.soft : const Color(0xFFFFEFED), borderRadius: BorderRadius.circular(20)),
        child: Text(text, style: TextStyle(color: good ? HadirBrand.primary : HadirBrand.danger, fontSize: 9, fontWeight: FontWeight.w900)),
      );

  Widget _logoutCard() => Container(
        padding: const EdgeInsets.all(15),
        decoration: BoxDecoration(color: Theme.of(context).cardColor, borderRadius: BorderRadius.circular(HadirBrand.radiusLg), border: Border.all(color: const Color(0xFFE8D9D6))),
        child: Row(
          children: [
            Container(width: 44, height: 44, decoration: BoxDecoration(color: const Color(0xFFFFEFED), borderRadius: BorderRadius.circular(14)), child: const Icon(Icons.logout_rounded, color: HadirBrand.danger)),
            const SizedBox(width: 12),
            const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('جلسة الموظف', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w900)), SizedBox(height: 3), Text('إنهاء الجلسة على هذا الجهاز بأمان', style: TextStyle(color: HadirBrand.muted, fontSize: 10.5))])),
            FilledButton.tonal(
              onPressed: _loggingOut ? null : _logout,
              child: _loggingOut
                  ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2))
                  : const Text('خروج', style: TextStyle(fontWeight: FontWeight.w900)),
            ),
          ],
        ),
      );
          ...
          child: Column(
            ...
            children: [
              Container(
                width: 72,
                height: 72,
                decoration: BoxDecoration(color: const Color(0xFFFFEFED), borderRadius: BorderRadius.circular(22)),
                child: const Icon(Icons.cloud_off_rounded, color: HadirBrand.danger, size: 35),
              ),
              const SizedBox(height: 14),
              Text(error ?? 'تعذر تحميل الحساب.', textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w800)),
              const SizedBox(height: 14),
              FilledButton(onPressed: _load, child: const Text('إعادة المحاولة', style: TextStyle(fontWeight: FontWeight.w800))),
            ],
          ),
        ),
      );
...
class _ProfileSkeleton extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListView(
        ...
        children: [
          _SkeletonBlock(height: 190, radius: HadirBrand.radiusXl),
          const SizedBox(height: 16),
          _SkeletonBlock(height: 130, radius: HadirBrand.radiusLg),
          const SizedBox(height: 16),
          _SkeletonBlock(height: 190, radius: HadirBrand.radiusLg),
          const SizedBox(height: 16),
          _SkeletonBlock(height: 74, radius: HadirBrand.radiusLg),
        ],
      );
}
