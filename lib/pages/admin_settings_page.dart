...
class _AdminSettingsPageState extends State<AdminSettingsPage> {
  ...
  Future<void> _editValue(String key, dynamic value) async {
    ...
    if (value is bool) {
      next = result.toLowerCase() == 'true' || result == '1' || result == 'نعم';
    }
    if (value is List) {
      next = result
          .split(',')
  ...
  Widget _settingTile(String key, dynamic value) {
    final editable = key != 'adminAccounts' &&
        key != 'locations' &&
        key != 'brandLogo';
    return InkWell(
  ...
  Widget _general() {
    ...
    final rows = known
        ...
        .map((key) => _settingTile(key, _settings[key]))
        .toList();
    return Column(
      ...
      children: [
        _header(
          'الهوية والحسابات',
          'مطابقة خيارات الهوية والحسابات الموجودة في الموقع الرئيسي.',
        ...
        _section(
          ...
          Icons.business_rounded,
          rows.isEmpty
              ? [const Text('لا توجد بيانات هوية إضافية من الخادم.')]
              : rows,
        ),
      ],
    );
  ...
  Widget _locationsView() {
    ...
    final settingRows = known
        .where(_settings.containsKey)
        .map((key) => _settingTile(key, _settings[key]))
        .toList();

    final locationRows = _locations.isEmpty
        ? <Widget>[
            const Text(
              'لا توجد مواقع إضافية مسجلة.',
              style: TextStyle(color: _muted),
            ),
          ]
        : _locations.map<Widget>((raw) {
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 3),
                        Text(
                          '${item['lat'] ?? '—'}, ${item['lng'] ?? '—'} · ${item['radiusMeters'] ?? '—'} م',
    ...
    return Column(
      ...
      children: [
        _header(
          'المواقع و QR',
          'إعداد الموقع الرئيسي ورمز الحضور وإدارة المواقع المسجلة في الخادم.',
        ...
        _section(
          ...
          Icons.location_on_outlined,
          settingRows.isEmpty
              ? [const Text('لا توجد إحداثيات محفوظة حالياً.')]
              : settingRows,
        ),
        const SizedBox(height: 12),
        _section('المواقع المسجلة', Icons.map_outlined, locationRows),
      ],
    );
  }
  ...
  Widget _workView() {
    ...
    return Column(
      ...
      children: [
        _header(
          'ساعات العمل',
          'إعدادات الدوام والمهل المستخدمة في الحضور والانصراف.',
        ...
        _section(
          ...
          Icons.schedule_rounded,
          rows.isEmpty
              ? [const Text('لا توجد إعدادات دوام إضافية من الخادم.')]
              : rows,
        ),
      ],
    );
  ...
  Widget _advancedView() {
    ...
    final extras = _settings.entries
    ...
    return Column(
      ...
      children: [
        _header(
          'الأمان والنظام والمتقدم',
          'كل مفاتيح الإعدادات التي يرسلها الخادم تظهر هنا دون حذف الخيارات غير المعروفة للتطبيق.',
          Icons.security_rounded,
        ),
        const SizedBox(height: 12),
        _section(
          'إعدادات الخادم الإضافية',
          Icons.tune_rounded,
          extras.isEmpty
              ? [const Text('لا توجد مفاتيح إضافية من الخادم.')]
              : extras
                  .map((entry) => _settingTile(entry.key, entry.value))
                  .toList(),
        ),
        const SizedBox(height: 12),
        _section(
          ...
          [
            ListTile(
              ...
              subtitle: const Text(
                'يتحقق من استجابة واجهة الصحة بدون تغيير البيانات.',
              ),
              trailing: IconButton(
            ...
            ListTile(
              contentPadding: EdgeInsets.zero,
              leading: const Icon(Icons.refresh_rounded, color: _brand),
              title: const Text(
                'إعادة تحميل الإعدادات',
                style: TextStyle(fontWeight: FontWeight.w800),
              ),
              subtitle: const Text(
                'يجلب أحدث نسخة من الإعدادات والمواقع.',
              ),
              trailing: IconButton(
                onPressed: _load,
    ...
    try {
      ...
      showDialog<void>(
        ...
        builder: (dialogContext) => AlertDialog(
          ...
          content: Text(
            data.isEmpty ? 'لم تصل بيانات تشخيص.' : _displayValue(data),
          ),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(dialogContext),
  ...
  Widget build(BuildContext context) {
    const tabs = <String>['عام', 'المواقع و QR', 'الدوام', 'متقدم'];
