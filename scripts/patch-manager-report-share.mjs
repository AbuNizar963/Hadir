...
if (source.includes(printAnchor) && !source.includes('const sharePdf = async')) {
  source = source.replace(printAnchor, `${printAnchor}${shareFunction}`);
} else if (!source.includes('const sharePdf = async')) {
  throw new Error("ManagerReports share patch: print function anchor not found.");
}
