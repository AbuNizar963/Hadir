...
if (!source.includes('from "@/lib/professionalPdf"') || !source.includes('const sharePdf = async') || !source.includes('readyPdf') || !source.includes('sharingPdf') || !source.includes('data:image/') || !source.includes('تفقد الحضور والغياب ليوم') || !hasRenderedShareControl || !hasRenderedPrintControl) {
  throw new Error(`ManagerReports share patch: PDF sharing, embedded logo, filename and print buttons were not all applied completely (share=${hasRenderedShareControl}, print=${hasRenderedPrintControl}).`);
}

writeFileSync(file, source, "utf8");
console.log("ManagerReports share patch: daily PDF preparation embeds the company logo as a data URL, names the PDF from company name + weekday + report date, and keeps native sharing split into two explicit user gestures; monthly and annual CSV preserved.");
