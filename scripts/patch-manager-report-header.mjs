...
const newHeader = `        <div className="flex flex-col items-center gap-2">{settings.brandLogo && <img src={settings.brandLogo} alt="شعار الشركة" className="h-14 w-auto max-w-[180px] object-contain" />}<h1 className="text-xl md:text-2xl font-black">{settings.brandName || "خدمة الدوام اليومية"}</h1><div className="text-sm font-bold">خدمة الدوام ليوم {days[dateOf(date).getDay()]} · {dateOf(date).toLocaleDateString("ar-EG", { day: "2-digit", month: "2-digit", year: "numeric" })}</div></div>`;
...
console.log("ManagerReports header patch: removed department owner/assistant and added company branding with Arabic weekday/date.");
