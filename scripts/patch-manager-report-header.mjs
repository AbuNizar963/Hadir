...
if (!source.includes('.daily-print-report { display: block !important;')) {
  if (source.includes(printMediaOld)) source = source.replace(printMediaOld, printMediaNew);
  else console.log("ManagerReports print media layout anchor absent; external report-print.css will provide print layout and continuing.");
}
