...
const files = ["src/lib/auth.ts", "src/lib/storage.ts", "src/lib/backend.ts", "backend/src/ai-entry.ts", "backend/src/entry.ts", "backend/src/secureApp.ts", "backend/wrangler.jsonc"];
...
if (!text["backend/src/secureApp.ts"].includes("const { token: _token, ...safeData }")) throw new Error("backend/src/secureApp.ts: auth response token stripping is missing");
if (!text["backend/src/entry.ts"].includes("sanitizeAuthResponse")) throw new Error("backend/src/entry.ts: login responses must be sanitized before reaching the browser");
...
if (text["src/lib/backend.ts"].includes("localStorage.getItem") || text["src/lib/backend.ts"].includes("localStorage.setItem") || text["src/lib/backend.ts"].includes("localStorage.removeItem")) throw new Error("src/lib/backend.ts: authentication state must not use localStorage");
