...
class _AdminHomePageState extends State<AdminHomePage> {
                ...
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        const Text('لوحة الإدارة', style: TextStyle(fontSize: 12, color: _adminMuted)),
                        Text(name, style: const TextStyle(fontSize: 21, fontWeight: FontWeight.w800, color: _adminInk)),
                      ],
                    ),
                  ),
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  ...
                  _statsGrid(),
                  const SizedBox(height: 22),
                  const Text('الإدارة', style: TextStyle(fontSize: 19, fontWeight: FontWeight.w800, color: _adminInk)),
                  const SizedBox(height: 10),
                  ...
                  _AdminFeature(icon: Icons.location_on_outlined, title: 'المواقع', subtitle: '$locationCount موقعًا مسجلًا في النظام', onTap: () => context.push('/admin/manage')),
                  ...
                  const Text('حاضر Native', textAlign: TextAlign.center, style: TextStyle(color: _adminMuted, fontSize: 11, fontWeight: FontWeight.w600)),
                ]),
              ),
            ),
          ],
  ...
  Widget _overviewCard() {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(begin: Alignment.topRight, end: Alignment.bottomLeft, colors: [_adminBrand, Color(0xFF084F44)]),
        borderRadius: BorderRadius.circular(27),
        boxShadow: const [BoxShadow(blurRadius: 25, offset: Offset(0, 12), color: Color(0x220B6B5A))],
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const Spacer(),
          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: Colors.white.withValues(alpha: .12), borderRadius: BorderRadius.circular(20)), child: Text(role, style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w700))),
        ]),
  ...
  Widget _errorCard() => Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFFFFF4F2), borderRadius: BorderRadius.circular(18)),
        child: Row(children: [
          const Icon(Icons.cloud_off_rounded, color: Color(0xFFB94A3D)),
          const SizedBox(width: 10),
          Expanded(child: Text(error!, style: const TextStyle(color: Color(0xFF8D332C), fontSize: 12))),
...
class _StatCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(color: _adminSurface, borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFFE2E9E6))),
        child: Row(children: [
          Container(width: 42, height: 42, decoration: BoxDecoration(color: _adminSoft, borderRadius: BorderRadius.circular(13)), child: Icon(icon, color: _adminBrand, size: 21)),
          const SizedBox(width: 11),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
            Text(title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11, color: _adminMuted)),
            const SizedBox(height: 2),
            Text(value, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: _adminInk)),
            Text(subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 9, color: _adminMuted)),
          ])),
        ]),
      );
}
...
class _AdminFeature extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: ListTile(
          ...
          leading: Container(width: 45, height: 45, decoration: BoxDecoration(color: _adminSoft, borderRadius: BorderRadius.circular(14)), child: Icon(icon, color: _adminBrand)),
          title: Text(title, style: const TextStyle(fontWeight: FontWeight.w800, color: _adminInk, fontSize: 14)),
          subtitle: Text(subtitle, style: const TextStyle(fontSize: 11.5)),
          trailing: const Icon(Icons.chevron_left_rounded, color: _adminMuted),
        ),
      );
}
