...
class _AttendancePageState extends State<AttendancePage> with WidgetsBindingObserver {
  ...
  Future<void> _refreshLocation() async {
    ...
    try {
      ...
      setState(() {
        ...
        _locating = false;
        if (distance > radius) {
          _error = 'أنت خارج نطاق العمل. المسافة الحالية ${distance.toStringAsFixed(1)} م، والحد ${radius.toStringAsFixed(0)} م.';
        }
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _prepareChallenge() async {
    ...
    try {
      ...
      if (!mounted) return;
      final seconds = challenge.expiresAt.difference(DateTime.now().toUtc()).inSeconds.clamp(1, 60);
      setState(() {
  ...
  void _startTimer(DateTime expiresAt) {
    ...
    _timer = Timer.periodic(const Duration(milliseconds: 250), (_) {
      final seconds = ((expiresAt.difference(DateTime.now().toUtc()).inMilliseconds + 999) ~/ 1000).clamp(0, 60);
      if (!mounted) return;
      if (seconds <= 0) {
        _timer?.cancel();
        setState(() {
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(_isCheckIn ? Icons.login_rounded : Icons.logout_rounded, color: Colors.white, size: 28),
          ),
          ...
          Expanded(
            child: Column(children: [
              Container(
                width: 34,
                height: 34,
                decoration: BoxDecoration(color: current >= i ? _brand : Colors.white, shape: BoxShape.circle, border: Border.all(color: current >= i ? _brand : _line)),
                child: Icon(icons[i], size: 17, color: current >= i ? Colors.white : _muted),
              ),
  ...
  Widget _locationCard() {
    ...
    return _panel(
      ...
      title: 'موقعك الحالي',
      subtitle: placeName,
      trailing: verified ? _statusPill('داخل النطاق', _brand, _soft) : _statusPill('بانتظار التحقق', _muted, const Color(0xFFF1F4F2)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        const SizedBox(height: 13),
        Row(children: [
          Expanded(child: _metric('المسافة', _distance == null ? '—' : '${_distance!.toStringAsFixed(1)} م')),
          const SizedBox(width: 10),
          Expanded(child: _metric('الدقة', _accuracy == null ? '—' : '±${_accuracy!.toStringAsFixed(1)} م')),
          const SizedBox(width: 10),
          Expanded(child: _metric('النطاق', rangeText)),
        ]),
        const SizedBox(height: 12),
        OutlinedButton.icon(
          onPressed: _locating || _checking || _submitting ? null : _refreshLocation,
          ...
          label: Text(_locating ? 'جاري تحديد موقعك...' : verified ? 'تحديث الموقع' : 'تحديد موقعي'),
        ),
      ]),
    );
  }
  ...
  Widget _deviceCard() {
    ...
    return _panel(
      ...
      subtitle: ok ? 'هذا الهاتف مرتبط بحساب الموظف.' : 'يجب استخدام الجهاز المرتبط بالحساب.',
      trailing: _statusPill(ok ? 'موثّق' : 'غير موثّق', ok ? _brand : _danger, ok ? _soft : const Color(0xFFFFF1F0)),
    );
  }
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        Container(
          ...
          child: _scanning
              ? Stack(fit: StackFit.expand, children: [
                  ...
                  Positioned(bottom: 15, left: 0, right: 0, child: Text('وجّه الكاميرا نحو رمز QR', textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700))),
        ...
        if (_challengeActive)
          Container(
            ...
            decoration: BoxDecoration(color: _soft, borderRadius: BorderRadius.circular(16), border: Border.all(color: _brand.withValues(alpha: .25))),
            child: Row(children: [
              const Icon(Icons.timer_outlined, color: _brand),
              const SizedBox(width: 9),
              const Expanded(child: Text('مهلة اعتماد العملية', style: TextStyle(fontWeight: FontWeight.w700, color: _ink))),
              Text('00:${_remainingSeconds.toString().padLeft(2, '0')}', style: const TextStyle(fontWeight: FontWeight.w900, color: _brand, fontSize: 18)),
            ]),
          ),
      ]),
    );
  ...
  Widget _actionArea() {
    if (_challengeActive) {
      return FilledButton.icon(
        ...
        icon: _submitting ? const SizedBox.square(dimension: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.verified_rounded),
        label: Text(_submitting ? 'جارٍ اعتماد العملية...' : 'تأكيد ${_isCheckIn ? 'تسجيل الحضور' : 'تسجيل الانصراف'}'),
      );
    }
    return FilledButton.icon(
      onPressed: _checking || _submitting || !_inRange || _qr.text.trim().isEmpty ? null : _prepareChallenge,
      icon: _checking ? const SizedBox.square(dimension: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.security_rounded),
      label: Text(_checking ? 'جارٍ التحقق...' : 'تحقق واستمر'),
    );
  }
    ...
    return Center(
      child: SingleChildScrollView(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(28), border: Border.all(color: _line)),
          child: Column(children: [
            ...
            Row(children: [
              Expanded(child: _metric('الوقت', result == null ? '—' : intl.DateFormat('HH:mm').format(result.time))),
              const SizedBox(width: 10),
              Expanded(child: _metric('المسافة', result == null ? '—' : '${result.distance.toStringAsFixed(1)} م')),
              const SizedBox(width: 10),
              Expanded(child: _metric('الدقة', result == null ? '—' : '±${result.accuracyMeters.toStringAsFixed(1)} م')),
            ]),
            const SizedBox(height: 20),
  ...
  Widget _panel({required IconData icon, required String title, required String subtitle, Widget? trailing, Widget? child}) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(21), border: Border.all(color: _line)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Row(children: [
          Container(width: 42, height: 42, decoration: BoxDecoration(color: _soft, borderRadius: BorderRadius.circular(13)), child: Icon(icon, color: _brand, size: 21)),
          const SizedBox(width: 11),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontWeight: FontWeight.w800, color: _ink)), const SizedBox(height: 2), Text(subtitle, style: const TextStyle(fontSize: 11, color: _muted))])),
  ...
  Widget _metric(String label, String value) {
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xFFF7F9F8), borderRadius: BorderRadius.circular(14)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(fontSize: 9, color: _muted)), const SizedBox(height: 3), Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: _ink))]),
    );
  }

  Widget _statusPill(String text, Color foreground, Color background) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 6), decoration: BoxDecoration(color: background, borderRadius: BorderRadius.circular(30)), child: Text(text, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: foreground)));
  }

  Widget _errorCard(String message) {
    return Container(
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(color: const Color(0xFFFFF1F0), borderRadius: BorderRadius.circular(17), border: Border.all(color: const Color(0xFFF0C8C4))),
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Icon(Icons.error_outline_rounded, color: _danger),
        const SizedBox(width: 9),
        Expanded(child: Text(message, style: const TextStyle(color: _danger, fontSize: 12, height: 1.45))),
      ]),
    );
  }
}
