...
class AttendanceRecorderWidgetState extends State<AttendanceRecorderWidget> {
  ...
  void initState() {
    ...
    initPlatformState();
    Future.microtask(() {
      geoFencingService = GeoFencing.of(context).service..addListener(onGeofenceStatusUpdate);
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(children: <Widget>[googleMap(context), buildContainer(context)]),
    );
  }
  ...
  Widget googleMap(BuildContext context) {
    ...
    return Container(
      ...
      child: GoogleMap(
        ...
        circles: _circles,
        initialCameraPosition: CameraPosition(target: LatLng(_initialLat, _initialLong), zoom: _initialZoom),
        markers: _markers,
            ...
            if (mounted) {
              setState(() {
                ...
                Timer.periodic(new Duration(milliseconds: 100), (timer) {
                  var radius = _circles.isEmpty ? office.radius : _circles.first.radius;
                  if ((radius > rMax) || (radius < rMin)) direction *= -1;
      ...
      showDialog(
        ...
        builder: (_) => Dialog(
          child: Container(
            ...
            child: Center(child: Text('Kindly retry after some time!', style: TextStyle(color: Colors.white, fontSize: 22))),
          ),
        ),
      );
    } else {
      ...
      if (serviceStatus) {
        ...
        if (_permission == PermissionStatus.granted) {
          ...
          _locationSubscription = _locationService.onLocationChanged.listen((LocationData result) async {
            final newLocation = LatLng(result.latitude ?? 0, result.longitude ?? 0);
            if (previousLocation != newLocation) {
              previousLocation = newLocation;
              _currentCameraPosition = CameraPosition(target: newLocation, zoom: 16, tilt: 50.0, bearing: 45.0);
              final GoogleMapController controller = await _controller.future;
              ...
              if (mounted) {
                setState(() {
                  ...
                  _markers.clear();
                  _markers.add(Marker(markerId: MarkerId('Current Location'), position: newLocation));
                });
              }
            }
          });
        ...
        if (serviceStatusResult) initPlatformState();
      }
