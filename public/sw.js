  ...
  event.waitUntil(
    (async () => {
      ...
      clients.forEach((client) =>
        client.postMessage({ type: "HADIR_SW_UPDATE_AVAILABLE" }),
      );
    })(),
  );
});
    ...
    caches
      ...
      .then((keys) =>
        Promise.all(
          keys
            .filter(
              (key) => key.startsWith("hadir-shell-") && key !== CACHE,
            )
  ...
  event.waitUntil(
    (async () => {
      ...
      try {
        ...
        data = {
          body: event.data?.text() || "لديك إشعار جديد في Hadir",
        };
      }

      const title = String(data.title || "إشعار جديد");
      const body = String(
        data.body || data.message || "لديك إشعار جديد في Hadir",
      );
      ...
      await self.registration.showNotification(title, {
        ...
        dir: "rtl",
        lang: "ar",
        tag: String(data.tag || `hadir-${data.type || "notification"}`),
        renotify: true,
        data: {
          url,
          type: String(data.type || "info"),
          notificationId: String(data.notificationId || data.id || ""),
        },
      });
    })(),
  );
      ...
      for (const client of windows) {
        if ("navigate" in client) {
          try {
            ...
            // Continue to the next available client or open a new window.
          }
        }
