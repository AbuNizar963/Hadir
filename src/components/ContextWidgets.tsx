...
export default function ContextWidgets() {
  ...
  const active =
    ...
    (location.pathname.startsWith("/manager") && !location.pathname.includes("/login"));
  const manager =
    location.pathname.startsWith("/manager") &&
    !location.pathname.includes("/login") &&
    Boolean(managerSession);
    ...
    <>
      {active && session && (
        <div className="fixed top-3 right-3 z-[80] w-[min(20rem,calc(100vw-1.5rem))] pointer-events-none">
          <div
            className={`hud-card p-3 shadow-xl transition-all duration-300 ${
      ...
      {manager && (
        <button
          ...
          title="المساعد الذكي"
          className="fixed z-[90] top-3 left-16 lg:left-auto lg:right-[17rem] h-11 w-11 rounded-xl bg-primary/10 border border-primary/30 text-primary shadow-lg grid place-items-center font-black text-xs hover:bg-primary/15 hover:-translate-y-0.5 transition-all"
        >
          AI
