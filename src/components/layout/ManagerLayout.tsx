...
async function loadServerNotifications(): Promise<AppNotification[]> {
  ...
  const rows = await response.json() as Array<{ id: string; userId?: string; recipientId?: string; title: string; message: string; body?: string; type: AppNotification["type"]; severity?: string; readAt: string | null; createdAt: string }>;
  return rows.map(n => ({ id: n.id, userId: String(n.userId ?? n.recipientId ?? ""), title: n.title, body: n.message ?? n.body ?? "", type: n.type ?? (n.severity === "danger" ? "error" : n.severity === "warning" ? "warning" : n.severity === "success" ? "success" : "info"), read: Boolean(n.readAt), createdAt: n.createdAt }));
}
...
export default function ManagerLayout({ title, subtitle, actions, children }: { title: string; subtitle?: string; actions?: React.ReactNode; children: React.ReactNode }) {
  ...
  const currentUserId = String((managerSession as any)?.accountId || (managerSession as any)?.id || (managerSession as any)?.userId || "");
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const sync = () => { document.documentElement.style.setProperty("--hadir-manager-topbar-h", `${Math.ceil(el.getBoundingClientRect().height)}px`); };
    sync();
  ...
  const closeTransient = () => { setMenuOpen(false); setThemeMenuOpen(false); setShowNotifications(false); setShowDiagnostics(false); };
  const logout = () => { closeTransient(); localStorage.removeItem("managerAuth"); backendLogout(); setManagerSession(null); nav("/manager/login"); };
  const filteredNav = NAV.filter(n => !n.editRoles || n.editRoles.includes(currentRole));
  const openDiagnostics = () => { setDiagnostics(getDiagnostics()); setShowDiagnostics(true); setMenuOpen(false); setThemeMenuOpen(false); setShowNotifications(false); };
  ...
  const utilityButton = (to: string, label: string, Icon: typeof CloudSun) => (<Link to={to} onClick={closeTransient} title={label} aria-label={label} className="manager-tool group flex h-12 w-20 shrink-0 flex-col items-center justify-center border border-border/70 bg-background/70 text-foreground/85 transition hover:border-primary/40 hover:bg-primary/5 hover:text-primary"><Icon className="h-5 w-5" strokeWidth={1.9} aria-hidden="true" /><span className="mt-0.5 text-[11px] font-semibold leading-none">{label}</span></Link>);
  return (<div className="manager-shell min-h-screen bg-background text-foreground" dir="rtl">
    <div ref={topbarRef} className="manager-topbar sticky top-0 z-[60] border-b border-border/70 bg-background/95 backdrop-blur-md">
      <div className="manager-brand-row mx-auto flex max-w-7xl items-center justify-between gap-3 px-2 sm:px-4"><Link to="/manager" aria-label="حاضر" onClick={closeTransient} className="shrink-0"><Brand /></Link><nav className="flex items-center gap-1.5 overflow-x-auto" aria-label="أدوات النظام"><button type="button" title="القائمة" aria-label="القائمة" aria-expanded={menuOpen} onClick={() => { setMenuOpen(v => !v); setThemeMenuOpen(false); setShowNotifications(false); setShowDiagnostics(false); }} className="manager-tool group flex h-12 w-20 shrink-0 flex-col items-center justify-center border border-border/70 bg-background/70 transition hover:border-primary/40 hover:bg-primary/5 hover:text-primary">{menuOpen ? <X className="h-5 w-5" aria-hidden="true" /> : <Menu className="h-5 w-5" aria-hidden="true" />}<span className="mt-0.5 text-[11px] font-semibold leading-none">القائمة</span></button><button type="button" onClick={() => { setShowNotifications(true); setMenuOpen(false); setThemeMenuOpen(false); setShowDiagnostics(false); }} title="الإشعارات" aria-label="الإشعارات" className="manager-tool group relative flex h-12 w-20 shrink-0 flex-col items-center justify-center border border-border/70 bg-background/70 text-foreground/85 transition hover:border-primary/40 hover:bg-primary/5 hover:text-primary"><Bell className="h-5 w-5" strokeWidth={1.9} aria-hidden="true" /><span className="mt-0.5 text-[11px] font-semibold leading-none">الإشعارات</span>{unreadCount > 0 && <span className="absolute top-0 right-0 z-10 min-w-[18px] h-[18px] px-1 rounded-full bg-destructive text-destructive-foreground text-[10px] font-bold grid place-items-center">{unreadCount > 99 ? "99+" : unreadCount}</span>}</button>{utilityButton("/weather", "الطقس", CloudSun)}{utilityButton("/prayer", "القبلة", Compass)}{utilityButton("/ai", "المساعد", Bot)}</nav></div>
      <div className="border-t border-border/60"><nav className="manager-nav mx-auto flex max-w-7xl items-stretch gap-1 overflow-x-auto px-2 py-1.5 sm:justify-center sm:px-4" aria-label="إدارة النظام">{filteredNav.map(n => { const Icon = n.icon; return <NavLink key={n.to} to={n.to} end={n.end as any} onClick={closeTransient} aria-label={n.label} title={n.label} className={({ isActive }) => cn("relative flex min-w-[86px] shrink-0 flex-col items-center justify-center rounded-xl px-2 py-1.5 text-center transition", isActive ? "bg-primary/15 text-primary ring-1 ring-primary/35 shadow-sm" : "text-foreground/80 hover:bg-secondary hover:text-foreground")}><Icon className="h-5 w-5" strokeWidth={1.9} aria-hidden="true" /><span className="mt-1 whitespace-nowrap text-[11px] font-semibold leading-none">{n.label}</span>{n.to === "/manager/requests" && unreadCount > 0 && <span className="absolute top-0 right-0 z-10 min-w-[18px] h-[18px] px-1 rounded-full bg-destructive text-destructive-foreground text-[10px] font-bold grid place-items-center">{unreadCount > 99 ? "99+" : unreadCount}</span>}</NavLink>; })}</nav></div>
    </div>
    {menuOpen && <div className="manager-utility-menu fixed left-0 right-0 z-[70] border-b border-border/60 bg-card/98 shadow-lg" dir="rtl"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-start gap-2 px-3 py-2">
      ...
      <div className="relative"><button type="button" onClick={() => setThemeMenuOpen(v => !v)} aria-expanded={themeMenuOpen} className="rounded-lg px-3 py-2 text-sm font-semibold hover:bg-secondary"><Palette className="mr-1 inline h-4 w-4" />المظهر</button>{themeMenuOpen && <div className="absolute right-0 top-full z-50 mt-1 w-48 rounded-xl border border-border bg-card p-1 shadow-xl"><button onClick={() => setThemeAndKeepMenu("dark")} className={cn("w-full rounded-lg px-3 py-2 text-right text-sm hover:bg-secondary", theme === "dark" && "bg-primary/10 text-primary")}><Moon className="mr-2 inline h-4 w-4" />داكن</button><button onClick={() => setThemeAndKeepMenu("light")} className={cn("w-full rounded-lg px-3 py-2 text-right text-sm hover:bg-secondary", theme === "light" && "bg-primary/10 text-primary")}><Sun className="mr-2 inline h-4 w-4" />فاتح</button><button onClick={() => setThemeAndKeepMenu("system")} className={cn("w-full rounded-lg px-3 py-2 text-right text-sm hover:bg-secondary", theme === "system" && "bg-primary/10 text-primary")}><Monitor className="mr-2 inline h-4 w-4" />تلقائي</button></div>}</div>
