      ...
      <header className="sticky top-0 z-[60] border-b border-border/70 bg-background/95 backdrop-blur">
        <div className="mx-auto flex min-h-16 max-w-6xl items-center justify-between gap-3 px-3 sm:px-5" dir="rtl">
          ...
          <nav className="flex min-w-0 items-center gap-1.5" aria-label="تنقل الموظف">
            ...
            <Link to="/employee/profile" title="الملف الشخصي" aria-label="الملف الشخصي" className="grid h-12 w-12 place-items-center rounded-xl border border-border/60 bg-background hover:bg-secondary transition">
              <UserRound className="h-5 w-5" aria-hidden="true" />
            </Link>
            <button type="button" title="القائمة" aria-label="القائمة" aria-expanded={menuOpen} onClick={() => setMenuOpen(value => !value)} className="grid h-12 w-12 place-items-center rounded-xl border border-border/60 bg-background hover:bg-secondary transition">
