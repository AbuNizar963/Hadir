...
export default function EmployeeLayout({ title = "لوحة الموظف", subtitle, actions, children }: { title?: string; subtitle?: string; actions?: ReactNode; children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background text-foreground" dir="rtl">
    ...
    {menuOpen && <div className="sticky top-[138px] z-50 border-b border-border/60 bg-card/98 shadow-lg"><div className="mx-auto flex max-w-7xl flex-wrap items-center gap-2 px-3 py-2" dir="rtl">
      <Link to="/employee/notifications" onClick={() => setMenuOpen(false)} className="rounded-lg px-3 py-2 text-sm font-semibold hover:bg-secondary"><Bell className="mr-1 inline h-4 w-4" aria-hidden="true" />الإشعارات</Link>
