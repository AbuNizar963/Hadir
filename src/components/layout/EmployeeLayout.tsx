...
import { Bot, CloudSun, Compass, LayoutDashboard, Menu, UserRound, X, Clock3, CreditCard, Building2, LogOut, Bell, Monitor, Moon, Palette, Sun } from "lucide-react";
...
export default function EmployeeLayout({ title = "لوحة الموظف", subtitle, actions, children }: { title?: string; subtitle?: string; actions?: ReactNode; children: ReactNode }) {
  ...
  const logout = async () => { try { await backendLogout(); } catch {} try { localStorage.removeItem("hadir.api.token.employee"); localStorage.removeItem("hadir.employee.session"); localStorage.removeItem("employeeAuth"); } catch {} setMenuOpen(false); setThemeMenuOpen(false); navigate("/login", { replace: true }); };
  const setThemeAndKeepMenu = (value: Theme) => { setTheme(value); setThemeMenuOpen(false); };
  ...
  return <div className="min-h-screen bg-background text-foreground" dir="rtl">
    <div className="sticky top-0 z-[60] border-b border-border/70 bg-background/95 backdrop-blur-md">
      <div className="mx-auto flex min-h-[76px] max-w-7xl items-center justify-between gap-3 px-2 sm:px-4">
        ...
        <nav className="flex items-center gap-1.5 overflow-x-auto" aria-label="أدوات النظام">
          <button type="button" title="القائمة" aria-label="القائمة" aria-expanded={menuOpen} onClick={() => { setMenuOpen(v => !v); setThemeMenuOpen(false); }} className={cn(utilityClass, menuOpen && "border-primary/40 bg-primary/5 text-primary")}>{menuOpen ? <X className="h-5 w-5" aria-hidden="true" /> : <Menu className="h-5 w-5" aria-hidden="true" />}<span className="mt-0.5 text-[11px] font-semibold leading-none">القائمة</span></button>
          <Link to="/employee/notifications" title="الإشعارات" aria-label="الإشعارات" className={utilityClass}><Bell className="h-5 w-5" strokeWidth={1.9} aria-hidden="true" /><span className="mt-0.5 text-[11px] font-semibold leading-none">الإشعارات</span></Link>
    ...
    {menuOpen && <div className="sticky top-[138px] z-50 border-b border-border/60 bg-card/98 shadow-lg"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-start gap-2 px-3 py-2" dir="rtl">
      <div className="relative"><button type="button" onClick={() => setThemeMenuOpen(v => !v)} aria-expanded={themeMenuOpen} className="rounded-lg px-3 py-2 text-sm font-semibold hover:bg-secondary"><Palette className="mr-1 inline h-4 w-4" />المظهر</button>{themeMenuOpen && <div className="absolute right-0 top-full mt-1 z-50 w-48 rounded-xl border border-border bg-card p-1 shadow-xl"><button type="button" onClick={() => setThemeAndKeepMenu("dark")} className={cn("w-full rounded-lg px-3 py-2 text-right text-sm hover:bg-secondary", theme === "dark" && "bg-primary/10 text-primary")}><Moon className="mr-2 inline h-4 w-4" />داكن</button><button type="button" onClick={() => setThemeAndKeepMenu("light")} className={cn("w-full rounded-lg px-3 py-2 text-right text-sm hover:bg-secondary", theme === "light" && "bg-primary/10 text-primary")}><Sun className="mr-2 inline h-4 w-4" />فاتح</button><button type="button" onClick={() => setThemeAndKeepMenu("system")} className={cn("w-full rounded-lg px-3 py-2 text-right text-sm hover:bg-secondary", theme === "system" && "bg-primary/10 text-primary")}><Monitor className="mr-2 inline h-4 w-4" />تلقائي</button></div>}</div>
      <button type="button" onClick={() => void logout()} className="rounded-lg px-3 py-2 text-sm font-semibold text-destructive hover:bg-destructive/10"><LogOut className="mr-1 inline h-4 w-4" aria-hidden="true" />تسجيل خروج</button>
    </div></div>}
