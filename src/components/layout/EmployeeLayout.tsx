...
import { BarChart3, Bot, CloudSun, Compass, LayoutDashboard, Menu, UserRound, X, Clock3, CreditCard, Building2 } from "lucide-react";
        ...
        <div className="mx-auto flex min-h-[76px] max-w-7xl items-center justify-between gap-3 px-2 sm:px-4">
          ...
          <nav className="flex items-center gap-1.5 overflow-x-auto" aria-label="أدوات النظام">
            <button
              ...
              className="group flex h-12 w-20 shrink-0 flex-col items-center justify-center rounded-xl border border-border/70 bg-background/70 transition hover:border-primary/40 hover:bg-primary/5 hover:text-primary"
            >
              {menuOpen ? <X className="h-5 w-5" aria-hidden="true" /> : <Menu className="h-5 w-5" aria-hidden="true" />}
              <span className="mt-0.5 text-[11px] font-semibold leading-none">القائمة</span>
            </button>

            <Link to="/weather" title="الطقس" aria-label="الطقس" className="group flex h-12 w-20 shrink-0 flex-col items-center justify-center rounded-xl border border-border/70 bg-background/70 transition hover:border-primary/40 hover:bg-primary/5 hover:text-primary">
              <CloudSun className="h-5 w-5" strokeWidth={1.9} aria-hidden="true" />
              <span className="mt-0.5 text-[11px] font-semibold leading-none">الطقس</span>
            </Link>

            <Link to="/prayer" title="القبلة" aria-label="القبلة" className="group flex h-12 w-20 shrink-0 flex-col items-center justify-center rounded-xl border border-border/70 bg-background/70 transition hover:border-primary/40 hover:bg-primary/5 hover:text-primary">
              <Compass className="h-5 w-5" strokeWidth={1.9} aria-hidden="true" />
              <span className="mt-0.5 text-[11px] font-semibold leading-none">القبلة</span>
            </Link>

            <Link to="/ai" title="المساعد" aria-label="المساعد" className="group flex h-12 w-20 shrink-0 flex-col items-center justify-center rounded-xl border border-border/70 bg-background/70 transition hover:border-primary/40 hover:bg-primary/5 hover:text-primary">
              <Bot className="h-5 w-5" strokeWidth={1.9} aria-hidden="true" />
              <span className="mt-0.5 text-[11px] font-semibold leading-none">المساعد</span>
            </Link>
          </nav>
        </div>
      ...
      {menuOpen && (
        <div className="sticky top-[138px] z-50 border-b border-border/60 bg-card/98 shadow-lg">
          <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-start gap-2 px-3 py-2" dir="rtl">
            ...
            <span className="px-3 py-2 text-xs text-muted-foreground">{session?.employeeId ? "حساب الموظف" : "الموظف"}</span>
          </div>
        </div>
      )}
