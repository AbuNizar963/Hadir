...
import { Bot, CloudSun, Compass, LayoutDashboard, Menu, UserRound, X, Clock3, CreditCard, Building2, LogOut, Bell } from "lucide-react";
...
export default function EmployeeLayout({ title = "لوحة الموظف", subtitle, actions, children }: { title?: string; subtitle?: string; actions?: ReactNode; children: ReactNode }) {
  ...
  useEffect(() => { if (typeof document !== "undefined") { document.documentElement.style.colorScheme = "dark"; document.documentElement.classList.add("dark"); } }, []);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground" dir="rtl">
      <div className="sticky top-0 z-[60] border-b border-border/70 bg-background/95 backdrop-blur-md">
        <div className="mx-auto flex min-h-[76px] max-w-7xl items-center justify-between gap-3 px-2 sm:px-4">
          ...
          <nav className="flex items-center gap-1.5 overflow-x-auto" aria-label="أدوات النظام">
            ...
            <div className="shrink-0"><NotificationBell userId={session?.employeeId ? String(session.employeeId) : undefined} onlyBell /></div>
      ...
      {menuOpen && <div className="sticky top-[138px] z-50 border-b border-border/60 bg-card/98 shadow-lg"><div className="mx-auto flex max-w-7xl flex-wrap items-center gap-2 px-3 py-2" dir="rtl"><Link to="/employee" onClick={() => setMenuOpen(false)} className="rounded-lg px-3 py-2 text-sm font-semibold hover:bg-secondary">لوحة الموظف</Link><Link to="/employee/profile" onClick={() => setMenuOpen(false)} className="rounded-lg px-3 py-2 text-sm font-semibold hover:bg-secondary">الملف الشخصي</Link><Link to="/employee/scan/attendance" onClick={() => setMenuOpen(false)} className="rounded-lg px-3 py-2 text-sm font-semibold hover:bg-secondary">تسجيل الحضور</Link><Link to="/employee/scan/departure" onClick={() => setMenuOpen(false)} className="rounded-lg px-3 py-2 text-sm font-semibold hover:bg-secondary">تسجيل الانصراف</Link><span className="rounded-lg px-3 py-2 text-xs text-muted-foreground">{session?.employeeId ? "حساب الموظف" : "الموظف"}</span><button type="button" onClick={() => void logout()} className="rounded-lg px-3 py-2 text-sm font-semibold text-destructive hover:bg-destructive/10"><LogOut className="mr-1 inline h-4 w-4" aria-hidden="true" />تسجيل خروج</button></div></div>}
