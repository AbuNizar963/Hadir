...
export default function AdminAccountsPanel() {
  ...
  const toggle = async (id: string, active: boolean) => {
    try { if (backendEnabled) { await updateBackendAdmin(id, { active: !active }); await loadRemote(); } else saveLocal(accounts.map(a=>a.id===id?{...a,active:!a.active}:a)); }
    catch (e) { setMessage(e instanceof Error ? e.message : "تعذر تعديل الحساب"); }
  };
  const remove = async (id: string) => {
    if (!confirm("حذف هذا الحساب الإداري؟")) return;
    try { if (backendEnabled) { await deleteBackendAdmin(id); await loadRemote(); } else saveLocal(accounts.filter(a=>a.id!==id)); }
    catch (e) { setMessage(e instanceof Error ? e.message : "تعذر حذف الحساب"); }
  };
  ...
  return <section className="hud-card p-5 sm:p-6 lg:col-span-2 border-primary/50 bg-primary/5">
