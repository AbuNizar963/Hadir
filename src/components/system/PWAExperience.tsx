...
function isAndroidTwa() {
  ...
  return /^android-app:\/\//i.test(document.referrer);
}
...
function installDismissedThisVersion() {
  try {
    return (
      sessionStorage.getItem(INSTALL_DISMISSED_KEY) === PWA_INSTALL_VERSION
    );
  } catch {
    return false;
  }
...
export default function PWAExperience() {
  ...
  useEffect(() => {
    ...
    const shouldShowUpdate = () => {
      ...
      try {
        if (
          !cancelled &&
          sessionStorage.getItem(UPDATE_DISMISSED_KEY) !== "1"
        ) {
    ...
    const checkDeploymentVersion = async () => {
      ...
      try {
        ...
        try {
          lastOpenBuildVersion =
            localStorage.getItem(UPDATE_BUILD_VERSION_KEY)?.trim() || "";
        } catch {
          // Continue without persistent build-version state.
        }

        if (!lastOpenBuildVersion) {
          try {
            localStorage.setItem(UPDATE_BUILD_VERSION_KEY, buildVersion);
          } catch {
            // Continue without persistent build-version state.
          }
          return;
  ...
  useEffect(() => {
    const onControllerChange = () => {
      const versionToRemember =
        pendingBuildVersionRef.current || detectedBuildVersionRef.current;

      if (versionToRemember) {
        try {
          localStorage.setItem(UPDATE_BUILD_VERSION_KEY, versionToRemember);
        } catch {
          // Continue even when persistent storage is unavailable.
        }
      }
  ...
  const dismissInstall = () => {
    try {
      sessionStorage.setItem(INSTALL_DISMISSED_KEY, PWA_INSTALL_VERSION);
    } catch {
      // Storage can be unavailable in private/restricted browser contexts.
    }
  ...
  const update = async () => {
    ...
    try {
      ...
      if (!versionResponse.ok) {
        throw new Error(
          "تعذر الوصول إلى ملف إصدار النشر (build-version.json). تحقق من اتصال الإنترنت.",
        );
      }

      const versionData = (await versionResponse.json().catch(() => null)) as {
        commitSha?: unknown;
      } | null;
      const buildVersion =
        typeof versionData?.commitSha === "string"
          ? versionData.commitSha.trim()
          : "";

      if (!buildVersion) {
        throw new Error("ملف إصدار النشر لا يحتوي على رقم إصدار صالح.");
      }
      ...
      const reloadAfterActivation = async (worker: ServiceWorker) => {
        ...
        setUpdating(false);
        setUpdateError(
          "تم تثبيت Service Worker الجديد لكن لم يكتمل تفعيل النسخة الجديدة تلقائيًا. أغلق التطبيق وافتحه مرة واحدة لإكمال التحديث.",
        );
      };
      ...
      const installedWorkerPromise = new Promise<ServiceWorker | null>(
        (resolve) => {
          let settled = false;
      ...
      setUpdating(false);
      setUpdateError(
        `لم يصل Service Worker الجديد إلى حالة التثبيت بعد 20 ثانية. الإصدار المطلوب: ${buildVersion.slice(0, 12)}…`,
      );
    } catch (error) {
      setUpdating(false);
      const message =
        error instanceof Error && error.message
          ? error.message
          : "حدث خطأ غير معروف أثناء تثبيت Service Worker الجديد.";
  ...
  if (offline) {
    return (
      <div
        dir="rtl"
        className="fixed bottom-4 left-4 right-4 z-[100] mx-auto max-w-xl rounded-2xl border border-amber-500/30 bg-background/95 p-4 shadow-2xl backdrop-blur-xl"
      >
        <div className="flex items-center gap-3">
          <div className="grid size-10 shrink-0 place-items-center rounded-xl bg-amber-500/15 text-xl">
            📡
          </div>
          <div className="min-w-0 flex-1">
            <p className="font-bold">أنت غير متصل بالإنترنت</p>
            <p className="mt-0.5 text-xs text-muted-foreground">
              تم الحفاظ على جلسة الدخول. سيُستأنف الاتصال تلقائيًا عند عودته.
            </p>
          </div>
        </div>
      </div>
  ...
  if (updateVisible && standalone && !isAndroidTwa()) {
    return (
      <div
        dir="rtl"
        className="fixed bottom-4 left-4 right-4 z-[100] mx-auto max-w-xl rounded-2xl border border-primary/25 bg-background/95 p-4 shadow-2xl backdrop-blur-xl"
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              سيتم تحديث واجهة التطبيق بأمان دون حذف بياناتك المحلية أو جلسة
              تسجيل الدخول.
            </p>
            {updateError && (
  ...
  if (installVisible && installEvent) {
    return (
      ...
      >
        <div className="flex items-start gap-3">
          <div className="grid size-12 shrink-0 place-items-center rounded-2xl bg-primary/10 text-2xl">
            📱
          </div>
          <div className="min-w-0 flex-1">
            <p className="font-bold">ثبّت حاضر كتطبيق</p>
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              هذا هو تثبيت PWA الحقيقي، وليس اختصارًا عاديًا. سيعمل حاضر بواجهة
              مستقلة عن المتصفح ويحافظ على جلسة الحساب.
            </p>
            <div className="mt-3 flex gap-2">
              <button
                type="button"
    ...
    return (
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-bold">أضف حاضر إلى الشاشة الرئيسية</p>
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              على iPhone أو iPad: افتح قائمة المشاركة في Safari ثم اختر «إضافة
              إلى الشاشة الرئيسية». هذه هي طريقة تثبيت PWA الرسمية على أجهزة
              Apple.
            </p>
            <button
              type="button"
