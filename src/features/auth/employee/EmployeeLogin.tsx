      ...
      <main className="flex-1 grid place-items-center px-5 pb-10">
        <div className="w-full max-w-md hud-card p-7">
          ...
          <form onSubmit={submit} className="mt-6 space-y-4" noValidate={false}>
            <div>
              <label className="block text-sm text-semibold mb-1.5" htmlFor="employee-job-number">الرقم الوظيفي</label>
              <input id="employee-job-number" name="username" className="input w-full" value={jobNumber} onChange={e => setJobNumber(e.target.value)} autoComplete="username" inputMode="text" required />
            </div>
            <div>
              <label className="block text-sm font-semibold mb-1.5" htmlFor="employee-pin">رمز الدخول</label>
              <input id="employee-pin" name="password" className="input w-full" type="password" value={pin} onChange={e => setPin(e.target.value)} autoComplete="current-password" inputMode="numeric" required />
            </div>
            {error && <div className="rounded-xl border border-destructive/40 bg-destructive/10 p-3 text-sm" role="alert">{error}</div>}
            <button type="submit" className="btn-primary w-full py-3" disabled={loading} aria-busy={loading}>
