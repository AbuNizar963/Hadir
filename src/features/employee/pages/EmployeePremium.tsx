...
export default function EmployeePremium() {
  ...
  return <div className="min-h-screen bg-background" dir="rtl">
    ...
    <main className="max-w-5xl mx-auto px-4 pb-12 space-y-5">
      ...
      {tab === "performance" && <section className="hud-card p-5"><h2 className="font-black text-xl">أدائي وإحصائياتي</h2><div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-5"><Metric label="أيام الحضور" value={present}/><Metric label="مكتمل" value={complete}/><Metric label="التأخر" value={late}/><Metric label="ساعات العمل" value={formatDurationMinutes(worked)}/></div><div className="mt-5 rounded-2xl border p-4"><div className="text-xs text-muted-foreground">نسبة اكتمال تسجيلات الحضور</div><div className="text-3xl font-black text-primary mt-1">{present ? Math.round(complete/present*100) : 0}%</div></div></section>}
