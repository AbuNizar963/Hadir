...
export default function ManagerAudit() {
  ...
  const exportCsv=()=>{const headers=["م","الوقت","الموظف","الرقم الوظيفي","العملية","النتيجة","السبب","الجهاز","IP","خط العرض","خط الطول","المسافة (م)"];const body:CsvCell[][]=filtered.map((a,index)=>[index+1,safeDate(a.timestamp),a.actorName??"",a.jobNumber??"",actionLabel(a.action),a.result==="success"?"نجاح":"رفض",a.reason??"",a.deviceId??"",a.ip??"",a.lat??"",a.lng??"",a.distanceMeters??""]);downloadCSV(`audit-${new Date().toISOString().slice(0,10)}`,headers,body);};
