...
export function getEmployeeWorkPeriod(employee: Employee | null | undefined, target: Date = new Date()): WorkPeriod {
  ...
  if ((employee.scheduleType ?? "ADMIN") === "ADMIN") {
    const workDays = normalizeWorkDays(employee.workDays); const day = target.getDay();
    ...
    const start = withTime(target, parseTime(employee.workStartTime, "09:00")); let end = withTime(target, parseTime(employee.workEndTime, "16:00")); if (end.getTime() <= start.getTime()) end = new Date(end.getTime() + DAY_MS);
  ...
  const end = withTime(addLocalDays(periodStart, info.daysOn), parseTime(employee.workEndTime, employee.workStartTime || "09:00"));
...
export function getActiveWorkPeriod(employee: Employee | null | undefined, target: Date = new Date()): WorkPeriod {
  const current = getEmployeeWorkPeriod(employee, target); if (current.isWorkDay) return current;
  if (employee?.scheduleType === "ROTATION" && employee.rotationStartDate) { const previous = new Date(target.getTime() - DAY_MS); const previousPeriod = getEmployeeWorkPeriod(employee, previous); if (previousPeriod.isWorkDay && previousPeriod.start && previousPeriod.end && target >= previousPeriod.start && target < previousPeriod.end) return previousPeriod; }
  return current;
}

export function getScheduleCountdown(employee: Employee | null | undefined, target: Date = new Date()): ScheduleCountdown {
  if (!employee) return { kind: "NONE", target: null, label: "" }; const period = getEmployeeWorkPeriod(employee, target);
...
function getNextAdminWorkStart(employee: Employee, target: Date): Date | null { const days = normalizeWorkDays(employee.workDays); if (!days.length) return null; const startTime = parseTime(employee.workStartTime, "09:00"); for (let offset = 0; offset <= 7; offset++) { const candidate = new Date(target.getTime()); candidate.setDate(candidate.getDate() + offset); candidate.setHours(startTime.hours, startTime.minutes, 0, 0); if (!days.includes(candidate.getDay())) continue; if (candidate.getTime() > target.getTime()) return candidate; } return null; }
function getRotationInfo(employee: Employee, target: Date): { firstStart: Date; periodStart: Date; daysOn: number; daysOff: number; cycleDay: number; workDay: number; phase: "WORK" | "OFF" | "NOT_STARTED" } | null {
  const startDate = parseYYYYMMDD(employee.rotationStartDate); if (!startDate) return null; const daysOn = Math.max(1, Math.floor(employee.rotationDaysOn ?? 4)); const daysOff = Math.max(0, Math.floor(employee.rotationDaysOff ?? 4)); const cycleLength = daysOn + daysOff; const firstStart = withTime(startDate, parseTime(employee.workStartTime || employee.rotationStartTime, "09:00")); const elapsed = target.getTime() - firstStart.getTime(); if (elapsed < 0) return { firstStart, periodStart: firstStart, daysOn, daysOff, cycleDay: 0, workDay: 0, phase: "NOT_STARTED" }; const cycleMs = cycleLength * DAY_MS; const cycleIndex = Math.floor(elapsed / cycleMs); const cycleElapsed = elapsed - cycleIndex * cycleMs; const cycleDay = Math.floor(cycleElapsed / DAY_MS); const periodStart = new Date(firstStart.getTime() + cycleIndex * cycleMs); if (cycleDay >= daysOn) return { firstStart, periodStart, daysOn, daysOff, cycleDay, workDay: 0, phase: "OFF" }; return { firstStart, periodStart, daysOn, daysOff, cycleDay, workDay: cycleDay, phase: "WORK" };
}
...
function addLocalDays(date: Date, days: number): Date { const result = new Date(date); result.setDate(result.getDate() + days); return result; }
