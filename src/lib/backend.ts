...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    throw new Error("تعذر الاتصال بخادم حاضر. تحقق من اتصال Cloudflare Worker.");
  }
...
export async function bootstrapBackend() {
  const data = await request<{ token: string; bootstrap: boolean }>("/api/bootstrap");
  if (!data.bootstrap) throw new Error("تم إعداد حساب المالك مسبقًا");
  localStorage.setItem("hadir.api.token", data.token);
  return data;
}

export async function createBootstrapOwner(input: { name: string; username: string; password: string }) {
  const data = await request<{ token: string; user: any; kind: "admin" }>("/api/bootstrap/owner", {
