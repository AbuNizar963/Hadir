...
export async function backendLogin(username:string,password:string){const data=await request<{token:string;user:any;kind:"admin"|"employee"}>("/api/auth/login",{method:"POST",body:JSON.stringify({username,password,deviceId:getDeviceId()})});localStorage.setItem("hadir.api.token",data.token);return data;}
export async function backendEmployeeLogin(username:string,password:string){return backendLogin(username,password);}
