...
function hasEmployeeSession(): boolean {
  const session = readStoredJson<{ employeeId?: string }>(EMPLOYEE_SESSION_KEY);
  return Boolean(session?.employeeId);
}
function hasManagerSession(): boolean {
  const session = readStoredJson<{ accountId?: string; role?: string }>(
    MANAGER_SESSION_KEY,
  );
  return Boolean(session?.accountId || session?.role);
}
function tokenForRole(role: RoleHint): string {
...
function activeRole(): RoleHint | undefined {
  if (hasManagerSession()) return "admin";
  if (hasEmployeeSession()) return "employee";
  return undefined;
}
function token(): string {
  const role = activeRole();
  return role ? tokenForRole(role) : "";
}
function clearRoleSession(role: RoleHint): void {
  if (typeof window === "undefined") return;
  localStorage.removeItem(
    role === "admin" ? ADMIN_TOKEN_KEY : EMPLOYEE_TOKEN_KEY,
  );
  if (role === "admin") localStorage.removeItem(MANAGER_SESSION_KEY);
  else localStorage.removeItem(EMPLOYEE_SESSION_KEY);
}
function persistToken(role: RoleHint, value: string): void {
  if (typeof window === "undefined") return;
  const opposite: RoleHint = role === "admin" ? "employee" : "admin";
  clearRoleSession(opposite);
...
export function validateAvatarDataUrl(value: unknown): string | null {
  ...
  if (bytes > MAX_AVATAR_BYTES)
    throw new Error("حجم صورة الموظف الأصلية يجب ألا يتجاوز 10 ميغابايت.");
  return value;
}
async function prepareAvatar(value: unknown): Promise<string | null> {
  const validated = validateAvatarDataUrl(value);
  if (!validated) return null;
  const compressed = await compressProfileImageDataUrl(validated, {
    maxWidth: 512,
    maxHeight: 512,
    quality: 0.78,
    type: "image/webp",
    maxBytes: MAX_STORED_AVATAR_BYTES,
  });
  ...
  const padding = base64.endsWith("==") ? 2 : base64.endsWith("=") ? 1 : 0;
  const bytes = Math.max(0, Math.floor((base64.length * 3) / 4) - padding);
  if (bytes > MAX_STORED_AVATAR_BYTES)
    throw new Error("تعذر ضغط صورة الموظف إلى أقل من 100 كيلوبايت.");
  return compressed;
}
...
function isTransientNetworkError(error: unknown): boolean {
  ...
  return (
    ...
    error.message.startsWith("تعذر الاتصال بخادم حاضر (")
  );
}
...
async function waitForVisible(timeoutMs = 15000): Promise<void> {
...
async function request<T>(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        typeof error.error === "string"
          ? error.error
          : `فشل الاتصال بالخادم (${response.status})`,
      );
    }
    return data as T;
  } catch (error) {
    if (error instanceof DOMException && error.name === "AbortError")
      throw new Error(
        "انتهت مهلة الاتصال بخادم حاضر. سيتم إعادة المحاولة تلقائيًا عند عودة الاتصال.",
      );
    if (
      ...
      throw new Error(
        `تعذر الاتصال بخادم حاضر (${API_URL}). تحقق من اتصال الإنترنت وCloudflare Worker.`,
      );
    throw error;
  } finally {
...
async function uploadEmployeeAvatar(
  ...
  if (
    ...
    blob.size <= 0 ||
    blob.size > MAX_STORED_AVATAR_BYTES
  )
    throw new Error("صورة الموظف يجب أن تكون WebP وأقل من 100 كيلوبايت.");
  const form = new FormData();
  form.append("file", blob, "avatar.webp");
  return requestWithRetry<{ key: string; size: number }>(
...
async function deleteEmployeeAvatar(employeeId: string): Promise<void> {
  await requestWithRetry<{ ok: boolean }>(
    `/api/employees/${encodeURIComponent(employeeId)}/avatar`,
    { method: "DELETE" },
    3,
    "admin",
...
export async function backendEmployeeLogin(username: string, password: string) {
  ...
  if (data.kind !== "employee")
    throw new Error("هذا الحساب إداري وليس حساب موظف");
  persistToken("employee", data.token);
  return data.user;
}
export async function registerEmployeePasskey() {
  const options = await request<any>(
    "/api/device/passkey/registration/options",
    {},
    "employee",
  );
  ...
  return request<{ verified: boolean }>(
    "/api/device/passkey/registration/verify",
    { method: "POST", body: JSON.stringify(credential) },
    "employee",
  );
}
export async function getEmployeeDeviceStatus() {
  return request<{
    bound: boolean;
    passkeyCount: number;
    deviceLabel: string | null;
  }>("/api/device/status", {}, "employee");
}
export async function bootstrapBackend() {
  const data = await requestWithRetry<{ token: string; bootstrap: boolean }>(
    "/api/bootstrap",
  );
  if (!data.bootstrap) throw new Error("تم إعداد حساب المالك مسبقًا");
  persistToken("admin", data.token);
  return data;
}
export async function createBootstrapOwner(input: {
  name: string;
  username: string;
  password: string;
}) {
  const data = await requestWithRetry<{
    ...
    user: AdminAccount;
    kind: "admin";
  }>("/api/bootstrap/owner", { method: "POST", body: JSON.stringify(input) });
  persistToken("admin", data.token);
  return data.user;
}
export async function backendLogout(role?: RoleHint) {
  const resolved = role || activeRole();
  if (!resolved) return;
  try {
    await requestWithRetry<{ ok: boolean }>(
      "/api/auth/logout",
    ...
    clearRoleSession(resolved);
  }
}
export async function backendMe() {
  const role = activeRole();
  return requestWithRetry<{ user: AdminAccount | Employee }>(
    "/api/me",
    {},
    5,
    role,
...
export async function createBackendAdmin(input: {
  name: string;
  username: string;
  password: string;
  role: "manager" | "supervisor";
}) {
  return requestWithRetry<{ ok: boolean }>(
    "/api/admins",
...
export async function updateBackendAdmin(
  id: string,
...
export async function resetBackendEmployeeDevice(id: string) {
  return requestWithRetry<{ ok: boolean }>(
    `/api/employees/${encodeURIComponent(id)}/device`,
    { method: "DELETE" },
    3,
    "admin",
  );
}
export async function getBackendAttendance(limit = 500) {
  const role = activeRole();
  if (role === "admin") {
    const audit = await request<any[]>(
      `/api/audit?limit=${Math.min(limit, 2000)}`,
      {},
      "admin",
    );
    return audit
      .filter(
        (row) =>
          row.result === "success" &&
          (row.action === "check-in" || row.action === "check-out") &&
          row.employeeId,
      )
      .map((row) => ({
        id: String(row.id),
        ...
        employeeName: String(row.actorName || ""),
        type: row.action as "check-in" | "check-out",
  ...
  return request<AttendanceRecord[]>(
    `/api/attendance?limit=${Math.min(limit, 2000)}`,
    {},
    role,
  );
}
export async function createAttendanceChallenge(input: {
...
export async function createBackendAttendance(
  record: Omit<AttendanceRecord, "id" | "ip"> & { challengeId?: string },
) {
  const submit = (payload: typeof record) =>
    requestWithRetry<{ ok: boolean; record?: AttendanceRecord }>(
      "/api/attendance",
  ...
  try {
    ...
    const requiresChallenge =
      /تحقق\s*الحضور\s*(مطلوب|ضروري)|attendance\s*verification\s*(required|needed)/i.test(
        reason,
      );
    ...
    if (!challenge?.ok || !challenge.challengeId)
      throw new Error("تعذر إنشاء تحقق الحضور. أعد المحاولة.");
    return await submit({ ...record, challengeId: challenge.challengeId });
  }
}
export async function getBackendRequests(role?: RoleHint) {
  return request<EmployeeRequest[]>("/api/requests", {}, role || activeRole());
}
export async function createBackendRequest(
  input: Omit<EmployeeRequest, "id" | "status" | "createdAt">,
) {
  return requestWithRetry<{ ok: boolean }>(
    "/api/requests",
    { method: "POST", body: JSON.stringify(input) },
...
export async function updateBackendRequest(
  id: string,
...
export async function getBackendAudit(limit = 500) {
  return request<any[]>(
    `/api/audit?limit=${Math.min(limit, 2000)}`,
    {},
    "admin",
...
export async function saveBackendLocation(location: Location) {
  return requestWithRetry<{ ok: boolean }>(
    "/api/locations",
...
export async function deleteBackendLocation(id: string) {
  return requestWithRetry<{ ok: boolean }>(
...
export async function getBackendEmployeeLocation(): Promise<{
  location: Location;
}> {
  const employee = await getBackendEmployeeProfile();
  const locations = await getBackendLocations("employee");
  const assigned = employee.locationId
    ? locations.find((item) => String(item.id) === String(employee.locationId))
    : undefined;
  const main =
    locations.find((item) => String(item.id) === "main") || locations[0];
...
export async function getBackendEmployeeProfile() {
  const currentToken = tokenForRole("employee");
  if (!currentToken)
    throw new Error("جلسة الموظف غير موجودة. يرجى تسجيل الدخول مرة أخرى.");
  if (employeeProfilePromise && employeeProfileToken === currentToken)
    return employeeProfilePromise;
  employeeProfileToken = currentToken;
  const promise = requestWithRetry<Employee>(
    "/api/employee/profile",
    {},
    5,
    "employee",
  );
  employeeProfilePromise = promise;
  try {
    return await promise;
  } catch (error) {
    if (employeeProfilePromise === promise) {
...
export async function createBackendEmployee(input: Record<string, unknown>) {
  const avatar = await prepareAvatar(input.avatar);
  const { avatar: _avatar, ...payload } = input;
  const created = await requestWithRetry<{ ok: boolean; employee: Employee }>(
    "/api/employees",
    { method: "POST", body: JSON.stringify(payload) },
    3,
    "admin",
  ...
  if (avatar && created.employee?.id) {
    ...
    created.employee = {
      ...created.employee,
      avatar: employeeAvatarUrl(uploaded.key, created.employee.id),
    };
  }
  return created;
}
export async function updateBackendEmployee(
  id: string,
  ...
  const { avatar: _avatar, ...payload } = input;
  const updated = await requestWithRetry<{ ok: boolean; employee: Employee }>(
  ...
  if (hasAvatar) {
    if (avatar) {
      ...
      updated.employee = {
        ...updated.employee,
        avatar: employeeAvatarUrl(uploaded.key, id),
      };
    } else {
...
export async function deleteBackendEmployee(id: string) {
  return requestWithRetry<{ ok: boolean }>(
    `/api/employees/${encodeURIComponent(id)}`,
    { method: "DELETE" },
    3,
...
export async function getBackendEscapeEvents(employeeId?: string, limit = 500) {
  const query = new URLSearchParams({ limit: String(Math.min(limit, 2000)) });
  if (employeeId) query.set("employeeId", employeeId);
  return request<EscapeEvent[]>(
    `/api/escape-events?${query.toString()}`,
    {},
    "admin",
  );
}
export async function createBackendEscapeEvent(input: {
  employeeId: string;
  ...
  return requestWithRetry<{ ok: boolean; event: EscapeEvent }>(
    "/api/escape-events",
    { method: "POST", body: JSON.stringify(input) },
    3,
    "admin",
...
export async function resetBackendTestData() {
  return requestWithRetry<{
    ...
    "/api/workforce/reset",
    { method: "POST", body: JSON.stringify({ confirmation: "تأكيد" }) },
    3,
    "admin",
  );
}
