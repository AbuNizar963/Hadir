...
function persistToken(role: RoleHint, value: string): void { if (typeof window === "undefined") return; const opposite: RoleHint = role === "admin" ? "employee" : "admin"; clearRoleSession(opposite); localStorage.setItem(role === "admin" ? ADMIN_TOKEN_KEY : EMPLOYEE_TOKEN_KEY, value); persistPwaSession(value); }
...
export async function backendEmployeeLogin(username: string, password: string) { if (typeof window === "undefined") throw new Error("تسجيل الدخول متاح من المتصفح فقط."); const deviceId = getDeviceId(); const fingerprint = await getDeviceFingerprint(); const data = await requestWithRetry<{ token: string; user: Employee; kind: "admin" | "employee" }>("/api/auth/login", { method: "POST", body: JSON.stringify({ username: username.trim(), password, deviceId, deviceLabel: getDeviceLabel(), deviceFingerprint: fingerprint }) }, 3); if (data.kind !== "employee") throw new Error("هذا الحساب إداري وليس حساب موظف"); persistToken("employee", data.token); return data.user; }
export async function registerEmployeePasskey() { const options = await request<any>("/api/device/passkey/registration/options", {}, "employee"); const { startRegistration } = await import("@simplewebauthn/browser"); const credential = await startRegistration({ optionsJSON: options }); return request<{ verified: boolean }>("/api/device/passkey/registration/verify", { method: "POST", body: JSON.stringify(credential) }, "employee"); }
export async function getEmployeeDeviceStatus() { return request<{ bound: boolean; passkeyCount: number; deviceLabel: string | null }>("/api/device/status", {}, "employee"); }
