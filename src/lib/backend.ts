...
function hasManagerSession(): boolean {
  const session = readStoredJson<{ accountId?: string; role?: string }>(MANAGER_SESSION_KEY);
...
function clearRoleSession(role: RoleHint): void {
  if (typeof window === "undefined") return;
  localStorage.removeItem(role === "admin" ? ADMIN_TOKEN_KEY : EMPLOYEE_TOKEN_KEY);
...
function persistToken(role: RoleHint, value: string): void {
  ...
  localStorage.setItem(role === "admin" ? ADMIN_TOKEN_KEY : EMPLOYEE_TOKEN_KEY, value);
}
...
async function request<T>(path: string, init: RequestInit = {}, roleHint?: RoleHint): Promise<T> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(typeof error.error === "string" ? error.error : `فشل الاتصال بالخادم (${response.status})`);
    }
    return data as T;
  } catch (error) {
    ...
    if (error instanceof Error && !error.message.startsWith("Failed to fetch")) throw error;
...
async function requestWithRetry<T>(path: string, init: RequestInit = {}, attempts = 3, roleHint?: RoleHint): Promise<T> {
...
async function uploadEmployeeAvatar(employeeId: string, dataUrl: string): Promise<{ key: string; size: number }> {
  ...
  return requestWithRetry<{ key: string; size: number }>(`/api/employees/${encodeURIComponent(employeeId)}/avatar`, { method: "POST", body: form }, 3, "admin");
}

async function deleteEmployeeAvatar(employeeId: string): Promise<void> {
  await requestWithRetry<{ ok: boolean }>(`/api/employees/${encodeURIComponent(employeeId)}/avatar`, { method: "DELETE" }, 3, "admin");
}

export async function backendLogin(username: string, password: string) {
  const data = await requestWithRetry<{ token: string; user: AdminAccount; kind: "admin" | "employee" }>("/api/auth/login", { method: "POST", body: JSON.stringify({ username: username.trim(), password }) }, 3);
...
export async function backendEmployeeLogin(username: string, password: string) {
  if (typeof window === "undefined") throw new Error("تسجيل الدخول متاح من المتصفح فقط.");
  const key = "hadir.device.id";
  const existing = localStorage.getItem(key);
  const deviceId = existing && existing.trim().length >= 8 ? existing : `dev-${crypto.randomUUID()}`;
  localStorage.setItem(key, deviceId);
  const data = await requestWithRetry<{ token: string; user: Employee; kind: "admin" | "employee" }>("/api/auth/login", { method: "POST", body: JSON.stringify({ username: username.trim(), password, deviceId, deviceLabel: getDeviceLabel() }) }, 3);
...
export function backendLogout(role?: RoleHint) { const resolved = role || activeRole(); if (resolved) clearRoleSession(resolved); }
...
export async function getBackendEmployeeLocation(): Promise<{ location: Location }> {
  ...
  const lat = Number(candidate.lat); const lng = Number(candidate.lng); const radiusMeters = Number(candidate.radiusMeters);
  if (!Number.isFinite(lat) || !Number.isFinite(lng) || !Number.isFinite(radiusMeters) || radiusMeters <= 0) throw new Error("بيانات موقع العمل في قاعدة بيانات D1 غير صالحة.");
...
export async function getBackendEmployeeProfile() {
  ...
  const promise = requestWithRetry<Employee>("/api/employee/profile", {}, 3, "employee");
  employeeProfilePromise = promise;
  try { return await promise; } catch (error) { if (employeeProfilePromise === promise) { employeeProfilePromise = null; employeeProfileToken = ""; } throw error; }
}
...
export async function createBackendEmployee(input: Record<string, unknown>) {
  ...
  const created = await requestWithRetry<{ ok: boolean; employee: Employee }>("/api/employees", { method: "POST", body: JSON.stringify(payload) }, 3, "admin");
...
export async function updateBackendEmployee(id: string, input: Record<string, unknown>) {
  ...
  const { avatar: _avatar, ...payload } = input;
  const updated = await requestWithRetry<{ ok: boolean; employee: Employee }>(`/api/employees/${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify(payload) }, 3, "admin");
  if (hasAvatar) {
    if (avatar) { const uploaded = await uploadEmployeeAvatar(id, avatar); updated.employee = { ...updated.employee, avatar: uploaded.key }; }
    else { await deleteEmployeeAvatar(id); updated.employee = { ...updated.employee, avatar: null }; }
  }
  return updated;
}
