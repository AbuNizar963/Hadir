...
export async function backendLogin(username: string, password: string) { const data = await request<{ token:string;user:any;kind:"admin"|"employee" }>("/api/auth/login",{method:"POST",body:JSON.stringify({username,password})}); if(data.kind!=="admin")throw new Error("هذا الحساب موظف وليس حساب إدارة"); localStorage.setItem("hadir.api.token",data.token); return data.user; }
export async function backendEmployeeLogin(username: string, password: string) {
  let deviceId: string; try { deviceId = await getPersistentFingerprintId(); } catch { throw new Error("تعذر التحقق من الجهاز. فعّل JavaScript وحاول مرة أخرى."); }
  const data = await request<{ token:string;user:any;kind:"admin"|"employee" }>("/api/auth/login",{method:"POST",body:JSON.stringify({username:username.trim(),password,deviceId,deviceLabel:getDeviceLabel()})});
