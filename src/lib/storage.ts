...
export function saveSettings(next:Settings):void{if(backendEnabled){const {brandLogo:_brandLogo,...settingsWithoutLogo}=next;void saveBackendSettings(settingsWithoutLogo).then(()=>hydrateLocalData());return;}if(typeof window!=="undefined")window.dispatchEvent(new Event("hadir:settings-changed"));}
