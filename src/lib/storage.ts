...
export const defaultSettings:Settings={qrCode:"HADIR-SITE-01-STATIC",workSiteLat:24.7136,workSiteLng:46.6753,radiusMeters:100,workStart:"08:00",workEnd:"16:00",lateGraceMinutes:10,ownerUsername:"",ownerPasswordHash:"",ownerName:"",managerUsername:"",managerPasswordHash:"",managerName:"",supervisorUsername:"",supervisorPasswordHash:"",supervisorName:"",adminAccounts:[],brandName:"حاضِر",brandLogo:null,locations:[]};
export function getSettings():Settings{if(!backendEnabled)return defaultSettings;const view=getD1View();const base=view.settings||defaultSettings;return {...base,locations:view.locations.length?view.locations:(base.locations||[])};}
...
function readSession<T>(key:string):T|null{if(typeof window==="undefined")return null;try{const raw=sessionStorage.getItem(key);return raw?JSON.parse(raw) as T:null;}catch{return null;}}
function writeSession<T>(key:string,value:T|null){if(typeof window==="undefined")return;if(value===null)sessionStorage.removeItem(key);else sessionStorage.setItem(key,JSON.stringify(value));}
...
export function seedIfEmpty():void{if(backendEnabled){void hydrateLocalData();}}
export function resetAll():void{if(backendEnabled)return;if(typeof window!=="undefined"){sessionStorage.removeItem(K.SESSION);sessionStorage.removeItem(K.MANAGER_SESSION);}}
