...
async function syncFromD1() { try { const r = await fetch(`${API_URL}/api/notifications`, { headers: headers(), credentials: "include", cache: "no-store" }); if (!r.ok) return; const rows = await r.json() as any[]; const previous = new Map(readAll().map(n => [n.id, n])); const mapped = prune(rows.map(n => ({ id:String(n.id), userId:String(n.recipientId ?? n.userId ?? ""), title:String(n.title ?? "إشعار"), body:String(n.message ?? n.body ?? ""), type:(n.severity === "danger" ? "error" : n.severity === "warning" ? "warning" : n.severity === "success" ? "success" : n.type ?? "info") as NotificationType, read:Boolean(n.readAt), createdAt:String(n.createdAt) })) as AppNotification[]); for (const n of mapped) if (!previous.has(n.id)) showBrowserNotification(n); writeAll(mapped); } catch {} }
let polling = false;
