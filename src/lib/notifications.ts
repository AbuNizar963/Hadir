...
export async function fetchNotifications(): Promise<AppNotification[]> { try { const r = await fetch(`${API_URL}/api/notifications`, { credentials: "include", cache: "no-store" }); if (!r.ok) return []; const rows = await r.json() as any[]; if (!Array.isArray(rows)) return []; const mapped = prune(rows.map(n => ({ id:String(n.id), userId:String(n.userId ?? n.recipientId ?? ""), title:String(n.title || "إشعار"), body:String(n.message ?? n.body ?? ""), type:(n.type ?? (n.severity === "danger" ? "error" : n.severity === "warning" ? "warning" : n.severity === "success" ? "success" : "info")) as NotificationType, read:Boolean(n.readAt), createdAt:String(n.createdAt) })) as AppNotification[]); const previous = new Map(readAll().map(n => [n.id, n])); for (const n of mapped) if (!previous.has(n.id)) showBrowserNotification(n); writeAll(mapped); return mapped; } catch { return []; } }
let polling = false;
export function startNotificationPolling() { if (polling || typeof window === "undefined") return () => {}; polling = true; void requestNotificationPermission(); void fetchNotifications(); const timer = window.setInterval(() => void fetchNotifications(), 15000); return () => { window.clearInterval(timer); polling = false; }; }
...
export function getNotifications(userId?: string): AppNotification[] { const list=readAll(); return userId ? list.filter(n=>n.userId===userId || !n.userId) : list; }
export function getUnreadCount(userId?: string): number { return getNotifications(userId).filter(n=>!n.read).length; }
export async function markAsRead(id: string) { writeAll(readAll().map(n=>n.id===id?{...n,read:true}:n)); await fetch(`${API_URL}/api/notifications/read`,{method:"POST",headers:headers(),credentials:"include",body:JSON.stringify({id})}).catch(()=>{}); }
export async function markAllAsRead(userId?: string) { writeAll(readAll().map(n=>(!userId||n.userId===userId)?{...n,read:true}:n)); await fetch(`${API_URL}/api/notifications/read`,{method:"POST",headers:headers(),credentials:"include",body:"{}"}).catch(()=>{}); }
