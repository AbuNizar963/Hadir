...
function activeToken(): string {
  ...
  return localStorage.getItem("hadir.api.token.employee") || localStorage.getItem("hadir.api.token.admin") || localStorage.getItem("hadir.api.token") || "";
}
...
function showBrowserNotification(n: AppNotification) {
  ...
  try {
    if ("serviceWorker" in navigator) {
      void navigator.serviceWorker.ready.then(reg => reg.showNotification(n.title, { body: n.body, tag: `hadir-${n.id}`, icon: "/pwa-192x192.png", badge: "/pwa-192x192.png" })).catch(() => new Notification(n.title, { body: n.body }));
    } else new Notification(n.title, { body: n.body });
  } catch {}
}

async function syncFromD1() {
  const t = activeToken(); if (!t) return;
  try {
    const r = await fetch(`${API_URL}/api/notifications`, { headers: headers(), credentials: "include", cache: "no-store" });
    if (!r.ok) return;
    const rows = await r.json() as any[];
    const previous = new Map(readAll().map(n => [n.id, n]));
    const mapped = rows.map(n => ({ id:n.id, userId:n.userId, title:n.title, body:n.message, type:n.type, read:Boolean(n.readAt), createdAt:n.createdAt })) as AppNotification[];
    for (const n of mapped) if (!previous.has(n.id)) showBrowserNotification(n);
    writeAll(mapped);
  } catch {}
}
...
export function startNotificationPolling() {
  ...
  void syncFromD1();
  const timer = window.setInterval(() => void syncFromD1(), 15000);
  return () => { window.clearInterval(timer); polling = false; };
}
