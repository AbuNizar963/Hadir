...
export default function ProfessionalAttendanceReports() {
  ...
  const exportExcel = () => {
    ...
    downloadProfessionalAttendanceReport({
      ...
      dailyRows: dailyRows.map((x) => ({ employee: x.employeeName, jobNumber: x.jobNumber, specialty: sourceLabels[x.attendanceSource] || x.attendanceSource, date: x.attendanceDay, day: x.attendanceDay, status: statusLabels[x.status] || x.status, checkIn: x.checkInAt || "—", checkOut: x.checkOutAt || "—", worked: fmtMinutes(x.workedMinutes || 0), late: x.lateMinutes, early: x.earlyLeaveMinutes, detail: x.exceptionCode || "" })),
      chartData: statusData,
      absenceRows: dailyRows.filter((x) => x.status === "ABSENT").map((x) => ({ employee: x.employeeName, jobNumber: x.jobNumber, specialty: sourceLabels[x.attendanceSource] || x.attendanceSource, date: x.attendanceDay, day: x.attendanceDay, status: "غياب", checkIn: "—", checkOut: "—", worked: "—", late: 0, early: 0, detail: x.exceptionCode || "" })),
    });
  };

  const exportCsv = () => {
    if (!report) return;
    const rows = report.rows.map((x) => [x.attendanceDay, x.employeeName, x.jobNumber || "", statusLabels[x.status] || x.status, sourceLabels[x.attendanceSource] || x.attendanceSource, x.checkInAt || "", x.checkOutAt || "", fmtMinutes(x.workedMinutes || 0), x.lateMinutes, x.earlyLeaveMinutes, x.overtimeMinutes, x.exceptionCode || ""]);
  ...
  return <ManagerLayout title="التقارير" subtitle="نظام التقارير العالمي · الحضور والموارد البشرية" actions={<div className="flex flex-wrap gap-2"><Button variant="outline" onClick={exportExcel} disabled={!report}><FileSpreadsheet className="ml-2 h-4 w-4" />Excel</Button><Button variant="outline" onClick={exportCsv} disabled={!report}><Download className="ml-2 h-4 w-4" />CSV</Button><Button onClick={printReport} disabled={!report}><FileText className="ml-2 h-4 w-4" />PDF / طباعة</Button></div>}>
    <div dir="rtl" className="space-y-5 pb-10 print:bg-white">
      ...
      {loading && !report ? <Card><CardContent className="p-10 text-center text-muted-foreground">جاري بناء التقرير من طبقة البيانات الرسمية…</CardContent></Card> : report && <>
        ...
        {tab === "daily" && <Card><CardHeader><CardTitle className="text-lg">السجل اليومي الرسمي</CardTitle></CardHeader><CardContent className="overflow-x-auto"><table className="w-full min-w-[1120px] text-sm"><thead><tr className="border-b text-right"><th className="p-3">التاريخ</th><th className="p-3">الموظف</th><th className="p-3">الحالة</th><th className="p-3">مصدر الحضور</th><th className="p-3">الحضور</th><th className="p-3">الانصراف</th><th className="p-3">العمل</th><th className="p-3">تأخر</th><th className="p-3">مبكر</th><th className="p-3">إضافي</th><th className="p-3">استثناء</th></tr></thead><tbody>{dailyRows.map((r) => <tr key={`${r.attendanceDay}-${r.employeeId}`} className="border-b last:border-0"><td className="p-3">{r.attendanceDay}</td><td className="p-3 font-semibold">{r.employeeName}<div className="text-xs text-muted-foreground">{r.jobNumber || "—"}</div></td><td className="p-3">{statusLabels[r.status] || r.status}</td><td className="p-3">{sourceLabels[r.attendanceSource] || r.attendanceSource}</td><td className="p-3">{r.checkInAt ? new Date(r.checkInAt).toLocaleTimeString("ar-SY", { hour: "2-digit", minute: "2-digit" }) : "—"}</td><td className="p-3">{r.checkOutAt ? new Date(r.checkOutAt).toLocaleTimeString("ar-SY", { hour: "2-digit", minute: "2-digit" }) : "—"}</td><td className="p-3">{fmtMinutes(r.workedMinutes || 0)}</td><td className="p-3">{r.lateMinutes}د</td><td className="p-3">{r.earlyLeaveMinutes}د</td><td className="p-3">{r.overtimeMinutes}د</td><td className="p-3">{r.exceptionCode || "—"}</td></tr>)}</tbody></table></CardContent></Card>}

        {tab === "employees" && <Card><CardHeader><CardTitle className="text-lg">ملخص الموظفين</CardTitle></CardHeader><CardContent className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead><tr className="border-b text-right"><th className="p-3">الموظف</th><th className="p-3">الأيام</th><th className="p-3">حاضر</th><th className="p-3">متأخر</th><th className="p-3">غياب</th><th className="p-3">إجازة</th><th className="p-3">استئذان</th><th className="p-3">راحة</th><th className="p-3">العمل</th><th className="p-3">التأخر</th></tr></thead><tbody>{topEmployees.map((r) => <tr key={r.employeeId} className="border-b last:border-0"><td className="p-3 font-semibold">{r.employeeName}<div className="text-xs text-muted-foreground">{r.jobNumber || "—"}</div></td><td className="p-3">{r.days}</td><td className="p-3">{r.present}</td><td className="p-3">{r.late}</td><td className="p-3">{r.absent}</td><td className="p-3">{r.leave}</td><td className="p-3">{r.permission}</td><td className="p-3">{r.rest}</td><td className="p-3">{fmtMinutes(r.workedMinutes || 0)}</td><td className="p-3">{r.lateMinutes}د</td></tr>)}</tbody></table></CardContent></Card>}

        {tab === "exceptions" && <Card><CardHeader><CardTitle className="text-lg">الاستثناءات القابلة للتتبع</CardTitle></CardHeader><CardContent className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead><tr className="border-b text-right"><th className="p-3">التاريخ</th><th className="p-3">الموظف</th><th className="p-3">الحالة</th><th className="p-3">الرمز</th><th className="p-3">الدقائق</th><th className="p-3">المصدر</th></tr></thead><tbody>{exceptions.map((r, i) => <tr key={`${r.attendanceDay}-${r.employeeId}-${r.code}-${i}`} className="border-b last:border-0"><td className="p-3">{r.attendanceDay}</td><td className="p-3 font-semibold">{r.employeeName}<div className="text-xs text-muted-foreground">{r.jobNumber || "—"}</div></td><td className="p-3">{statusLabels[r.status] || r.status}</td><td className="p-3">{r.code}</td><td className="p-3">{r.minutes}</td><td className="p-3">{sourceLabels[r.attendanceSource] || r.attendanceSource}</td></tr>)}</tbody></table></CardContent></Card>}
      </>}
    </div>
  </ManagerLayout>;
}
