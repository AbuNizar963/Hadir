    ...
    throw new Error(
      String(
        (data as { error?: unknown })?.error ||
          "تعذر تحميل طلبات فك ربط الهاتف",
      ),
    );
  }
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <>
          {pending.length === 0 ? (
            <div className="hud-card p-5 text-center text-sm text-muted-foreground mt-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="font-bold">
            {request.employeeName} · {request.jobNumber}
          </div>
          <div className="text-xs text-muted-foreground mt-1">
            طلب {typeLabel(request.type)} · {request.reason || "بدون سبب"}
          </div>
        </div>
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          {isRebind && request.deviceLabel && (
            <div className="text-xs text-muted-foreground mt-1">
              الهاتف المطلوب: {request.deviceLabel}
            </div>
          )}
        ...
        <div className="flex gap-2">
          <button
            disabled={busy}
            onClick={() => void review(request.id, "approved", request.type)}
            className="rounded-xl bg-primary text-primary-foreground px-4 py-2 text-xs font-bold disabled:opacity-50"
          >
            {busy ? "جاري…" : "موافقة"}
          </button>
          <button
            disabled={busy}
            onClick={() => void review(request.id, "rejected", request.type)}
            className="rounded-xl border border-destructive/30 text-destructive px-4 py-2 text-xs font-bold disabled:opacity-50"
          >
            رفض
          </button>
        </div>
