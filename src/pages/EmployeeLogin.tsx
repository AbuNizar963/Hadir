...
export default function EmployeeLogin() {
  ...
  // Restore the persistent employee session before showing the login form.
  // localStorage survives normal browser/PWA restarts; the server remains authoritative.
  useEffect(() => {
    let cancelled = false;
    ...
    void backendMe().then((result) => {
      ...
      // An invalid/expired server session must not be trusted locally.
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Passkey is a durable device anchor. Register it only when this device has none,
      // rather than prompting the employee on every subsequent login.
      try {
        const device = await getEmployeeDeviceStatus();
        if (!device.passkeyCount) await registerEmployeePasskey();
      } catch { /* device binding remains enforced by the server */ }
      nav("/employee", { replace: true });
    } catch (error) {
  ...
  if (restoring) {
    return <div dir="rtl" className="min-h-screen grid place-items-center bg-background p-6 text-sm font-bold">جاري استعادة جلسة الموظف…</div>;
  }
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 grid place-items-center px-5 py-10"><section className="w-full max-w-md">
        ...
        <div className="hud-card p-5 sm:p-6 shadow-sm"><form onSubmit={submit} className="space-y-4">
          ...
          <label className="block"><div className="flex items-center justify-between mb-2"><span className="text-sm font-bold">رمز الدخول</span><span className="text-[11px] text-muted-foreground">PIN</span></div><input type="password" inputMode="numeric" autoComplete="current-password" className="input w-full p-3.5 rounded-2xl border border-border bg-secondary/45 text-base tracking-[0.3em]" value={pin} onChange={e=>setPin(e.target.value)} placeholder="••••" required disabled={loading}/></label>
