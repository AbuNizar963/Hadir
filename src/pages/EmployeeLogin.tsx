        ...
        <div className="w-full max-w-md hud-card p-7">
          ...
          <form onSubmit={submit} className="mt-6 space-y-4">
            <div>
              ...
              <input type="text" className="input w-full p-2.5 rounded-xl border border-border bg-secondary/50 text-sm" value={jobNumber} onChange={(e) => setJobNumber(e.target.value)} placeholder="مثال: 1001" required />
            </div>
            <div>
              <label className="block text-sm font-semibold mb-1.5">رمز الـ PIN السري</label>
              <input type="password" className="input w-full p-2.5 rounded-xl border border-border bg-secondary/50 text-sm" value={pin} onChange={(e) => setPin(e.target.value)} placeholder="أدخل رمز الـ PIN" required />
            </div>
