...
export default function EmployeeLogin() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    let timer = 0;
    try {
      if (!isValidEmployeePassword(pin)) { setErr("رمز الدخول يجب أن يتكون من 4 أحرف أو أكثر، باستخدام الأحرف الإنجليزية والأرقام."); setLoading(false); return; }
      setManagerSession(null);
  ...
  return <div className="min-h-screen flex flex-col bg-background">
    ...
    <main className="flex-1 grid place-items-center px-5 py-10"><section className="w-full max-w-md">
      ...
      <div className="hud-card p-5 sm:p-6 shadow-sm"><form onSubmit={submit} className="space-y-4">
        ...
        <label className="block"><div className="flex items-center justify-between mb-2"><span className="text-sm font-bold">رمز الدخول</span><span className="text-[11px] text-muted-foreground">4+ أحرف</span></div><input type="password" inputMode="text" autoComplete="current-password" className="input w-full p-3.5 rounded-2xl border border-border bg-secondary/45 text-base tracking-[0.3em]" value={pin} onChange={e=>setPin(e.target.value.replace(/[^A-Za-z0-9]/g, ""))} placeholder="4 أحرف أو أكثر" required disabled={loading}/></label>
