    ...
    <div className="min-h-screen">
      <header className="max-w-xl mx-auto px-5 py-5 flex items-center justify-between">
        <div ref={menuRef} className="relative z-[100]">
          ...
          {menuOpen && (
            <div className="absolute left-0 top-full mt-2 z-[9999] w-60 rounded-2xl border border-border/70 bg-background shadow-2xl p-2" role="menu">
              <button type="button" onClick={handleProfile} className="w-full flex items-center gap-3 rounded-xl px-3 py-3 text-right text-sm font-bold hover:bg-secondary transition-colors" role="menuitem">
      ...
      <main className="max-w-xl mx-auto px-5 pb-16 space-y-5">
        ...
        <section className="hud-card p-5 sm:p-6 border-primary/20 bg-primary/[0.03]"><div className="flex items-center justify-between gap-3"><div><div className="text-sm font-bold">مهلة تأكيد {title}</div><div className="text-[11px] text-muted-foreground mt-1">بعد التحقق من الموقع وQR. لا يمكن تأكيد العملية بعد انتهاء الدقيقة.</div></div><div className={`shrink-0 rounded-2xl border px-4 py-2.5 text-center min-w-[88px] ${confirmationActive ? "border-primary/30 bg-secondary/60" : "border-border/60 bg-secondary/30"}`}><div className={`mono font-black text-xl leading-none tabular-nums ${confirmationActive ? "text-primary" : "text-muted-foreground"}`}>{timerText}</div><div className="text-[10px] text-muted-foreground mt-1">{timerReadyText}</div></div></div></section>
        <section className="hud-card p-5 sm:p-6"><div className="flex items-center justify-between mb-3"><div className="text-sm font-bold">١. التحقق من الموقع</div><StepBadge state={step === "loading-location" || step === "gps" || isLocating ? "active" : step === "error" ? "fail" : "done"} /></div><GpsRadar /><div className="text-center mt-4"><div className="text-base font-extrabold radar-status">{statusText}</div><div className="text-xs text-muted-foreground mt-1">{step === "loading-location" ? "يتم تحميل موقع العمل من الخادم..." : isLocating ? "جاري استقبال بيانات الموقع من جهازك ومقارنتها بالموقع المسجل..." : step === "gps" || step === "error" ? "اضغط لتحديد موقعك ومقارنته بموقع العمل." : "تم تحديد موقع جهازك ومقارنته بنطاق العمل."}</div></div><button type="button" onClick={() => void refreshGps()} disabled={isLocating || !location} className="btn-primary w-full mt-4 py-3 flex items-center justify-center gap-2">{isLocating ? <><span className="animate-spin">⟳</span> جاري تحديد موقعي...</> : <>📍 تحديد موقعي الحالي</>}</button><div className="grid grid-cols-2 gap-2 text-center mt-4 text-xs"><Cell label="المسافة الحالية" value={distance !== null ? `${Math.round(distance)} م` : "…"} /><Cell label="النطاق المسموح" value={Number.isFinite(targetRadius) ? `${Math.round(targetRadius)} م` : "…"} /><Cell label="خط العرض" value={pos ? pos.lat.toFixed(5) : "…"} /><Cell label="خط الطول" value={pos ? pos.lng.toFixed(5) : "…"} /></div>{distance !== null && !inRange && <div className="mt-3 rounded-xl border border-destructive/40 bg-destructive/10 p-3 text-center text-xs text-destructive font-semibold">الموقع خارج النطاق؛ لن يتم السماح بتسجيل العملية.</div>}</section>
        <section className={`hud-card p-6 ${!inRange ? "opacity-60" : ""}`}><div className="flex items-center justify-between mb-3"><div className="text-sm font-bold">٢. مسح رمز QR</div><StepBadge state={qrInput ? "done" : step === "scan" && inRange ? "active" : "idle"} /></div><div className="rounded-2xl border border-border/70 bg-background overflow-hidden text-center">{isCameraActive ? <div className="relative min-h-[72vh] sm:min-h-[580px] w-full bg-black overflow-hidden"><video ref={videoRef} className="absolute inset-0 h-full w-full object-cover" muted playsInline autoPlay /><div className="absolute inset-0 bg-black/25 pointer-events-none" /><div className="absolute inset-x-0 top-0 z-10 flex items-center justify-between p-4 text-white"><div><div className="text-sm font-bold">ماسح رمز QR</div><div className="text-[11px] text-white/70 mt-0.5">طابق الرمز مع الإطار الأخضر</div></div><button type="button" onClick={stopCamera} className="h-10 w-10 rounded-full bg-black/55 border border-white/20 grid place-items-center text-lg" aria-label="إغلاق الكاميرا">×</button></div><div className="absolute inset-0 grid place-items-center pointer-events-none"><div className="relative w-[76vw] max-w-[340px] aspect-square rounded-[28px] bg-black/10 shadow-[0_0_0_9999px_rgba(0,0,0,.30)]"><div className="absolute inset-0 rounded-[28px] border-[3px] border-primary shadow-[0_0_24px_hsl(var(--primary)/.35),inset_0_0_18px_hsl(var(--primary)/.08)]" /><div className="absolute inset-[9px] rounded-[20px] border border-white/20" /><div className="absolute left-[12%] right-[12%] top-1/2 h-px bg-primary shadow-[0_0_14px_hsl(var(--primary)/.95)] animate-pulse" /><div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-2.5 w-2.5 rounded-full bg-primary shadow-[0_0_16px_hsl(var(--primary)/1)]" /></div></div><div className="absolute inset-x-0 bottom-0 z-10 p-5"><div className="mx-auto max-w-sm rounded-2xl bg-black/55 backdrop-blur-md border border-white/15 p-3 flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-primary animate-pulse shrink-0" /><span className="text-xs text-white/90 flex-1 text-right">{scannerSupported ? "ضع رمز QR داخل الإطار وسيتم التعرف عليه تلقائيًا" : "الكاميرا تعمل، لكن المسح التلقائي غير مدعوم في هذا المتصفح"}</span>{torchSupported && <button type="button" onClick={() => void toggleTorch()} className={`h-10 w-10 shrink-0 rounded-xl border text-base ${torchOn ? "bg-white text-black border-white" : "bg-white/10 text-white border-white/20"}`} aria-label={torchOn ? "إطفاء الإضاءة" : "تشغيل الإضاءة"}>⌁</button>}</div><button type="button" onClick={stopCamera} className="mt-2 w-full max-w-sm mx-auto block rounded-xl bg-white/10 border border-white/20 text-white py-2.5 text-xs font-bold">إغلاق الماسح</button></div></div> : <div className="p-6 sm:p-8"><div className="mx-auto h-20 w-20 rounded-2xl bg-secondary grid place-items-center mb-4 border border-border/60"><QrIcon /></div><p className="text-sm font-semibold">امسح رمز QR المخصص لـ{locationName}</p><p className="text-xs text-muted-foreground max-w-sm mx-auto mt-1.5 leading-6">افتح الكاميرا ووجّه الرمز داخل الإطار الأخضر. سيتم التعرف عليه تلقائيًا.</p><div className="flex flex-col sm:flex-row justify-center gap-2 mt-5"><button type="button" onClick={() => void startCamera()} className="btn-primary text-xs shadow" disabled={step !== "scan" || !inRange}>📷 فتح ماسح QR</button><button type="button" onClick={() => setQrInput(settings.qrCode || "")} className="btn-secondary text-xs" disabled={step !== "scan" || !inRange || !settings.qrCode}>إدخال القيمة يدويًا</button></div></div>}</div><div className="mt-4"><label className="block text-xs text-muted-foreground mb-1" htmlFor="qr-code">قيمة QR</label><input id="qr-code" className="input mono text-sm" placeholder="أدخل القيمة المطبوعة على QR" value={qrInput} onChange={(e) => setQrInput(e.target.value)} disabled={step !== "scan" || !inRange} autoComplete="off" /></div></section>
        <section className="hud-card p-6">{step === "error" && <div className="rounded-xl border border-destructive/40 bg-destructive/10 p-3 text-sm mb-4"><div className="font-semibold text-destructive">فشل التحقق</div><div className="text-xs text-muted-foreground mt-1">{error}</div></div>}{step === "success" && result && <div className="rounded-xl border border-primary/40 bg-primary/10 p-4 text-sm mb-4"><div className="font-extrabold text-primary text-lg">تمت العملية بنجاح</div><div className="text-xs text-muted-foreground mt-1">{title} في {formatTime(result.time)}{result.timeNote ? ` · ${result.timeNote}` : ""}</div></div>}<div className="flex gap-3 items-center"><button className="btn-primary flex-1 py-3" onClick={submit} disabled={step !== "scan" || !inRange || !qrInput.trim() || !pos || !confirmationActive}>{step === "submitting" ? "جاري التسجيل..." : `تأكيد ${title}`}</button>{(step === "success" || step === "error") && <button onClick={() => { stopCamera(); nav("/employee"); }} className="btn-secondary">عودة</button>}</div></section>
      </main>
    </div>
  );
}
