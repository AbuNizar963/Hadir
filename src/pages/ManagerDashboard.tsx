...
const labels: Record<string, string> = { PRESENT: "حاضر", LATE: "متأخر", ABSENT: "غياب", LEAVE: "إجازة", PERMISSION: "استئذان", REST: "راحة", ESCAPED: "انصراف دون تسجيل", NOT_STARTED: "لم يبدأ", INVALID: "غير صالح", OPEN: "انصراف معلق" };
...
export default function GlobalAttendanceReports() {
  ...
  const statusData = report ? [
    ...
    { name: labels.ESCAPED, value: report.summary.escaped }, { name: labels.NOT_STARTED, value: report.summary.notStarted },
    ...
    <Dialog open={detail !== null || detailLoading || detailError !== null} onOpenChange={(open) => { if (!open && !detailLoading) { setDetail(null); setDetailError(null); } }}>
      <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto" dir="rtl">
        ...
        {detail && <div className="space-y-5">
          ...
          <Card><CardHeader><CardTitle className="text-base">سجل التدقيق المرتبط ({detail.sources.audit.length})</CardTitle></CardHeader><CardContent className="space-y-2">{detail.sources.audit.map((x) => <div key={String(x.id)} className="rounded-md border p-3 text-xs"><div className="font-bold">ID: {String(x.id)}</div><div className="mt-1 grid gap-1 sm:grid-cols-2"><div>الإجراء: {String(x.action || "—")}</div><div>النتيجة: {String(x.result || "—")}</div><div>الوقت: {time(x.timestamp)}</div><div className="sm:col-span-2">السبب: {String(x.reason || "—")}</div></div></div>)}{!detail.sources.audit.length && <div className="text-sm text-muted-foreground">لا توجد سجلات تدقيق مرتبطة.</div>}</CardContent></Card>
