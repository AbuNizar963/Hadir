...
export default function EmployeeHome() {
    ...
    const loadFromD1 = async () => {
      ...
      try {
        const timeout = new Promise<never>((_, reject) => {
          window.setTimeout(
            () => reject(new Error("انتهت مهلة الاتصال بقاعدة البيانات. تحقق من الاتصال ثم أعد المحاولة.")),
            PROFILE_TIMEOUT_MS,
          );
        });
  ...
  const assignedLocation = useMemo(
    () =>
      emp?.locationId
        ? locations.find((loc) => loc.id === emp.locationId) || null
        : null,
    [emp?.locationId, locations],
  );
  ...
  const todays = useMemo(
    () =>
      attendance.filter(
        (r) => r.employeeId === emp?.id && r.timestamp.startsWith(todayKey()),
      ),
    [attendance, emp?.id],
  );
  ...
  if (checkIn) {
    ...
    const diff = Math.round(
      (new Date(checkIn.timestamp).getTime() - scheduled.getTime()) / 60000,
    );
  ...
  const handleSendRequest = async (e: React.FormEvent) => {
    ...
    try {
      if (!backendEnabled) throw new Error("خادم D1 غير مفعّل.");
      ...
      setProfileError(
        error instanceof Error ? error.message : "تعذر إرسال الطلب إلى D1.",
      );
    }
  };
  ...
  if (loadingProfile) {
    return (
      <div className="min-h-screen grid place-items-center px-5">
        <div className="hud-card w-full max-w-md p-7 text-center">
          <Brand />
          <div className="mt-5 font-bold">جاري تحميل بيانات الموظف من D1...</div>
          <div className="text-xs text-muted-foreground mt-2">
            يتم جلب ملف الموظف المرتبط بحسابك مباشرة من قاعدة البيانات.
          </div>
        </div>
      </div>
    );
  ...
  if (profileError || !emp) {
    return (
      <div className="min-h-screen grid place-items-center px-5">
        <div className="hud-card w-full max-w-md p-7 text-center">
          <Brand />
          <div className="mt-5 font-bold">تعذر تحميل بيانات الموظف</div>
          <div className="text-sm text-muted-foreground mt-2">
            {profileError || "لم يتم العثور على ملف الموظف في D1."}
          </div>
    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-xl mx-auto px-4 sm:px-5 pb-16 space-y-4 sm:space-y-5">
        <section className="hud-card p-5 sm:p-6">
          <div className="flex items-center justify-between gap-3 sm:gap-4">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                ...
                <div className="text-base sm:text-xl font-extrabold mt-0.5 truncate">{emp.name}</div>
                <div className="text-[11px] sm:text-xs text-muted-foreground mono mt-0.5">رقم وظيفي: {emp.jobNumber}</div>
              </div>
            </div>
            <div className="text-left shrink-0">
              <div className="mono text-xl sm:text-3xl font-extrabold tabular-nums">
                {now.toLocaleTimeString("ar-EG", { hour: "2-digit", minute: "2-digit", hour12: false })}
              </div>
              <div className="text-[10px] sm:text-xs text-muted-foreground mono">
                {now.toLocaleDateString("ar-EG", { weekday: "long", day: "2-digit", month: "2-digit" })}
              </div>
            </div>
          </div>
          ...
          {isWorkDay && lateMinutes > 0 && (
            <div className="mt-4 rounded-xl border border-[hsl(var(--warning))]/40 bg-[hsl(var(--warning))]/10 p-3 text-xs text-[hsl(var(--warning))] font-semibold">
              تم رصد تأخر {lateMinutes} دقيقة عن بداية الدوام ({workStartTime}).
            </div>
          )}
        </section>
        ...
        <section className="grid grid-cols-2 gap-3">
          <Link
            ...
            className={`hud-card p-4 sm:p-5 text-center transition ${!canCheckIn ? "opacity-40 pointer-events-none" : "hover:brightness-110"}`}
          >
            ...
            <div className="text-[11px] sm:text-xs text-muted-foreground mt-1 leading-relaxed">
              {!isWorkDay ? "أنت في يوم راحة (Off)" : checkIn ? "تم بالفعل اليوم" : "امسح رمز QR داخل المقر"}
            </div>
          </Link>
          {shiftEnded ? (
            <Link
              ...
              className={`hud-card p-4 sm:p-5 text-center transition ${!canCheckOut ? "opacity-40 pointer-events-none" : "hover:brightness-110"}`}
            >
        ...
        <section className="hud-card p-4 sm:p-5">
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-3 text-[11px] sm:text-xs">
            ...
            <Row label="فرع / موقع العمل" value={assignedLocation?.name ?? "المقر الرئيسي"} />
