...
export default function EmployeeHome() {
  ...
  useEffect(() => { if (!session) nav("/login", { replace: true }); }, [session, nav]);
  ...
  const assignedLocation = useMemo(() => emp?.locationId && settings.locations ? settings.locations.find((loc) => loc.id === emp.locationId) : null, [emp, settings]);
  const scheduleStatus = useMemo(() => getEmployeeScheduleStatus(emp, now), [emp, now]);
  const isWorkDay = scheduleStatus.isWorkDay;
  const todays = useMemo(() => getAttendance().filter((r) => r.employeeId === session.employeeId && r.timestamp.startsWith(todayKey())), [session, now]);
  ...
  const handleSendRequest = (e: React.FormEvent) => { e.preventDefault(); if (!emp) return; addRequest({ employeeId: emp.id, employeeName: emp.name, jobNumber: emp.jobNumber, type: requestType, reason }); setRequestSent(true); setTimeout(() => { setRequestSent(false); setShowRequestModal(false); setReason(""); }, 2000); };
  return (<div className="min-h-screen">
    ...
    <main className="max-w-xl mx-auto px-4 sm:px-5 pb-16 space-y-4 sm:space-y-5">
      <section className="hud-card p-5 sm:p-6"><div className="flex items-center justify-between gap-3 sm:gap-4"><div className="flex items-center gap-3 min-w-0 flex-1">{emp?.avatar ? <img src={emp.avatar} alt={session.name} className="h-14 w-14 sm:h-16 sm:w-16 rounded-full object-cover border-2 border-primary/40 shrink-0" /> : <div className="h-14 w-14 sm:h-16 sm:w-16 rounded-full bg-primary/15 grid place-items-center border-2 border-primary/30 shrink-0"><span className="text-primary font-extrabold text-lg sm:text-xl">{session.name ? session.name.charAt(0) : "م"}</span></div>}<div className="min-w-0"><div className="text-[10px] text-muted-foreground mono">EMPLOYEE</div><div className="text-base sm:text-xl font-extrabold mt-0.5 truncate">{session.name}</div><div className="text-[11px] sm:text-xs text-muted-foreground mono mt-0.5">رقم وظيفي: {session.jobNumber}</div></div></div><div className="text-left shrink-0"><div className="mono text-xl sm:text-3xl font-extrabold tabular-nums">{now.toLocaleTimeString("ar-EG", { hour: "2-digit", minute: "2-digit", hour12: false })}</div><div className="text-[10px] sm:text-xs text-muted-foreground mono">{now.toLocaleDateString("ar-EG", { weekday: "long", day: "2-digit", month: "2-digit" })}</div></div></div>
