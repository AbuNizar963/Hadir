...
export default function EmployeeHome() {
  ...
  useEffect(() => {
    ...
    const loadFromD1 = async () => {
      ...
      try {
        ...
        const [profile, attendanceRows, locationRows] = await Promise.all([
          getBackendEmployeeProfile(),
  ...
  const assignedLocation = useMemo(
    () => emp?.locationId ? locations.find((loc) => loc.id === emp.locationId) || null : null,
    [emp, locations]
  );
  ...
  const todays = useMemo(
    () => attendance.filter((r) => r.employeeId === emp?.id && r.timestamp.startsWith(todayKey())),
    [attendance, emp]
  );
  ...
  const workedMinutes = checkIn
    ? minutesBetween(checkIn.timestamp, checkOut?.timestamp ?? new Date().toISOString())
    : 0;
  ...
  const handleSendRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setProfileError(error instanceof Error ? error.message : "تعذر إرسال الطلب إلى D1.");
    }
  };
    ...
    return (
      <div className="min-h-screen grid place-items-center px-5">
        <div className="hud-card w-full max-w-md p-7 text-center">
          ...
          <div className="text-xs text-muted-foreground mt-2">يتم جلب ملف الموظف المرتبط بحسابك مباشرة من قاعدة البيانات.</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen grid place-items-center px-5">
        <div className="hud-card w-full max-w-md p-7 text-center">
          ...
          <div className="text-sm text-muted-foreground mt-2">{profileError || "لم يتم العثور على ملف الموظف في D1."}</div>
          <div className="flex gap-2 mt-5">
            <button onClick={() => window.location.reload()} className="btn-primary flex-1 py-2.5 rounded-xl">إعادة المحاولة</button>
            <button onClick={logout} className="btn-ghost flex-1 py-2.5 rounded-xl">تسجيل الخروج</button>
          </div>
        </div>
      </div>
    );
