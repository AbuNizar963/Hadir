...
import { getBackendAttendance, getBackendEscapeEvents } from "@/lib/backend";
...
export default function ManagerReports() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const [live, liveAttendance, escapes] = await Promise.all([
          fetch(`${API_URL}/api/workforce/live`, { headers, cache: "no-store" }).then(r => r.ok ? r.json() : Promise.reject(new Error("live"))),
          getBackendAttendance(2000),
          getBackendEscapeEvents(undefined, 2000),
        ]);
        if (cancelled) return;
        if (Array.isArray(live?.employees) && live.employees.length) setEmployees(live.employees as Employee[]);
  ...
  return <ManagerLayout title="التقارير" subtitle={periodLabel} actions={<div className="flex flex-wrap gap-2"><Button onClick={exportExcel}><FileSpreadsheet className="ml-2 h-4 w-4" />تصدير Excel ذكي</Button><Button onClick={exportCsv} variant="outline"><FileText className="ml-2 h-4 w-4" />CSV</Button><Button onClick={exportEscapeCsv} variant="outline"><ShieldAlert className="ml-2 h-4 w-4" />سجل الهروب</Button></div>}>
    <div className="hud-card p-4 mb-5 space-y-3"><div className="flex flex-wrap items-center gap-3"><div className="inline-flex bg-secondary/50 rounded-xl p-1 border border-border/50">{(["daily", "monthly", "annual"] as Mode[]).map(m => <button key={m} onClick={() => setMode(m)} className={`px-4 py-1.5 rounded-lg text-sm font-semibold ${mode === m ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>{m === "daily" ? "يومي" : m === "monthly" ? "شهري" : "سنوي"}</button>)}</div>{mode === "daily" ? <input aria-label="تاريخ التقرير" type="date" className="input max-w-[200px] mono" value={date} onChange={e => setDate(e.target.value)} /> : mode === "monthly" ? <input aria-label="شهر التقرير" type="month" className="input max-w-[200px] mono" value={month} onChange={e => setMonth(e.target.value)} /> : <input aria-label="سنة التقرير" type="number" min="2000" max="2100" className="input max-w-[140px] mono" value={year} onChange={e => setYear(e.target.value)} />}</div><div className="flex items-center gap-2 text-xs text-muted-foreground"><Wand2 className="h-4 w-4 text-primary" />التقرير يحسب الحالة من جدول الموظف نفسه: يوم عمل، راحة، غياب، حضور بدون انصراف أو فترة مكتملة. سجل الهروب والعودة محفوظ بشكل مستقل وقابل للتصدير.</div></div>
    <div className="hud-card overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="text-xs text-muted-foreground bg-secondary/40"><tr><Th>م</Th><Th>الموظف</Th><Th>الرقم</Th>{mode === "daily" ? <><Th>الحضور</Th><Th>الانصراف</Th><Th>التأخر</Th><Th>ساعات العمل</Th><Th>الحالة</Th><Th>الجدول</Th></> : <><Th>ملخص الجدول</Th><Th>التأخر</Th><Th>ساعات العمل</Th></>}</tr></thead><tbody>{rows.map((r, i) => <tr key={r.key} className="border-t border-border/50 hover:bg-secondary/20"><Td className="mono font-bold">{i + 1}</Td><Td className="font-semibold">{r.name}</Td><Td className="mono">{r.jobNumber}</Td>{mode === "daily" ? <><Td className="mono">{r.checkIn}</Td><Td className="mono">{r.checkOut}</Td><Td className="mono">{r.late} د</Td><Td className="mono">{formatDurationMinutes(r.worked)}</Td><Td><Status value={r.status} /></Td><Td className="text-xs text-muted-foreground">{r.detail}</Td></> : <><Td className="text-xs">{r.detail}</Td><Td className="mono">{r.late} د</Td><Td className="mono">{formatDurationMinutes(r.worked)}</Td></>}</tr>)}</tbody></table>{rows.length === 0 && <div className="p-10 text-center text-muted-foreground">لا توجد بيانات موظفين لعرض التقرير.</div>}</div></div>
    <section className="hud-card p-5 mt-5"><div className="flex items-center justify-between mb-4"><div><h2 className="font-black text-lg">بطاقات الموظفين الرقمية</h2><p className="text-xs text-muted-foreground mt-1">صورة الموظف، الاسم، الرقم، القسم، QR وحالة الحساب.</p></div><span className="text-xs text-muted-foreground">{employees.length} موظف</span></div><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{employees.map(e => <button key={e.id} onClick={() => setCardEmployee(e)} className="text-right rounded-2xl border border-border/60 p-4 hover:bg-secondary/30 transition"><div className="flex items-center gap-3"><div className="h-12 w-12 rounded-xl bg-secondary flex items-center justify-center overflow-hidden">{e.avatar ? <img src={e.avatar} alt="" className="h-full w-full object-cover" /> : <IdCard className="h-6 w-6 text-muted-foreground" />}</div><div className="min-w-0"><div className="font-bold truncate">{e.name}</div><div className="text-xs text-muted-foreground mono">{e.jobNumber}</div></div><QrCode className="mr-auto h-5 w-5 text-primary" /></div></button>)}</div></section>
