...
function EmployeeCardBase({ e, location, canManage, escapeStatus, onEdit, onRemove, onReset, onDirect, onCheckout, onWorkforceUpdate, isOwner, onEscape, onReturn }: {
  e: Employee;
  location?: Location;
  canManage: boolean;
      ...
      <div className="space-y-5">
        ...
        {showForm && (
          <section className={editingId ? "fixed inset-y-0 right-0 z-[70] w-full max-w-2xl overflow-y-auto rounded-none border-l border-border/80 bg-card p-5 shadow-2xl sm:p-6" : "hud-card p-5 sm:p-6"}>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              ...
              <Field label="رمز PIN" hint={editingId ? "اتركه فارغًا للإبقاء على الحالي." : "4 أحرف/أرقام على الأقل."}><input className="input mono w-full" type="password" placeholder={editingId ? "اختياري" : "أدخل PIN"} value={form.pin} onChange={(e) => setField("pin", e.target.value)} /></Field>
              <Field label="الحالة"><select className="input w-full" value={form.status} onChange={(e) => setField("status", e.target.value as FormState["status"])}><option value="active">فعال</option><option value="suspended">موقوف</option></select></Field>
            </div>
            <div className="mt-4 grid gap-4 md:grid-cols-3">
              <Field label="نوع الدوام"><select className="input w-full" value={form.scheduleType} onChange={(e) => setField("scheduleType", e.target.value as ScheduleType)}><option value="ADMIN">إداري</option><option value="ROTATION">تناوبي</option></select></Field>
              <Field label="بداية الدوام"><input className="input w-full" type="time" value={form.workStartTime} onChange={(e) => setField("workStartTime", e.target.value)} /></Field>
              <Field label="نهاية الدوام"><input className="input w-full" type="time" value={form.workEndTime} onChange={(e) => setField("workEndTime", e.target.value)} /></Field>
            </div>
            <div className="mt-4 grid gap-4 md:grid-cols-3">
            ...
            {form.scheduleType === "ADMIN" && (
              <div className="mt-4">
                <div className="mb-2 text-xs font-bold">أيام الدوام</div>
                <div className="flex flex-wrap gap-2">{days.map((d, i) => <button key={d} type="button" className={form.workDays.includes(i) ? "btn-primary text-xs" : "btn-secondary text-xs"} onClick={() => toggleDay(i)}>{d}</button>)}</div>
              </div>
            )}
            ...
            {form.scheduleType === "ROTATION" && (
              <div className="mt-4 grid gap-4 rounded-2xl border border-border/60 bg-background/20 p-4 md:grid-cols-3">
                <Field label="أيام العمل"><select className="input w-full" value={form.rotationDaysOn} onChange={(e) => setField("rotationDaysOn", Number(e.target.value))}>{rotationDays.map((d) => <option key={d} value={d}>{d} يوم</option>)}</select></Field>
                <Field label="أيام الراحة"><select className="input w-full" value={form.rotationDaysOff} onChange={(e) => setField("rotationDaysOff", Number(e.target.value))}><option value={0}>بدون راحة</option>{rotationDays.map((d) => <option key={d} value={d}>{d} يوم</option>)}</select></Field>
                <Field label="تاريخ أول مناوبة"><input className="input w-full" type="date" value={form.rotationStartDate} onChange={(e) => setField("rotationStartDate", e.target.value)} /></Field>
              </div>
            )}
        ...
        <section className="hud-card overflow-hidden border-primary/25 bg-primary/[0.025] p-5 sm:p-6">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-6">
            <div className="rounded-2xl border border-sky-400/25 bg-sky-500/[0.07] p-4 text-right shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
              <div className="flex items-center justify-between gap-2"><span className="text-xs font-bold text-sky-700 dark:text-sky-300">الإجمالي</span><span className="live-status-dot bg-sky-400" data-status="info" aria-hidden="true" /></div>
              <div className="mono mt-2 text-2xl font-black text-foreground dark:text-white">{stats.total}</div>
            </div>
            <div className="rounded-2xl border border-emerald-400/25 bg-emerald-500/[0.07] p-4 text-right shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
              <div className="flex items-center justify-between gap-2"><span className="text-xs font-bold text-emerald-700 dark:text-emerald-300">فعال</span><span className="live-status-dot bg-emerald-400" data-status="online" aria-hidden="true" /></div>
              <div className="mono mt-2 text-2xl font-black text-foreground dark:text-white">{stats.active}</div>
            </div>
            <div className="rounded-2xl border border-rose-400/25 bg-rose-500/[0.07] p-4 text-right shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
              <div className="flex items-center justify-between gap-2"><span className="text-xs font-bold text-rose-700 dark:text-rose-300">موقوف</span><span className="live-status-dot bg-rose-400" data-status="danger" aria-hidden="true" /></div>
              <div className="mono mt-2 text-2xl font-black text-foreground dark:text-white">{stats.suspended}</div>
            </div>
            <div className="rounded-2xl border border-violet-400/25 bg-violet-500/[0.07] p-4 text-right shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
              <div className="flex items-center justify-between gap-2"><span className="text-xs font-bold text-violet-700 dark:text-violet-300">أجهزة موثقة</span><span className="live-status-dot bg-violet-400" data-status="info" aria-hidden="true" /></div>
              <div className="mono mt-2 text-2xl font-black text-foreground dark:text-white">{stats.devices}</div>
            </div>
            <div className="rounded-2xl border border-amber-400/25 bg-amber-500/[0.07] p-4 text-right shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
              <div className="flex items-center justify-between gap-2"><span className="text-xs font-bold text-amber-700 dark:text-amber-300">VIP مفعل</span><span className="live-status-dot bg-amber-400" data-status="vip" aria-hidden="true" /></div>
              <div className="mono mt-2 text-2xl font-black text-foreground dark:text-white">{stats.vip}</div>
            </div>
            <div className="rounded-2xl border border-red-400/25 bg-red-500/[0.07] p-4 text-right shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
              <div className="flex items-center justify-between gap-2"><span className="text-xs font-bold text-red-700 dark:text-red-300">هارب الآن</span><span className="live-status-dot bg-red-400" data-status="danger" aria-hidden="true" /></div>
              <div className="mono mt-2 text-2xl font-black text-foreground dark:text-white">{stats.escaped}</div>
            </div>
          </div>
        </section>
        <section className="hud-card overflow-hidden p-5 sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><div className="mono text-xs font-bold text-primary">LIVE DIRECTORY</div><h2 className="mt-1 text-lg font-black">قائمة الموظفين <span className="font-normal text-muted-foreground">({filtered.length})</span></h2><p className="mt-1 text-xs text-muted-foreground">{role === "supervisor" ? "المشرف: مشاهدة، إضافة، وفك ربط الهاتف فقط." : "المالك والمدير: إدارة الموظفين والتحضير المباشر للمهمات والمأموريات وتسجيل الهروب والعودة."}</p></div><div className="grid w-full gap-2 sm:grid-cols-[1fr_auto_auto] lg:w-auto"><input className="input lg:w-72" placeholder="بحث بالاسم أو الرقم أو الجهاز" value={query} onChange={(e) => setQuery(e.target.value)} /><select className="input" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as typeof statusFilter)}><option value="all">كل الحالات</option><option value="active">فعال</option><option value="suspended">موقوف</option></select><select className="input" value={scheduleFilter} onChange={(e) => setScheduleFilter(e.target.value as typeof scheduleFilter)}><option value="all">كل أنواع الدوام</option><option value="ADMIN">إداري</option><option value="ROTATION">تناوبي</option></select></div></div>
          {loading ? <div className="py-16 text-center text-sm text-muted-foreground">جاري مزامنة الموظفين من قاعدة بيانات النظام…</div> : error && !showForm ? <div className="mt-5 rounded-xl border border-destructive/40 bg-destructive/10 p-4 text-sm text-destructive">{error}<button type="button" className="mr-3 underline" onClick={() => void load(true)}>إعادة المحاولة</button></div> : filtered.length === 0 ? <div className="mt-5 rounded-2xl border border-dashed border-border/70 py-16 text-center"><div className="text-3xl">⌕</div><p className="mt-2 text-sm font-bold">لا توجد نتائج</p><p className="mt-1 text-xs text-muted-foreground">جرّب تغيير البحث أو الفلاتر.</p></div> : <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{filtered.map((e) => <EmployeeCard key={e.id} e={e} location={locations.find((l) => String(l.id) === String(e.locationId)) || locations.find((l) => l.id === "main")} canManage={canManage} isOwner={isOwner} onCheckout={directCheckout} onWorkforceUpdate={updateWorkforce} escapeStatus={escapeStatusFor(e.id)} onEdit={edit} onRemove={remove} onReset={resetDevice} onDirect={direct} onEscape={(x) => void changeEscape(x, "escaped")} onReturn={(x) => void changeEscape(x, "returned")} />)}</div>}
        </section>
      </div>
    </ManagerLayout>
  );
