...
import { backendEnabled, createBackendEmployee, createBackendEscapeEvent, deleteBackendEmployee, getBackendEmployees, getBackendEscapeEvents, getBackendLocations, getBackendSettings, resetBackendEmployeeDevice, updateBackendEmployee, } from "@/lib/backend";
import { getEmployees, saveEmployees, getSettings } from "@/lib/storage";
...
function CompanySpecialtySelect({ value, onChange }: {
    value: string;
    ...
    useEffect(() => {
        ...
        getBackendSettings().then(remote => {
            ...
            if (alive && remoteItems.length)
                setOptions(Array.from(new Set(remoteItems)));
        }).catch(() => undefined);
        return () => { alive = false; };
    }, []);
...
function EmployeeCardBase({ e, location, canManage, escapeStatus, onEdit, onRemove, onReset, onDirect, onCheckout, onWorkforceUpdate, isOwner, onEscape, onReturn, onRequest }: {
    ...
    onWorkforceUpdate: (e: Employee, patch: {
        isVip?: boolean;
        autoCheckIn?: boolean;
    ...
    return (<article className="rounded-2xl border border-border/70 bg-card/60 p-3 shadow-sm transition hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lg">
...
export default function ManagerEmployees() {
        ...
        try {
            ...
            if (backendEnabled) {
                try {
                    ...
                    if (response.ok && Array.isArray(controls)) {
                        ...
                        next = next.map((employee) => { const control = byId.get(String(employee.id)); if (!control)
                            return employee; return { ...employee, isVip: Boolean(control.isVip), autoCheckIn: Boolean(control.autoCheckIn), autoCheckOut: Boolean(control.autoCheckOut) }; });
                    }
            ...
            if (backendEnabled) {
                ...
                try {
                    ...
                    setEscapeEvents((prev) => JSON.stringify(prev) === JSON.stringify(nextEscapes) ? prev : nextEscapes);
                }
                catch { }
            }
            setError(null);
            ...
            setError(err instanceof Error ? err.message : "تعذر تحميل الموظفين من النظام.");
        }
        finally {
            if (showSpinner)
                setLoading(false);
    ...
    const changeEscape = async (e: Employee, status: "escaped" | "returned") => {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : `تعذر ${verb}.`;
            setError(message);
            toast.error(verb, message);
        }
    };
    ...
    const createAdminEmployeeRequest = async () => {
        if (!requestDialog || !canManage || requestDialog.employee.status !== "active")
            return;
        const label = requestDialog.type === "permission" ? "استئذان" : "إجازة";
        if (!requestStartDate || !requestEndDate) {
            const message = `حدد تاريخ بداية ونهاية ${label}.`;
            setError(message);
            toast.warning(message);
            return;
        }
        if (requestEndDate < requestStartDate) {
            const message = "تاريخ النهاية يجب أن يكون مساويًا أو بعد تاريخ البداية.";
            setError(message);
            toast.warning(message);
            return;
        }
        ...
        try {
            ...
            if (!response.ok)
                throw new Error(typeof data.error === "string" ? data.error : `تعذر تسجيل ${label}.`);
            ...
            const message = err instanceof Error ? err.message : `تعذر تسجيل ${label}.`;
            setError(message);
            toast.error(`تعذر تسجيل ${label}`, message);
        }
        finally {
    ...
    const submit = async () => {
        ...
        if (!name || !jobNumber) {
            const message = "اسم الموظف والرقم الوظيفي مطلوبان.";
            setError(message);
            toast.warning(message);
            return;
        }
        if (!editingId && pin.length < 4) {
            const message = "رمز PIN يجب أن يتكون من 4 أحرف/أرقام على الأقل.";
            setError(message);
            toast.warning(message);
            return;
        }
        if (form.scheduleType === "ADMIN" && !form.workDays.length) {
            const message = "اختر يوم دوام واحدًا على الأقل.";
            setError(message);
            toast.warning(message);
            return;
        }
        ...
        try {
            ...
            if (savedEmployee)
                setEmployees((prev) => editingId ? prev.map((item) => item.id === savedEmployee!.id ? savedEmployee! : item) : [savedEmployee!, ...prev.filter((item) => item.id !== savedEmployee!.id)]);
            setForm(emptyForm);
            setEditingId(null);
            setShowForm(false);
            void load(false);
            ...
            const message = err instanceof Error ? err.message : "تعذر حفظ الموظف.";
            setError(message);
            toast.error(editingId ? "تعذر تحديث الموظف" : "تعذر إضافة الموظف", message);
        }
        finally {
            setSaving(false);
        }
    ...
    const resetDevice = async (e: Employee) => {
        if (!confirm(`إلغاء ربط جهاز «${e.name}»؟`))
            return;
        try {
            if (backendEnabled)
                await resetBackendEmployeeDevice(e.id);
            else
                saveEmployees(employees.map((x) => x.id === e.id ? { ...x, deviceId: null, deviceLabel: null } : x));
            await load(false);
            toast.success("تم إلغاء ربط جهاز الموظف");
        }
        catch (err) {
            const message = err instanceof Error ? err.message : "تعذر إلغاء ربط الجهاز.";
            setError(message);
            toast.error("تعذر إلغاء ربط الجهاز", message);
        }
    };
    const directCheckout = async (e: Employee) => {
        if (e.status !== "active" || !isOwner)
            return;
        if (!confirm(`تسجيل انصراف مباشر للموظف «${e.name}»؟`))
            return;
        try {
            const token = localStorage.getItem("hadir.api.token.admin") || "";
            const api = String(import.meta.env.VITE_API_URL || "https://hadir-api.abunizar963.workers.dev").replace(/\/$/, "");
            ...
            if (!res.ok)
                throw new Error(typeof data.error === "string" ? data.error : "تعذر تسجيل الانصراف المباشر.");
            await load(false);
            toast.success("تم تسجيل الانصراف المباشر");
        }
        catch (err) {
            const message = err instanceof Error ? err.message : "تعذر تسجيل الانصراف المباشر.";
            setError(message);
            toast.error("تعذر تسجيل الانصراف المباشر", message);
        }
    };
    const updateWorkforce = async (e: Employee, patch: {
        isVip?: boolean;
        autoCheckIn?: boolean;
        autoCheckOut?: boolean;
    }) => {
        ...
        try {
            ...
            const data = await res.json().catch(() => ({}));
            if (!res.ok)
                throw new Error(typeof data.error === "string" ? data.error : "تعذر حفظ إعدادات Workforce.");
            await load(false);
            toast.success("تم حفظ إعدادات Workforce");
        }
        catch (err) {
            const message = err instanceof Error ? err.message : "تعذر حفظ إعدادات Workforce.";
            setError(message);
            toast.error("تعذر حفظ إعدادات Workforce", message);
            await load(false);
        }
    };
    const direct = async (e: Employee) => {
        if (e.status !== "active" || !isOwner)
            return;
        if (!confirm(`تسجيل حضور مباشر للموظف «${e.name}» كمأمورية/مهمة؟`))
            return;
        try {
            const token = localStorage.getItem("hadir.api.token.admin") || "";
            const api = String(import.meta.env.VITE_API_URL || "https://hadir-api.abunizar963.workers.dev").replace(/\/$/, "");
            ...
            const data = await res.json().catch(() => ({}));
            if (!res.ok)
                throw new Error(typeof data.error === "string" ? data.error : "تعذر تسجيل الحضور المباشر.");
            await load(false);
            toast.success("تم تسجيل الحضور المباشر");
        }
        catch (err) {
            const message = err instanceof Error ? err.message : "تعذر تسجيل الحضور المباشر.";
            setError(message);
            toast.error("تعذر تسجيل الحضور المباشر", message);
        }
    };
    ...
    const escapedIds = new Set(employees
    ...
    return (<ManagerLayout title="الموظفون" subtitle="دليل الموظفين الموحد — البيانات من النظام مباشرة.">
      <div className="space-y-5">
        ...
        {requestDialog && (<div className="fixed inset-0 z-[90] grid place-items-center bg-black/45 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="request-dialog-title">
