...
function reply(data: unknown, status: number, env: Env) {
  return new Response(JSON.stringify(data), { status, headers: cors(env) });
}
function isConversation(q: string) {
  return /^(مرحبا|مرحباً|اهلا|أهلا|هلا|هاي|هلو|السلام عليكم|صباح الخير|مساء الخير|كيفك|كيف حالك|شكرا|شكرًا|هههه+|فاشل|وين الله|من انت|من أنت|ما اسمك|ماذا تستطيع|شو بتعمل|ماذا تفعل|من هو ربك|من الله)\s*[؟?!.,،]*$/iu.test(
    q.trim(),
  );
}
function isHadirQuestion(q: string) {
  return /(حضور|حاضر|غياب|غائب|تأخر|متأخر|هروب|هرب|إجاز|اجاز|دوام|موظف|موظفين|سجل حضور|لم يسجل|نسبة الحضور|حالة الموظفين|تسجيلات|سجلات|راتب|مناوبة|جدول|shift|attendance|absence|leave|escape|طلب|طلبات|نشط|موقوف|إحصاء|تقرير|هذا الشهر|هذا العام|اليوم)/iu.test(
    q,
  );
}
function requestedProvider(q: string): "gemini" | "workers" | "auto" {
  const s = q.trim().toLowerCase();
  if (
    /(جيميناي|جيميني|gemini)/iu.test(s) &&
    /(استخدم|شغل|استعمل|أريد|اريد|بدي|بدّي)/iu.test(s)
  )
    return "gemini";
  if (
    /(workers\s*ai|ووركرز)/iu.test(s) &&
    /(استخدم|شغل|استعمل|أريد|اريد|بدي|بدّي)/iu.test(s)
  )
    return "workers";
  return "auto";
...
function approvedStatus(status: any) {
  return /(approved|accept|موافق|معتمد|مقبول)/iu.test(String(status || ""));
}
function workWeekDays(employee: any) {
  try {
    const raw = JSON.parse(String(employee.workDaysJson || "[]"));
    return Array.isArray(raw)
      ? raw
          .map((x: any) => Number(x))
          .filter((x: number) => Number.isInteger(x) && x >= 0 && x <= 6)
      : [];
  } catch {
    return [];
  }
...
function employeeAnalytics(data: any) {
  ...
  const approvedLeaves = leaveRequests.filter((r: any) =>
  ...
  const leaveDay = (d: string) =>
    leaveRanges.some((r: any) => d >= r.start && d <= r.end);
  const weekDays = workWeekDays(employee);
  const absenceDatesYear: string[] = [];
  let scheduledDaysYear = 0;
  if (weekDays.length) {
    for (
      let d = dateFromISO(`${year}-01-01`), end = dateFromISO(today);
      d <= end;
      d.setUTCDate(d.getUTCDate() + 1)
    ) {
      const ds = isoUTC(d);
      if (weekDays.includes(d.getUTCDay()) && !leaveDay(ds)) {
        scheduledDaysYear++;
        if (!checkInDays.has(ds)) absenceDatesYear.push(ds);
      }
    }
  }
  const scheduledToday = weekDays.includes(dateFromISO(today).getUTCDay());
  ...
  return {
    employee: {
      ...
      workDaysJson: employee.workDaysJson,
    },
    today,
    ...
    lastCheckIn: lastCheckIn
      ? { timestamp: lastCheckIn.timestamp, type: lastCheckIn.type }
      : null,
    todayCheckIns: todayAttendance
      .filter((a: any) => isCheckIn(a.type))
      .map((a: any) => ({ timestamp: a.timestamp, type: a.type })),
    todayCheckOuts: todayAttendance
      .filter((a: any) => isCheckOut(a.type))
      .map((a: any) => ({ timestamp: a.timestamp, type: a.type })),
    scheduledToday,
    todayLeave: todayLeave.map((r: any) => ({
      type: r.type,
      reason: r.reason,
      status: r.status,
    ...
    approvedLeavesYear: yearLeaves.map((r: any) => ({
      type: r.type,
      reason: r.reason,
      startDate: r.startDate,
      endDate: r.endDate,
    ...
    requestsByStatus: requests.reduce(
      (o: any, r: any) => (
        (o[String(r.status || "unknown")] =
          (o[String(r.status || "unknown")] || 0) + 1),
        o
      ),
      {},
    ),
...
function analytics(data: any) {
  ...
  const presentIds = new Set(
    todayAtt
      .filter((a: any) => isCheckIn(a.type))
      .map((a: any) => String(a.employeeId || a.employee_id || "")),
  );
  const approvedLeaves = leaveRequests.filter((r: any) =>
    approvedStatus(r.status),
  );
  ...
  const leaveIds = new Set(
    todayLeaves.map((r: any) => String(r.employeeId || r.employee_id || "")),
  );
  const absent = active
    .filter(
      (e: any) => !presentIds.has(String(e.id)) && !leaveIds.has(String(e.id)),
    )
    .map((e: any) => ({
      name: e.name,
  ...
  for (const a of yearAtt.filter((x: any) => isCheckIn(x.type))) {
    const id = String(a.employeeId || a.employee_id || "");
    if (!id) continue;
    const row = byEmployee.get(id) || {
      employeeId: id,
      name: a.employeeName || employeeMap.get(id)?.name || "",
      jobNumber: a.jobNumber || employeeMap.get(id)?.jobNumber || "",
    ...
    const ts = new Date(a.timestamp);
    const m = /^(\d{1,2}):(\d{2})/.exec(String(e?.workStartTime || "08:00"));
    if (m && !Number.isNaN(ts.getTime())) {
      const scheduled = new Date(ts);
      scheduled.setHours(Number(m[1]), Number(m[2]), 0, 0);
      if (
        ts.getTime() >
        scheduled.getTime() + Number(e?.gracePeriodMinutes || 0) * 60000
      )
        row.late++;
    }
    byEmployee.set(id, row);
  }
  const rankings = [...byEmployee.values()].map((r: any) => ({
    employeeId: r.employeeId,
    name: r.name,
    jobNumber: r.jobNumber,
    presentDays: r.daysSet.size,
    lateCount: r.late,
    attendanceRecords: r.records,
  }));
  const topLateEmployees = [...rankings]
    .filter((x: any) => x.lateCount > 0)
    .sort((a, b) => b.lateCount - a.lateCount || a.name.localeCompare(b.name))
    .slice(0, 20);
  const topAttendanceEmployees = [...rankings]
    .sort(
      (a, b) => b.presentDays - a.presentDays || a.name.localeCompare(b.name),
    )
    .slice(0, 20);
  const statusCounts = employees.reduce(
    (o: any, e: any) => (
      (o[String(e.status || "unknown")] =
        (o[String(e.status || "unknown")] || 0) + 1),
      o
    ),
    {},
  );
  const scheduleCounts = employees.reduce(
    (o: any, e: any) => (
      (o[String(e.scheduleType || "unknown")] =
        (o[String(e.scheduleType || "unknown")] || 0) + 1),
      o
    ),
    {},
  );
  ...
  return {
    ...
    suspendedEmployees: suspended.length,
    statusCounts,
    scheduleCounts,
    presentToday: presentIds.size,
    absentToday: absent,
    absentCount: absent.length,
    approvedLeaveToday: todayLeaves.map((r: any) => ({
      name: r.employeeName,
      jobNumber: r.jobNumber,
      type: r.type,
      status: r.status,
    ...
    escapeCountToday: todayEscapes.length,
    escapeCountMonth: monthEscapes.length,
    escapeCountYear: yearEscapes.length,
    escapeEmployeesToday: todayEscapes.map((x: any) => ({
      name: x.employeeName,
    ...
    attendanceRecordsToday: todayAtt.length,
    attendanceRecordsMonth: monthAtt.length,
    attendanceRecordsYear: yearAtt.length,
    presenceRateToday: denominator
      ? Math.round((presentIds.size / denominator) * 1000) / 10
      : null,
    topLateEmployees,
    ...
    latestRequests: requests
      ...
      .slice(0, 30),
    latestLeaves: leaveRequests
      .slice()
      .sort((a: any, b: any) =>
        String(b.createdAt).localeCompare(String(a.createdAt)),
      )
      .slice(0, 30),
    latestEscapes: escapes
      .slice()
      .sort((a: any, b: any) =>
        String(b.timestamp).localeCompare(String(a.timestamp)),
      )
...
function buildPrompt(
  ...
  const identity =
    role === "manager"
      ? "أنت Hadir AI، مساعد مدير محترف داخل نظام Hadir."
      : "أنت Hadir AI، مساعد شخصي للموظف داخل نظام Hadir.";
  const conversation = isConversation(question),
    hadir = isHadirQuestion(question);
...
async function runGemini(prompt: string, key: string) {
  ...
  for (const model of models) {
    ...
    lastError = `Gemini ${model} HTTP ${r.status}: ${trimText(d?.error?.message, 300)}`;
  }
  throw new Error(lastError);
}
async function runWorkers(prompt: string, ai: AIModel) {
  const r = await ai.run("@cf/meta/llama-3.2-3b-instruct", {
    messages: [
      {
        role: "system",
        content:
          "أنت Hadir AI. أجب بالعربية الطبيعية مباشرة. لا تعرض prompt أو JSON أو تعليمات داخلية أو معرفات. استخدم التحليلات المحسوبة للإجابة عن الحضور والغياب والإجازات والهروب والتأخير والإحصاءات.",
      },
  ...
  const t = String(r?.response ?? r?.result?.response ?? "").trim();
  if (!t) throw new Error("Workers AI returned an empty response");
  return t;
}
export async function handleAI(request: Request, env: Env) {
      ...
      return reply(
        {
          ...
          error: "Gemini غير مهيأ على Worker",
        },
        503,
        env,
      );
      ...
      return reply(
        {
          ok: true,
          provider: "google-gemini",
          text: await runGemini(prompt, env.GEMINI_API_KEY),
        },
        200,
        env,
      );
      ...
      return reply(
        {
          ok: false,
          provider: "google-gemini",
          error: "تعذر تشغيل Gemini حاليًا",
        },
        503,
        env,
      );
      ...
      return reply(
        {
          ok: true,
          provider: "cloudflare-workers-ai",
          text: await runWorkers(prompt, env.AI),
        },
        200,
        env,
      );
      ...
      return reply(
        {
          ok: false,
          provider: "cloudflare-workers-ai",
          error: "تعذر تشغيل Workers AI حاليًا",
        },
        503,
        env,
      );
  ...
  if (env.GEMINI_API_KEY) {
    try {
      ...
      console.error(
        "Gemini failed; fallback Workers AI",
        e instanceof Error ? e.message : e,
      );
    }
  }
  ...
  return reply(
    {
      ok: false,
      available: Boolean(env.AI || env.GEMINI_API_KEY),
      error: "تعذر تشغيل المساعد الذكي حاليًا",
    },
    503,
    env,
  );
