  ...
  try {
    ...
    const leaveIds = new Set(
      requests
        .filter(
          (r: any) =>
            String(r.type).toLowerCase() === "leave" && requestActive(r),
        )
    ...
    const permissionIds = new Set(
      requests
        .filter(
          (r: any) =>
            String(r.type).toLowerCase() === "permission" && requestActive(r),
        )
        .map((r: any) => String(r.employeeId)),
    );
    const maxRotationOn = Math.max(
      1,
      ...employees.map((e: any) =>
        String(e.scheduleType || "")
          .trim()
          .toUpperCase() === "ROTATION"
          ? Math.max(1, Math.floor(Number(e.rotationDaysOn ?? 4)))
          : 1,
      ),
    );
    ...
    const result = scopedEmployees.map((employee) => {
      ...
      if (isRotation) {
        ...
        const scopedRows =
          schedule.work && schedule.start && schedule.end
            ...
            : [];
        checkIn =
          checkInRows.find((r) => String(r.type) === "check-in") || null;
        checkOut =
          [...scopedRows]
            .reverse()
            .find((r: any) => String(r.type) === "check-out") || null;
      }
      const isScheduledVip =
        Number(employee.isVip) === 1 &&
        Number(employee.autoCheckIn) === 1 &&
        schedule.work &&
        !leaveIds.has(id) &&
      ...
      const lateMinutes = dailyRotationAttendance
        ...
        : status === "LATE" && schedule.start && centralCheckInAt
          ? Math.max(
              0,
              minutesBetween(schedule.start.toISOString(), centralCheckInAt) ||
                0,
            )
          : 0;
      ...
      return {
        ...
        jobNumber: String(employee.jobNumber || ""),
        status,
        ...
        rotationDailyAttendanceTime: dailyRotationAttendance
          ? employee.rotationDailyAttendanceTime || null
          : null,
        workedMinutes,
        expectedMinutes,
        lateMinutes,
    ...
    if (persist && result.length) {
      const storedQuery = await env.DB.prepare(
        "SELECT employee_id AS employeeId,status,check_in_at AS checkInAt,check_out_at AS checkOutAt,schedule_type AS scheduleType FROM daily_attendance_status WHERE attendance_day=? AND employee_id IN (SELECT id FROM employees)",
      )
        .bind(day)
        .all<DailyStatusStoredRow>();
      const stored = new Map(
        (storedQuery.results || []).map((row) => [String(row.employeeId), row]),
      );
      const changed = result.filter((row) => {
        const previous = stored.get(row.employeeId);
        return (
          !previous ||
          String(previous.status || "") !== row.status ||
          String(previous.checkInAt || "") !== String(row.checkInAt || "") ||
          String(previous.checkOutAt || "") !== String(row.checkOutAt || "") ||
