...
async function archiveActor(request: Request, env: Env) {
  const token = (
  ...
  try {
    ...
    return await env.DB.prepare(
      "SELECT id,name,role FROM admin_accounts WHERE id=? AND active=1 LIMIT 1",
    )
      .bind(s.userId)
      .first<any>();
  } catch {
...
function json(data: unknown, status: number, o: string) {
...
function archiveAllowed(a: any) {
  return (
    !!a &&
    ["owner", "manager", "supervisor"].includes(String(a.role).toLowerCase())
  );
}
function archiveDeleteAllowed(a: any) {
  return !!a && String(a.role).toLowerCase() === "owner";
}
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext) {
      ...
      return new Response(null, {
        ...
        headers: {
          ...
          "access-control-allow-headers":
            "authorization,content-type,x-device-id,x-requested-with",
          "cache-control": "no-store",
        },
      });
    if (
      ...
      return json(
        {
          ok: true,
          readOnly: true,
          archives: await listReportArchives(env, limit),
        },
        200,
        o,
      );
    ...
    if (match && (request.method === "GET" || request.method === "DELETE")) {
      ...
      if (request.method === "DELETE") {
        if (!archiveDeleteAllowed(a))
          return json({ error: "حذف أرشيف التقارير متاح للمالك فقط" }, 403, o);
        if (!env.REPORT_ARCHIVES)
          return json(
            { error: "R2 binding REPORT_ARCHIVES غير موجود" },
            503,
        ...
        try {
          const result = await deleteReportArchive(env, id);
          if (!result.ok)
            return json(
              { error: "الأرشيف غير موجود أو تم حذفه مسبقًا" },
              404,
              o,
            );
          return json({ ok: true, deleted: true, reportId: id }, 200, o);
        } catch (error) {
          console.error("[report-archive] delete failed", error);
          return json(
            {
              error:
                error instanceof Error ? error.message : "تعذر حذف الأرشيف",
            },
            500,
            o,
          );
      ...
      const meta = await getReportArchive(env, id);
      if (!meta) return json({ error: "الأرشيف غير موجود" }, 404, o);
      const object = await env.REPORT_ARCHIVES.get(meta.file_key);
      if (!object)
        return json({ error: "ملف الأرشيف غير موجود في R2" }, 404, o);
      return new Response(object.body, {
        status: 200,
        headers: {
          "content-type": meta.mime_type,
          "content-length": String(meta.file_size),
          "content-disposition": `attachment; filename="${meta.file_name.replace(/[^A-Za-z0-9._-]/g, "_")}"`,
