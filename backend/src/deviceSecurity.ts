...
export async function bindEmployeeDevice(
  ...
  const employee = await env.DB.prepare("SELECT device_id AS deviceId, device_fingerprint AS deviceFingerprint FROM employees WHERE id=? LIMIT 1")
  ...
  if (!hasBinding) {
    await env.DB.prepare("UPDATE employees SET device_id=?, device_label=?, device_fingerprint=?, device_bound_at=?, device_last_seen_at=? WHERE id=?")
      .bind(deviceId, deviceLabel.slice(0, 120), fingerprintHash, now, now, employeeId).run();
    return { bound: true, firstBind: true };
  }

  const idMatches = Boolean(employee.deviceId && employee.deviceId === deviceId);
  ...
  await env.DB.prepare("UPDATE employees SET device_last_seen_at=?, device_label=COALESCE(?,device_label) WHERE id=?")
...
export async function deviceStatus(env: DeviceSecurityEnv, employeeId: string) {
  const row = await env.DB.prepare("SELECT device_id AS deviceId, device_label AS deviceLabel, device_bound_at AS deviceBoundAt FROM employees WHERE id=? LIMIT 1")
  ...
  return { bound: Boolean(row?.deviceId), deviceLabel: row?.deviceLabel || null, deviceBoundAt: row?.deviceBoundAt || null, passkeyCount: Number(passkey?.count || 0) };
}
...
async function saveChallenge(env: DeviceSecurityEnv, employeeId: string, kind: "registration" | "authentication", challenge: string) {
  ...
  await env.DB.prepare("INSERT INTO webauthn_challenges(id,employee_id,kind,challenge,expires_at,created_at) VALUES(?,?,?,?,?,?)")
...
async function takeChallenge(env: DeviceSecurityEnv, employeeId: string, kind: "registration" | "authentication") {
  const row = await env.DB.prepare("SELECT id,challenge,expires_at AS expiresAt FROM webauthn_challenges WHERE employee_id=? AND kind=? ORDER BY created_at DESC LIMIT 1")
...
export async function registrationOptions(env: DeviceSecurityEnv, employeeId: string, jobNumber: string, name: string): Promise<PublicKeyCredentialCreationOptionsJSON> {
  ...
  const options = await generateRegistrationOptions({
    ...
    excludeCredentials: (credentials.results || []).map((row: any) => ({ id: String(row.id), transports: row.transports ? JSON.parse(row.transports) : undefined })),
...
export async function verifyRegistration(env: DeviceSecurityEnv, employeeId: string, response: any) {
  ...
  await env.DB.prepare("INSERT INTO employee_passkeys(id,employee_id,credential_id,public_key,counter,transports,device_type,backed_up,created_at) VALUES(?,?,?,?,?,?,?,?,?)")
    .bind(crypto.randomUUID(), employeeId, credential.id, credential.publicKey, credential.counter, transports, credentialDeviceType, credentialBackedUp ? 1 : 0, new Date().toISOString()).run();
