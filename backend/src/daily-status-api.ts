...
export async function handleDailyStatus(req:Request,env:Env,actor:any){
  ...
  try{
    ...
    const result=employees.map(employee=>{
      ...
      let status:Status;if(leaveIds.has(id))status="LEAVE";else if(permissionIds.has(id))status="PERMISSION";else if(schedule.status==="REST")status="REST";else if(schedule.status==="NOT_STARTED")status="NOT_STARTED";else if(schedule.status==="INVALID")status="INVALID";else if(checkIn){const grace=Number.isFinite(Number(employee.gracePeriodMinutes))?Math.max(0,Number(employee.gracePeriodMinutes)):10;status=isRotation?"PRESENT":(schedule.start&&Date.parse(String(checkIn.timestamp))>schedule.start.getTime()+grace*60000?"LATE":"PRESENT");}else if(day===today&&schedule.start&&now.getTime()<schedule.start.getTime())status="NOT_STARTED";else if(schedule.work)status="ABSENT";else status="REST";
