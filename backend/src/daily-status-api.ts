...
export async function handleDailyStatus(req:Request,env:Env,actor:any){
  ...
  try{
    ...
    const result=employees.map(employee=>{
      ...
      const isScheduledVip=Number(employee.isVip)===1&&schedule.work&&leaveIds.has(id)===false&&permissionIds.has(id)===false;
      ...
      const vipCheckOut= isScheduledVip && !checkOut && schedule.end && (day<today || (day===today && now.getTime()>=schedule.end.getTime())) ? schedule.end.toISOString() : checkOut?.timestamp||null;
