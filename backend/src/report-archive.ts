...
function makeWorkbook(report: any) {
  ...
  for (const [name, rows] of [
    ["الملخص", summaryRows],
    ["اليومي", dailyRows],
    ["الموظفون", employeeRows],
    ["التفاصيل", detailRows],
    ["الاستثناءات", exceptionRows],
  ] as Array<[string, any[][]]>) {
    const ws = XLSX.utils.aoa_to_sheet(rows);
    ws["!cols"] = Array.from(
      { length: Math.max(...rows.map((r) => r.length), 1) },
      (_, c) => ({
        wch: Math.min(
          42,
...
export async function deleteReportArchive(env: Env, id: string) {
  ...
  await env.DB.prepare(
    "UPDATE report_archives SET status='DELETED' WHERE report_id=? AND status='LOCKED'",
  )
    .bind(id)
    .run();
