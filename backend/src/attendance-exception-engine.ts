...
function shiftForException(employee: any, now: Date, tz: string) {
  ...
  const candidates = [0, 1, 2, 3].map(offset => getAttendanceShiftForDay(employee, addDays(today, -offset), tz));
  return candidates.filter(shift => shift.start.getTime() <= now.getTime() && now.getTime() <= shift.end.getTime() + SAFETY_AFTER_SHIFT_MS).sort((a, b) => b.start.getTime() - a.start.getTime())[0] || candidates[0];
}
async function notifyManagers(db: D1Database, employeeName: string, jobNumber: string, code: string) {
  ...
  const body = code === "MISSING_CLOCK_IN" ? `الموظف ${employeeName} (${jobNumber}) لديه تسجيل خروج بدون تسجيل دخول. يلزم طلب تسوية.` : code === "MISSING_CLOCK_OUT" ? `الموظف ${employeeName} (${jobNumber}) لديه حضور مفتوح تجاوز فترة الأمان. ساعات الرواتب = 0 حتى التسوية.` : `الموظف ${employeeName} (${jobNumber}) لم يسجل أي حركة خلال فترة المناوبة. تم تسجيل غياب.`;
  ...
  for (const manager of (managers.results || []) as any[]) await db.prepare("INSERT INTO notifications(id,recipient_id,title,body,severity,type,created_at) VALUES(?,?,?,?,?,?,?)").bind(id(), String(manager.id), title, body, "warning", "attendance-exception", nowIso()).run().catch(() => undefined);
}
export async function runAttendanceExceptionEngine(env: Env) {
  const tz = String(env.APP_TIMEZONE || TZ), now = new Date(), nowMs = now.getTime();
  await env.DB.batch([
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS attendance_exceptions (id TEXT PRIMARY KEY, employee_id TEXT NOT NULL, attendance_day TEXT NOT NULL, shift_start TEXT NOT NULL, shift_end TEXT NOT NULL, schedule_type TEXT NOT NULL, exception_code TEXT NOT NULL, state TEXT NOT NULL DEFAULT 'OPEN', first_event_at TEXT, last_event_at TEXT, raw_worked_minutes INTEGER NOT NULL DEFAULT 0, payroll_approved_minutes INTEGER NOT NULL DEFAULT 0, safety_cutoff_at TEXT NOT NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, UNIQUE(employee_id, shift_start, exception_code))`),
  ...
  const employees = await env.DB.prepare(`SELECT id,name,job_number AS jobNumber,status,schedule_type AS scheduleType,work_start_time AS workStartTime,work_end_time AS workEndTime,work_days_json AS workDaysJson,rotation_start_date AS rotationStartDate,rotation_days_on AS rotationDaysOn,rotation_days_off AS rotationDaysOff,grace_period_minutes AS gracePeriodMinutes FROM employees WHERE status='active'`).all<any>();
  let created = 0;
  for (const employee of (employees.results || []) as any[]) {
    ...
    let code: string | null = null, cutoff = safetyCutoff, rawMinutes = 0;
    if (!firstIn && lastOut && lastOut >= shift.start.getTime() && lastOut <= safetyCutoff.getTime()) { code = "MISSING_CLOCK_IN"; cutoff = new Date(lastOut); }
    else if (firstIn && !lastOut && nowMs >= safetyCutoff.getTime()) { code = "MISSING_CLOCK_OUT"; rawMinutes = Math.max(0, Math.round((Math.min(nowMs, shift.end.getTime()) - firstIn) / 60000)); }
    else if (!firstIn && !lastOut && nowMs >= safetyCutoff.getTime()) code = "NO_SHOW";
    if (!code) continue;
