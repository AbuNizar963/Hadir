...
async function buildAIContext(actor:any,env:Env){
 if(actor?.role==="staff"){
  ...
  return{employee,attendance:attendance.results||[],requests:requests.results||[],leaveRequests:leaveRequests.results||[],escapes:escapes.results||[]};
 }
