...
const requestType = (value: unknown): RequestRow["type"] => {
  const type = String(value || "");
  return type === "permission" || type === "leave" || type === "checkout"
    ? type
    : (null as never);
};
...
async function notify(
  ...
  await env.DB.prepare(
    ...
    .bind(uid(), id, title, body, type, type, createdAt)
    .run();
...
async function refreshEmployeeReportFacts(
  env: Env,
  employeeId: string,
  from: string,
  to: string,
  ...
  const start = dateValue(from),
    end = dateValue(to) || start;
  if (!start || !end || end < start) return;
  await refreshProfessionalAttendanceFacts(
    env,
    start,
    end,
    { id: employeeId, role: "staff" },
    employeeId,
  );
}
export async function handleRequests(
  ...
  if (path === "/api/requests" && method === "GET") {
    ...
    const rows =
      actor.role === "staff"
        ? await env.DB.prepare(
            "SELECT id,employee_id AS employeeId,employee_name AS employeeName,job_number AS jobNumber,type,reason,status,created_at AS createdAt,start_date AS startDate,end_date AS endDate FROM requests WHERE employee_id=? ORDER BY created_at DESC LIMIT 200",
          )
            .bind(actor.id)
            .all<RequestRow>()
        : await env.DB.prepare(
            "SELECT id,employee_id AS employeeId,employee_name AS employeeName,job_number AS jobNumber,type,reason,status,created_at AS createdAt,start_date AS startDate,end_date AS endDate FROM requests ORDER BY created_at DESC LIMIT 500",
          ).all<RequestRow>();
  ...
  if (path === "/api/requests" && method === "POST") {
    if (
      !actor ||
      (!["owner", "manager"].includes(actor.role) && actor.role !== "staff")
    )
      return json({ error: "غير مصرح" }, 403, origin);
    ...
    if (endDate < startDate)
      return json(
        { error: "تاريخ النهاية يجب أن يكون بعد أو مساويًا لتاريخ البداية" },
        400,
        origin,
      );
    ...
    const employee = await env.DB.prepare(
      ...
      .bind(employeeId)
      .first<{ id: string; jobNumber: string; name: string }>();
    ...
    if (createdByManager) {
      ...
      try {
        await notify(
          env,
          employee.id,
          type === "leave" ? "تم تسجيل إجازة" : "تم تسجيل استئذان",
          `${typeLabel(type)} من ${startDate} إلى ${endDate}.`,
          "success",
          "/employee/notifications",
        );
      } catch {}
      return json(
        { ok: true, id: requestId, status: initialStatus, startDate, endDate },
        201,
        origin,
      );
    }
    ...
    try {
      for (const recipientId of recipients)
        await notify(
          ...
          recipientId,
          "طلب موظف جديد",
          `${employee.name} أرسل طلب ${typeLabel(type)} من ${startDate} إلى ${endDate}.`,
          "info",
          "/manager/requests",
        );
    } catch (error) {
    ...
    return json(
      { ok: true, id: requestId, status: "pending", startDate, endDate },
      201,
      origin,
    );
  }
  if (requestMatch && method === "PATCH") {
    ...
    const row = await env.DB.prepare(
      "SELECT employee_id AS employeeId,start_date AS startDate,end_date AS endDate FROM requests WHERE id=? AND status='pending' LIMIT 1",
    )
      .bind(requestMatch[1])
      .first<{
        employeeId: string;
    ...
    try {
      await notify(
        ...
        status === "approved"
          ? "تمت الموافقة على طلبك من قبل الإدارة."
      ...
      await env.DB.prepare(
        "UPDATE requests SET status='pending' WHERE id=? AND status=?",
      )
      ...
      return json(
        {
          error: "تمت إعادة الطلب إلى قيد المراجعة لأن إشعار الموظف لم يُنشأ",
          detail: error instanceof Error ? error.message : String(error),
        },
        500,
        origin,
  ...
  if (confirmMatch && method === "POST") {
    ...
    const requestRow = await env.DB.prepare(
      "SELECT start_date AS startDate,end_date AS endDate FROM requests WHERE id=? AND employee_id=? AND status='approved' LIMIT 1",
    )
      .bind(confirmMatch[1], actor.id)
      .first<{ startDate: string | null; endDate: string | null }>();
    if (!requestRow)
      return json({ error: "لا يمكن التأكيد قبل موافقة المدير" }, 403, origin);
