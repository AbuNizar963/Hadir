...
const minutesBetween = (from: string | null, to: string | null) => { if (!from || !to) return null; const a = Date.parse(from), b = Date.parse(to); if (!Number.isFinite(a) || !Number.isFinite(b) || b < a) return null; return Math.round((b - a) / 60000); };
const json = (value: unknown) => JSON.stringify(value ?? []);
function tzParts(date: Date) { const parts = new Intl.DateTimeFormat("en-CA", { timeZone: TZ, year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hour12: false }).formatToParts(date); const get = (type: string) => parts.find((part) => part.type === type)?.value || ""; return { year: Number(get("year")), month: Number(get("month")), day: Number(get("day")), hour: Number(get("hour")), minute: Number(get("minute")) }; }
...
export async function materializeDay(env: Env, day: string, actor: any, employeeId?: string) {
  ...
  const requestResult = employeeId ? await env.DB.prepare("SELECT id,employee_id AS employeeId,type,status,start_date AS startDate,end_date AS endDate,created_at AS createdAt FROM requests WHERE employee_id=? AND status IN ('approved','confirmed')").bind(employeeId).all<any>() : await env.DB.prepare("SELECT id,employee_id AS employeeId,type,status,start_date AS startDate,end_date AS endDate,created_at AS createdAt FROM requests WHERE status IN ('approved','confirmed')").all<any>();
  const requestsByEmployee = new Map<string, any[]>(); for (const requestRow of requestResult.results || []) { const id = String(requestRow.employeeId || ""); if (!id) continue; const requestStart = String(requestRow.startDate || requestRow.createdAt || "").slice(0, 10); const requestEnd = String(requestRow.endDate || requestRow.startDate || requestRow.createdAt || "").slice(0, 10); if (!requestStart || !requestEnd || requestStart > day || day > requestEnd) continue; const list = requestsByEmployee.get(id) || []; list.push(requestRow); requestsByEmployee.set(id, list); }
  const auditStart = localMidnightUtc(addDays(day, -7)).toISOString(), auditEnd = localMidnightUtc(addDays(day, 8)).toISOString();
  const auditResult = employeeId ? await env.DB.prepare("SELECT id,employee_id AS employeeId,timestamp FROM audit WHERE employee_id=? AND timestamp>=? AND timestamp<? ORDER BY timestamp ASC").bind(employeeId, auditStart, auditEnd).all<any>() : await env.DB.prepare("SELECT id,employee_id AS employeeId,timestamp FROM audit WHERE timestamp>=? AND timestamp<? ORDER BY timestamp ASC").bind(auditStart, auditEnd).all<any>();
  ...
  for (const e of filtered) {
    const id = String(e.employeeId || ""); if (!id) continue; const isRotation = String(e.scheduleType || "").toUpperCase() === "ROTATION"; const shiftStartMs = Date.parse(String(e.scheduledStart || "")); const shiftEndMs = Date.parse(String(e.scheduledEnd || ""));
    const events = (eventsByEmployee.get(id) || []).filter((x) => { const ts = Date.parse(String(x.timestamp)); if (!Number.isFinite(ts)) return false; if (isRotation && Number.isFinite(shiftStartMs) && Number.isFinite(shiftEndMs)) return ts >= shiftStartMs && ts <= shiftEndMs; return ts >= dayStartMs && ts < dayEndMs; });
    const ins = events.filter((x) => String(x.type) === "check-in"); const outs = events.filter((x) => String(x.type) === "check-out");
    const checkInAt = String(e.checkInAt || ins[0]?.timestamp || "") || null; const checkOutAt = String(e.checkOutAt || outs[outs.length - 1]?.timestamp || "") || null;
    const workedMinutes = minutesBetween(checkInAt, checkOutAt); const expectedMinutes = minutesBetween(String(e.scheduledStart || "") || null, String(e.scheduledEnd || "") || null); const lateMinutes = String(e.status) === "LATE" && e.scheduledStart && checkInAt ? Math.max(0, minutesBetween(e.scheduledStart, checkInAt) || 0) : 0; const earlyLeaveMinutes = e.scheduledEnd && checkOutAt ? Math.max(0, minutesBetween(checkOutAt, e.scheduledEnd) || 0) : 0; const overtimeMinutes = e.scheduledEnd && checkOutAt ? Math.max(0, minutesBetween(e.scheduledEnd, checkOutAt) || 0) : 0; const open = checkInAt && !checkOutAt ? 1 : 0; const exceptionCode = outs.length && !ins.length ? "CHECKOUT_WITHOUT_CHECKIN" : open ? "MISSING_CHECKOUT" : String(e.status) === "ABSENT" ? "ABSENT_NO_APPROVED_REASON" : lateMinutes ? "LATE_ARRIVAL" : earlyLeaveMinutes ? "EARLY_LEAVE" : overtimeMinutes ? "OVERTIME" : null;
    const requests = requestsByEmployee.get(id) || []; const audits = (auditsByEmployee.get(id) || []).filter((x) => { const ts = Date.parse(String(x.timestamp)); if (!Number.isFinite(ts)) return false; return isRotation && Number.isFinite(shiftStartMs) && Number.isFinite(shiftEndMs) ? ts >= shiftStartMs && ts <= shiftEndMs : ts >= dayStartMs && ts < dayEndMs; });
    const scheduleSnapshot = { scheduleType: e.scheduleType || null, scheduledStart: e.scheduledStart || null, scheduledEnd: e.scheduledEnd || null, expectedMinutes, timezone: TZ, capturedAt: new Date().toISOString() };
    statements.push(env.DB.prepare(`INSERT INTO attendance_reporting_facts (attendance_day,employee_id,job_number,employee_name,location_id,status,schedule_type,scheduled_start,scheduled_end,expected_minutes,check_in_at,check_out_at,worked_minutes,late_minutes,early_leave_minutes,overtime_minutes,open,exception_code,attendance_event_ids_json,request_ids_json,audit_ids_json,calculation_source,calculation_version,historical_data_quality,timezone,computed_at,schedule_snapshot_json,data_quality_reason) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(attendance_day,employee_id) DO UPDATE SET job_number=excluded.job_number,employee_name=excluded.employee_name,location_id=excluded.location_id,status=excluded.status,schedule_type=excluded.schedule_type,scheduled_start=excluded.scheduled_start,scheduled_end=excluded.scheduled_end,expected_minutes=excluded.expected_minutes,check_in_at=excluded.check_in_at,check_out_at=excluded.check_out_at,worked_minutes=excluded.worked_minutes,late_minutes=excluded.late_minutes,early_leave_minutes=excluded.early_leave_minutes,overtime_minutes=excluded.overtime_minutes,open=excluded.open,exception_code=excluded.exception_code,attendance_event_ids_json=excluded.attendance_event_ids_json,request_ids_json=excluded.request_ids_json,audit_ids_json=excluded.audit_ids_json,calculation_source=excluded.calculation_source,calculation_version=excluded.calculation_version,computed_at=excluded.computed_at,schedule_snapshot_json=CASE WHEN attendance_reporting_facts.historical_data_quality='exact' AND excluded.historical_data_quality='reconstructed' THEN attendance_reporting_facts.schedule_snapshot_json ELSE excluded.schedule_snapshot_json END,data_quality_reason=excluded.data_quality_reason`).bind(day,id,String(e.jobNumber || ""),String(e.employeeName || ""),e.locationId || null,String(e.status || "INVALID"),String(e.scheduleType || "ADMIN"),e.scheduledStart || null,e.scheduledEnd || null,expectedMinutes,checkInAt,checkOutAt,workedMinutes,lateMinutes,earlyLeaveMinutes,overtimeMinutes,open,exceptionCode,json(events.map((x) => String(x.id))),json(requests.map((x) => String(x.id))),json(audits.map((x) => String(x.id))),"attendance+requests+schedule+daily_attendance_status","report-v2",quality,TZ,new Date().toISOString(),json(scheduleSnapshot),qualityReason));
  }
