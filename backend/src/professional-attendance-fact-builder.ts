...
export async function materializeDay(env: Env, day: string, actor: any, employeeId?: string) {
  ...
  for (const e of eligible) {
    const id = String(e.employeeId || ""); if (!id) continue; const isRotation = String(e.scheduleType || "").toUpperCase() === "ROTATION"; const dailyRotationAttendance = isRotation && Boolean(e.rotationDailyAttendanceEnabled);
    const events = (eventsByEmployee.get(id) || []).filter((x) => { const ts = Date.parse(String(x.timestamp)); if (!Number.isFinite(ts)) return false; if (isRotation && !dailyRotationAttendance && e.scheduledStart && e.scheduledEnd) { const shiftStartMs = Date.parse(String(e.scheduledStart)); const shiftEndMs = Date.parse(String(e.scheduledEnd)); return Number.isFinite(shiftStartMs) && Number.isFinite(shiftEndMs) && ts >= shiftStartMs && ts <= shiftEndMs; } return ts >= dayStartMs && ts < dayEndMs; });
    ...
    const workedMinutes = e.workedMinutes ?? null; const expectedMinutes = e.expectedMinutes ?? null; const lateMinutes = Number(e.lateMinutes || 0); const earlyLeaveMinutes = Number(e.earlyLeaveMinutes || 0); const overtimeMinutes = Number(e.overtimeMinutes || 0); const open = Number(e.open || 0); const exceptionCode = e.exceptionCode || null;
