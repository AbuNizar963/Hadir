-- Final repair for the canonical daily-status cache.
-- Some production databases still have the pre-ESCAPED CHECK constraint even after
-- the earlier status migrations. Rebuild the derived table with every status emitted
-- by the canonical attendance engine, including ESCAPED and OPEN.
PRAGMA foreign_keys = OFF;

CREATE TABLE IF NOT EXISTS daily_attendance_status__final (
...
INSERT OR REPLACE INTO daily_attendance_status__final
...
ALTER TABLE daily_attendance_status__final RENAME TO daily_attendance_status;
