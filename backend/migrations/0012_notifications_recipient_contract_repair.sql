...
-- Re-apply the canonical notification contract safely. This repair is
-- intentionally idempotent at the migration level and preserves existing
-- notification rows while guaranteeing recipient_id is writable with a
-- default and that legacy user_id/message writes remain compatible.
ALTER TABLE notifications RENAME TO notifications_pre_0012;
...
SELECT
  ...
  read_at,
  created_at
FROM notifications_pre_0012;

DROP TABLE notifications_pre_0012;
...
-- Compatibility for any remaining legacy write path that only supplies
-- user_id/message. Canonical code should still provide recipient_id/body.
